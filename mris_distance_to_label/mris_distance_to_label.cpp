...
static char vcid[] = "$Id: mris_distance_to_label.cpp,v 1.1 2005/02/05 23:07:26 segonne Exp $";
...
int main(int argc, char *argv[])
{
  ...
  nargs = handle_version_option (argc, argv, "$Id: mris_distance_to_label.cpp,v 1.1 2005/02/05 23:07:26 segonne Exp $", "$Name:  $");
	...
	for( m = 0 ; m < nsubjects ; m++){
		...
		for(n=0 ; n < nlabels ; n++){
			...
			if(labels[n]>=0){
				...
				MRIextractDistanceMap(mri,mri_distance,labels[n],distance,mode);
				
				fprintf(stderr, "extracting distance values for label %d\n", labels[n]) ;
				mrisExtractMRIvalues(mris,mri,mri_distance,distance,mode);
