...
int vtkKWWindowCppCommand(vtkKWWindow *op, 
                          Tcl_Interp *interp,
                          int argc, char *argv[]);

int VTKTCL_EXPORT vtkKWQdecWindowCppCommand(vtkKWQdecWindow *op, 
                                            Tcl_Interp *interp,
                                            int argc, char *argv[]);

int VTKTCL_EXPORT vtkKWQdecWindowCommand(ClientData cd, 
                                         Tcl_Interp *interp,
                                         int argc, char *argv[])
{
  ...
  return vtkKWQdecWindowCppCommand(
    (vtkKWQdecWindow *)(((vtkTclCommandArgStruct *)cd)->Pointer),
    interp, argc, argv);
}
                                            ...
                                            int argc, char *argv[])
{
  ...
  if (argc < 2)
  {
    Tcl_SetResult(interp, 
                  (char *) "Could not find requested method.", 
                  TCL_VOLATILE);
    return TCL_ERROR;
  }
  if (!interp)
  ...
  try
  {
    ...
    if ((!strcmp("SafeDownCast",argv[1]))&&(argc == 3))
    {
      ...
      temp0 = (vtkObject *)(vtkTclGetPointerFromObject(argv[2],
                                                       (char *) "vtkObject",
                                                       interp,error));
      if (!error)
      {
    ...
    if ((!strcmp("SetCurrentSurfaceScalarsFromTableSelection",argv[1])) &&
        (argc == 2))
    {
    ...
    if ((!strcmp("ContinuousPlotGraphMouseoverEnterElement",argv[1])) &&
        (argc == 3))
    {
      char    *temp0;
      error = 0;
    ...
    if ((!strcmp("ContinuousPlotGraphMouseoverExitElement",argv[1])) &&
        (argc == 2))
    {
    ...
    if ((!strcmp("ContinuousPlotGraphSetUpContextualMenu",argv[1])) &&
        (argc == 4))
    {
      char    *temp0;
      vtkKWMenu  *temp1;
      ...
      temp0 = argv[2];
      temp1 = (vtkKWMenu *)(vtkTclGetPointerFromObject(argv[3],
                                                       (char *) "vtkKWMenu",
                                                       interp,
                                                       error));
      if (!error)
    ...
    {
      char temps2[256];
      sprintf(temps2,"Object named: %s, could not find requested method: "
