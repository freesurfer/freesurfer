...
main(int argc, char *argv[])
{
  ...
  sprintf(parms.base_name, "%s_inner_skull%s%s", "test", output_suffix, suffix) ;
	parms.mri_brain = mri_masked ; l_spring = parms.l_spring_norm ;
	...
	for (x = 0 ; x < mri_src->width ; x++)
	{
		for (y = 0 ; y < mri_src->height ; y++)
		{
			for (z = 0 ; z < mri_src->depth ; z++)
			{
				...
				outside /= nsamples ; inside /= nsamples ;
...
MRISpositionOptimalSphere(MRI_SURFACE *mris, MRI *mri_inner, float sample_dist)
{
	...
	min_x0 = mris->xctr ; min_y0 = mris->yctr ; min_z0 = mris->zctr ;
	min_r = mris->radius ;
	...
	{
		printf("scale = %d\n", scale) ;
		xmin = min_x0-scale ; xmax = min_x0+scale ;
		ymin = min_y0-scale ; ymax = min_y0+scale ;
		zmin = min_z0-scale ; zmax = min_z0+scale ;
...
compute_surface_sse(MRI_SURFACE *mris, MRI *mri, float sample_dist)
{
	...
	for (nsamples = 0, sse = 0.0, vno = 0 ; vno < mris->nvertices ; vno++)
	{
		...
		{
			x = v->x + dist*nx ; y = v->y + dist*ny ; z = v->z + dist*nz ;
...
MRISrepositionToInnerSkull(MRI_SURFACE *mris, MRI *mri_smooth, INTEGRATION_PARMS *parms)
{
  ...
  if (parms->momentum < 0.0)
	...
	MRIwrite(mri_bin, "bin.mgz") ; MRIwrite(mri_dist, "dist.mgz") ;
	...
	MRIfree(&mri_kernel) ; MRIfree(&mri_bin_smooth) ;
	...
	for (ic_order = 3 ; ic_order <= 3 ; ic_order++)
	{
		if (ic_order != ic_init)
		{
			...
			MRISfree(&mris) ; mris = mris_new ; 
		}

		printf("********************** using ICO order %d *********************\n", ic_order) ;
		parms->n_averages = avgs ; parms->l_spring_norm = l_spring ;
	...
	MRIfree(&mri_bin) ; MRIfree(&mri_dist) ;
	return(NO_ERROR) ;
}
...
MRISfindOptimalRigidPosition(MRI_SURFACE *mris, MRI *mri, INTEGRATION_PARMS *parms)
{
	...
	do
	{
		...
		sse = compute_surface_dist_sse(mris, mri) ;
		dx_total += dt*dx ; dy_total += dt*dy ; dz_total += dt*dz ; 
...
compute_rigid_gradient(MRI_SURFACE *mris, MRI *mri, double *pdx, double *pdy, double *pdz)
{
	...
	dx /= mris->nvertices ; dy /= mris->nvertices ; dz /= mris->nvertices ; 
