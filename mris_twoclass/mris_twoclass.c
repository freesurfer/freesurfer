...
static char vcid[] = "$Id: mris_twoclass.c,v 1.1 2000/10/10 16:43:54 fischl Exp $";
...
static int   cvector_compute_pvalues(float *c1_mean, float *c1_var, 
...
#define ARGV_OFFSET 5
  ...
  total_mean = cvector_alloc(nvertices) ; c1_mean = cvector_alloc(nvertices) ;
  pvals = cvector_alloc(nvertices) ; c2_mean = cvector_alloc(nvertices) ;
  c1_var = cvector_alloc(nvertices) ; c2_var = cvector_alloc(nvertices) ;
  ...
  cvector_compute_pvalues(c1_mean, c1_var, c2_mean, c2_var, num_class1, 
                             num_class2, pvals, nvertices, stat_type) ;
    ...
    if (read_dir == NULL)  /* recompute everything */
    {
      ...
      */
      ...
      for (avgs = 1 ; avgs <= max_avgs ; avgs++)
      {
        ...
        if (!(avgs%10))
        {
          printf("%d vertices found at scale %d\n", total_found, avgs) ;
          if (!total_found)
            break ;
          total_found = 0 ;
        }
  ...
  {
    MRISimportValVector(mris, vbest_avgs) ;
    sprintf(fname, "%s_optimal_scale-%s.w", out_prefix, hemi) ; 
    MRISwriteValues(mris, fname) ;
  }
  MRISimportValVector(mris, pvals) ;  /* was vbest_pvalues */
  sprintf(fname, "%s_%s-%s.w", out_prefix, stat_type == STAT_T ? "t" : "t",
          hemi) ; 
  MRISwriteValues(mris, fname) ;
  if (condition_0 >= 0)  /* write means and variances */
  {
    ...
    MRISimportValVector(mris, c1_best_mean) ; MRISwriteValues(mris, fname) ;
    sprintf(fname, "%s/sigvar%d-%s.w", path, condition_0, hemi) ; 
    MRISimportValVector(mris, c1_best_var) ; MRISwriteValues(mris, fname) ;

    /* write condition 1 stats */
    sprintf(fname, "%s/sigavg%d-%s.w", path, condition_1, hemi) ; 
    MRISimportValVector(mris, c2_best_mean) ; MRISwriteValues(mris, fname) ;
    sprintf(fname, "%s/sigvar%d-%s.w", path, condition_1, hemi) ; 
    MRISimportValVector(mris, c2_best_var) ; MRISwriteValues(mris, fname) ;
  }
                        ...
                        float *pvals, int num, int stat_type)
{
  ...
  case STAT_T:
    return(cvector_compute_t_test(c1_mean, c1_var, c2_mean, c2_var, 
                                  num_class1, num_class2, pvals, num)) ;
    break ;
  }
  return(NO_ERROR) ;
}
