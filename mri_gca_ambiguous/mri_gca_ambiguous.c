...
static char vcid[] = "$Id: mri_gca_ambiguous.c,v 1.2 2004/02/26 19:03:34 fischl Exp $";
...
main(int argc, char *argv[])
{
  ...
  nargs = handle_version_option (argc, argv, "$Id: mri_gca_ambiguous.c,v 1.2 2004/02/26 19:03:34 fischl Exp $", "$Name:  $");
	...
	case 3:
		{
			...
			TRs[0]  = TR ; TRs[1] = TR ; TRs[2] = TR ;
			TEs[0]  = TE ; TEs[1] = TE ; TEs[2] = TE ;
			min_amb = 10000000  ; min_fa3 = min_fa1 = min_fa2 = -1 ; 
			scale = 1.0/(gca->prior_width * gca->prior_height * gca->prior_depth);
			for (fa1 = MIN_FA1  ; fa1 <= MAX_FA1 ; fa1 += FA_STEP)
			{
				FAs[0]  = RADIANS(fa1)  ;
			...
			FAs[0] = min_fa1 ; FAs[1] = min_fa2 ; FAs[2] = min_fa3 ;
#if 0
			gca_flash = GCAcreateFlashGCAfromParameterGCA(gca, TRs, FAs, TEs, 3, lambda);
			mri =  GCAcomputeAmbiguity(gca_flash, NULL, &amb, class) ;
			MRIwrite(mri, out_name) ;
...
#define ISTEPS 100
							...
							else if (gca->ninputs == 2)
							{
								...
								v1 = VectorAlloc(2, MATRIX_REAL) ; v2 = VectorAlloc(2, MATRIX_REAL) ;
...
GCAcompute1DAmbiguity(GCA *gca, MRI *mri, double *pamb, int class)
{
  ...
  mri->x_s =  0; mri->y_s = -1; mri->z_s =  0; mri->c_s = 0.0;
		...
		for  (yp  = 0 ;  yp < gca->prior_height ; yp++)
		{
			for (zp = 0 ;  zp  < gca->prior_depth  ; zp++)
			{
				...
				for (i1 = 0 ; i1  <  gcap->nlabels ; i1++)
				{
					...
					Imin1 = gc1->means[0]-NSTDS*std1 ; Imax1 = gc1->means[0]+NSTDS*std1 ;
					Istep1 = (Imax1-Imin1)/MAX_VALS ;
					Imins1[i1] = Imin1 ; Imaxs1[i1] = Imax1 ;

					m_cov_inv = load_inverse_covariance_matrix(gc1, m_cov_inv, gca->ninputs) ;
					if (!v)
					{
						v = VectorAlloc(1, MATRIX_REAL) ; vtmp = VectorAlloc(1, MATRIX_REAL) ;
					}
				...
				for (n = 0, amax =  atotal = 0.0, i1 = 0 ;  i1 < gcap->nlabels ; i1++)
				{
					for (i1 = 0 ;  i1 < gcap->nlabels ; i1++)
					{
						...
						for (i2 = 0 ;  i2 < i1 ; i2++)
						{
							...
							if (l1 == Left_Hippocampus && l2 == Left_Amygdala)
								DiagBreak() ;
							Imin1 = MAX(Imins1[i1], Imins1[i2]) ; Imax1 = MIN(Imaxs1[i1], Imaxs1[i2]) ;
							Istep1 = (Imax1-Imin1)/MAX_VALS ; 
	...
	else
	{
		...
		MatrixFree(&m_cov_inv) ; VectorFree(&v) ; VectorFree(&vtmp) ;
	}
	*pamb = total_ambiguity;
	return(mri) ;
}
		...
		for  (yp  = 0 ;  yp < gca->prior_height ; yp++)
		{
			for (zp = 0 ;  zp  < gca->prior_depth  ; zp++)
			{
				...
				for (i1 = 0 ; i1  <  gcap->nlabels ; i1++)
				{
					...
					Imin2 = gc1->means[1]-NSTDS*std2 ; Imax2 = gc1->means[1]+NSTDS*std2 ;
					Istep2 = (Imax2-Imin2)/MAX_VALS ;
					Imins2[i1] = Imin2 ; Imaxs2[i1] = Imax2 ;

					m_cov_inv = load_inverse_covariance_matrix(gc1, m_cov_inv, gca->ninputs) ;
					if (!v)
					{
						v = VectorAlloc(2, MATRIX_REAL) ; vtmp = VectorAlloc(2, MATRIX_REAL) ;
					}
					pscale = (1.0 / (pow(2*M_PI,gca->ninputs/2.0)*sqrt(covariance_determinant(gc1, gca->ninputs)))); 
					for (index1 = 0, I1 = Imin1 ; I1 <= Imax1 ; I1 += Istep1, index1++)
					{
					...
					for (i1 = 0 ;  i1 < gcap->nlabels ; i1++)
					{
						...
						for (i2 = 0 ;  i2 < i1 ; i2++)
						{
							...
							if (l1 == Left_Hippocampus && l2 == Left_Amygdala)
								DiagBreak() ;
							Imin1 = MAX(Imins1[i1], Imins1[i2]) ; Imax1 = MIN(Imaxs1[i1], Imaxs1[i2]) ;
							Imin2 = MAX(Imins2[i1], Imins2[i2]) ; Imax2 = MIN(Imaxs2[i1], Imaxs2[i2]) ;
							Istep1 = (Imax1-Imin1)/MAX_VALS ; Istep2 = (Imax2-Imin2)/MAX_VALS ;
							for (ambiguity = 0,  I1 = Imin1 ; I1 < Imax1 ; I1 += Istep1)
							{
								index1_c1 = nint(MAX_VAL * (I1 - Imins1[i1]) / (Imaxs1[i1]-Imins1[i1])) ;
								if (index1_c1 < 0 || index1_c1 > MAX_VAL)
		...
		for  (yp  = 0 ;  yp < gca->prior_height ; yp++)
		{
			for (zp = 0 ;  zp  < gca->prior_depth  ; zp++)
			{
				...
				for (i1 = 0 ; i1  <  gcap->nlabels ; i1++)
				{
					...
					Imin3 = gc1->means[2]-NSTDS*std3 ; Imax3 = gc1->means[2]+NSTDS*std3 ;
					Istep3 = (Imax3-Imin3)/MAX_VALS ;
					Imins3[i1] = Imin3 ; Imaxs3[i1] = Imax3 ;

					m_cov_inv = load_inverse_covariance_matrix(gc1, m_cov_inv, gca->ninputs) ;
					if (!v)
					{
						v = VectorAlloc(3, MATRIX_REAL) ; vtmp = VectorAlloc(3, MATRIX_REAL) ;
					}
					pscale = (1.0 / (pow(2*M_PI,gca->ninputs/2.0)*sqrt(covariance_determinant(gc1, gca->ninputs)))); 
					for (index1 = 0, I1 = Imin1 ; I1 <= Imax1 ; I1 += Istep1, index1++)
					{
				...
				for (n = 0, amax =  atotal = 0.0, i1 = 0 ;  i1 < gcap->nlabels ; i1++)
				{
					for (i1 = 0 ;  i1 < gcap->nlabels ; i1++)
					{
						...
						for (i2 = 0 ;  i2 < i1 ; i2++)
						{
							...
							Imin1 = MAX(Imins1[i1], Imins1[i2]) ; Imax1 = MIN(Imaxs1[i1], Imaxs1[i2]) ;
							Imin2 = MAX(Imins2[i1], Imins2[i2]) ; Imax2 = MIN(Imaxs2[i1], Imaxs2[i2]) ;
							Imin3 = MAX(Imins3[i1], Imins3[i2]) ; Imax3 = MIN(Imaxs3[i1], Imaxs3[i2]) ;
							Istep1 = (Imax1-Imin1)/MAX_VALS ; Istep2 = (Imax2-Imin2)/MAX_VALS ;
							Istep3 = (Imax3-Imin3)/MAX_VALS ;
							for (ambiguity = 0,  I1 = Imin1 ; I1 < Imax1 ; I1 += Istep1)
							{
