...
// Revision Date  : $Date: 2005/11/15 15:02:35 $
															...
															MATRIX **pm_cov)
{
	...
	for (dofs_T1 = dofs_T2 = i = 0 ; i < l_damaged_wm->n_points ; i++)
	{
		...
		xv = V3_X(v_T1) ; yv = V3_Y(v_T1) ; zv = V3_Z(v_T1) ;
		...
		xv = V3_X(v_T2) ; yv = V3_Y(v_T2) ; zv = V3_Z(v_T2) ;
	...
	VectorFree(&v_T1) ; VectorFree(&v_T2) ; VectorFree(&v_ras) ; VectorFree(&v_X) ; VectorFree(&v_XT) ;
	MatrixFree(&m_T1_ras2vox) ; MatrixFree(&m_T2_ras2vox) ; MatrixFree(&m_tmp) ;
	*pv_mean = v_mean ;
												 ...
												 MATRIX **pm_inv_cov, int erode)
{
	...
	VECTOR_ELT(v_mean, 2) /= (float)dofs_T2 ;
	VECTOR_ELT(v_mean, 3) /= (float)dofs_T2 ;
	printf("mean %s (%d):\n", cma_label_to_name(label), label) ; MatrixPrint(stdout, v_mean) ;
	...
	if (dofs_T1 > 0)
		MatrixScalarMul(m_cov, 1.0/(float)dofs_T1, m_cov) ;
	printf("covariance matrix:\n") ; MatrixPrint(stdout, m_cov) ;
	VectorFree(&v_T1) ; VectorFree(&v_T2) ; VectorFree(&v_X) ; VectorFree(&v_XT) ;
	MatrixFree(&m_T1_to_T2) ; MatrixFree(&m_tmp) ;
	*pv_mean = v_mean ;
								 ...
								 VECTOR **v_means, MATRIX **m_inv_covariances, TRANSFORM *transform, GCA *gca)
{
	...
	for (ndamaged = x = 0 ; x < mri_PD->width ; x++)
	{
		...
		for (y = 0 ; y < mri_PD->height ; y++)
		{
			...
			for (z = 0 ; z < mri_PD->depth ; z++)
			{
				...
				xv = nint(V3_X(v_T1)) ; yv = nint(V3_Y(v_T1)) ; zv = nint(V3_Z(v_T1)) ;
				if (MRIindexNotInVolume(mri_T1, xv, yv, zv) != 0)
					continue ;
	...
	MatrixFree(&m_T2_to_T1) ; VectorFree(&v_T1) ; VectorFree(&v_T2) ;
	VectorFree(&v_vals) ; VectorFree(&v_vals_minus_means) ;
	VectorFree(&v_tmp) ;
