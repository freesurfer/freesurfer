/* $Id: togl.c,v 1.9 2007/10/22 04:39:32 kteich Exp $ */
...
static void (*SetClassProcsPtr) _ANSI_ARGS_
...
#if defined(WIN32)
  HDC tglGLHdc;/* context of device that OpenGL calls will be drawn on */
...
#else
  if (major >= 8 && minor >= 4) {
    TCL_ERR(interp,"Sorry, this instance of Togl was not "
            "compiled to work with Tcl/Tk 8.4 or higher.");
  }
#endif
...
static void Togl_Timer( ClientData clientData ) {
  ...
  if (togl->TimerProc) {
    ...
    togl->timerHandler =
      ...
      ( togl->TimerInterval, Togl_Timer, (ClientData)togl );
#else
    togl->timerHandler =
      Tk_CreateTimerHandler
      ( togl->TimeInterval, Togl_Timer, (ClientData)togl );
#endif
  }
}
...
static void SetMacBufRect(struct Togl *togl) {
  ...
  wrect[1] = ((TkWindow *)\
              (togl->TkWin))->privatePtr->toplevel->portPtr->portRect.bottom -
...
static void Togl_Render( ClientData clientData ) {
  ...
  if (togl->DisplayProc) {
...
#ifdef macintosh
    /* Mac is complicated here because OpenGL needs to
       know what part of the parent
       window to render into, and it seems that region
       need to be invalidated before
       drawing, so that QuickDraw will allow OpenGL
       to transfer pixels into that
       part of the window. I'm not even totally sure
       how or why this works as it
    ...
    /* this may seem an odd place to put this, with possibly redundant calls to
       aglSetInteger(AGL_BUFFER_RECT...), but for some reason
...
static LRESULT CALLBACK Win32WinProc( HWND hwnd, UINT message,
  ...
  case WM_WINDOWPOSCHANGED:
    /* Should be processed by DefWindowProc, otherwise a double buffered
       context is not properly resized when the corresponding
       window is resized.*/
    break;
  case WM_DESTROY:
    if (togl->tglGLHglrc) {
...
static int generateEPS(const char *filename, int inColor,
  ...
  for ( i = 0; i < width * height * components; ) {
    ...
    if ( inColor) {
      double pix = 0.0;
      pix = 0.30 *
            ( double)curpix[i] + 0.59 *
            ( double)curpix[i+1] + 0.11 *
            ( double)curpix[i+2];
      i += 3;
...
int Togl_DumpToEpsFile( const struct Togl *togl, const char *filename,
   ...
   * MesaGl: glXCreateGLXPixmapMESA
   *  ( dpy, vi, eps_pixmap, Tk_Colormap(togl->TkWin))
  ...
  if ( using_mesa)
    eps_glxpixmap =
      glXCreateGLXPixmapMESA( dpy, vi, eps_pixmap, Tk_Colormap(togl->TkWin));
  else
...
#ifdef macintosh
/* needed to make shared library on Mac with CodeWarrior;
   should be overridden by user app */
/*
