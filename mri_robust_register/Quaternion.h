...
//           a(real), b(img[0]), c(img[1]),d(img[2]),normed(tnormed) {};
     ...
     inline std::vector < double > getImg() const {std::vector <double> v(3); v[0] = b; v[1] = c; v[2] = d; return v;};
...
// m[16] in homogeneous coordinates
{
   ...
   std::vector < double > m(16);
   m[ 0] = mr[0]; m[ 1] = mr[1]; m[ 2] = mr[2]; m[ 3] = 0;
   m[ 4] = mr[3]; m[ 5] = mr[4]; m[ 6] = mr[5]; m[ 7] = 0;
   m[ 8] = mr[6]; m[ 9] = mr[7]; m[10] = mr[8]; m[11] = 0;
...
// This is one of the Euler Angle definitions
{
   ...
   Quaternion qz1; qz1.importRotVec(z1,0,0,1);
   Quaternion qx2; qx2.importRotVec(x2,1,0,0);
   Quaternion qz3; qz3.importRotVec(z3,0,0,1);
...
// This is one of the Euler Angle definitions
{
   ...
   Quaternion qx1; qx1.importRotVec(x1,1,0,0);
   Quaternion qy2; qy2.importRotVec(y2,0,1,0);
   Quaternion qz3; qz3.importRotVec(z3,0,0,1);
...
// This is one of the Euler Angle definitions
{
   ...
   Quaternion qz1; qz1.importRotVec(z1,0,0,1);
   Quaternion qy2; qy2.importRotVec(y2,0,1,0);
   Quaternion qx3; qx3.importRotVec(x3,1,0,0);
