...
pair < double , double> JointHisto::getMinMax()
{
  ...
  {
    tmp = histo[i][j];
    if (mm.first > tmp) mm.first = tmp;
    if (mm.second < tmp) mm.second = tmp;
  }
...
//         else
        ...
        //    zt>=0.0 && zt<dt[2]-1 && yt>=0.0 && yt<dt[1]-1 && xt>=0.0 && xt<dt[0]-1 )
          ...
          {
            case 0:
              MRIsetVoxVal(weights,x,y,z,0,1-(histo[ivs][ivt]-min)*scale);
              break;
            case 1:
              MRIsetVoxVal(weights,x,y,z,0,1-(histo[ivs+1][ivt]-min)*scale);
              break;
            case 2:
              MRIsetVoxVal(weights,x,y,z,0,1-(histo[ivs][ivt+1]-min)*scale);
              break;
            case 3:
              MRIsetVoxVal(weights,x,y,z,0,1-(histo[ivs+1][ivt+1]-min)*scale);
              break;
...
//   bool   ssamp = (d1>1 || d2 > 1 || d3 >1);
  ...
  double xt,yt,zt;
	...
	int    ivs, ivt;
  double sdiff, tdiff;
  double dt[3] = { mriT->coeff->width, mriT->coeff->height, mriT->coeff->depth};
 // double ds[3] = { mriS->width, mriS->height, mriS->depth};
