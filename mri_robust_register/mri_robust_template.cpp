 ...
 * @brief Creation of robust template of several volumes toghether with their linear registration
 *
 */
...
static struct Parameters P = { vector < string >(0),"",NULL,vector < string >(0),vector < string >(0),false,false,false,false,false,false,false,5,0.01,SAT,vector < string >(0),0,1,vector < MRI* >(0),vector < LTA* >(0),vector < MRI* >(0),vector < MRI* >(0),vector < double >(0)};


static void printUsage(void);
static bool parseCommandLine(int argc, char *argv[],Parameters & P) ;
static MRI* averageSet(const vector < MRI * >& set, MRI* mean, int method, double sat);
static MRI* initialAverageSet(const vector < MRI * >& set, MRI* mean, int method, double sat);
...
static char vcid[] = "$Id: mri_robust_template.cpp,v 1.1 2009/05/04 22:24:57 mreuter Exp $";
char *Progname = NULL;
  ...
  for (int i = 0;i<nin;i++)
  {
        ...
        if (P.lta_vox2vox)
	{
     ...
     if (P.iscale && P.intensities[i] >0 && P.nwarps.size() > 0)
     {
	    ...
	    cerr << " output warp :  " << P.nwarps[i] << " should end with .mgz!" << endl;
	    exit (1);
	 }
...
//  vector < MRI * > warped(P.mri_mov.size(),NULL);
  ...
  while(itcount < itmax && maxchange > eps)
  {
     ...
     for (int i = 0;i<nin;i++)
     {
        ...
        Rv[i].setTarget(P.mri_mean,P.fixvoxel,P.fixtype); // gaussian pyramid will be constructed for each Rv[i], could be optimized
	ostringstream oss;
	...
	{
	   case 1: maxres = 2; break;
	   case 2: maxres = 2; break;
	   case 3: maxres = 1; break;
	   case 4: maxres = 1; break;
	}

	Md = Rv[i].computeMultiresRegistration(maxres,iterate,epsit,NULL,NULL,transforms[i],P.intensities[i]);
	...
	P.ltas[i] = LTAalloc(1,P.mri_mov[i]); 
        P.ltas[i]->xforms[0].m_L = MRIvoxelXformToRasXform (P.mri_mov[i], P.mri_mean, Md.first, P.ltas[i]->xforms[0].m_L) ;
	...
	if (lastlta)
	{
	   dists[i] = sqrt(Rv[i].AffineTransDistSq(lastlta->xforms[0].m_L, P.ltas[i]->xforms[0].m_L));
	   LTAfree(&lastlta);
	   ...
	   cout << endl << "tp " << i+1 << " distance: " << dists[i] << endl;
        ...
        if (Rv[i].isIscale() && Md.second > 0)
        {
            ...
            P.mri_mov[i] = Rv[i].MRIvalscale(P.mri_mov[i], P.mri_mov[i], Md.second);	 
        }
	...
	if (P.debug)
	{
	   ...
	   P.mri_weights[i] = Rv[i].getWeights(); // in original half way space
	     ...
	     MatrixPrint(stdout,hinv) ;
  	     ...
  	     MRI * wtarg = MRIalloc(P.mri_weights[i]->width,P.mri_weights[i]->height,P.mri_weights[i]->depth,MRI_FLOAT);
...
//   oss << outdir << "tp" << i+1 << "_to_mean-it" << itcount;
   R.setName(P.mean);
	...
	// (use geometry of destination space for half-way)
         m2hwlta->xforms[0].m_L = MRIvoxelXformToRasXform (P.mri_mov[0], P.mri_mov[1], maps2weights.first, m2hwlta->xforms[0].m_L) ;
         m2hwlta->type = LINEAR_RAS_TO_RAS ;
         d2hwlta->xforms[0].m_L = MRIvoxelXformToRasXform (P.mri_mov[1], P.mri_mov[1], maps2weights.second, d2hwlta->xforms[0].m_L) ;
...
//     }
   ...
   {	
       ...
       for (x = 0 ; x < set[0]->width ; x++)
       {
 	   ...
 	   MRIsetVoxVal(mean,x,y,z,0,RobustGaussian::median(dd,(int)set.size()));
  ...
  //MatrixPrintFmt(stdout,"% 2.8f",b);
    ...
    } else if (method ==3) // needs more development (sigma..)
    {
	 ...
	 int pcount = 0;
	  ...
	  // cout << " x: " << x << " y: " << y << " z: " <<z << "  size: " << set.size() <<endl;
 	      //cout << "i: " << i << endl;
	      b->rptr[pcount*set.size()+i+1][1] = MRIgetVoxVal(set[i],x,y,z,0);
...
static MRI* initialAverageSet(const vector < MRI * >& set, MRI* mean, int method, double sat)
{
// the initial average can be anyplace as long as it is not identical to one of the
  ...
  if (!strcmp(option, "MOV") )
  {
	...
	if (option[0] != '-')
	{
	   nargs++;
           ...
           cout << "Using "<< P.mov.back() << " as movable/source volume." << endl;
	}
...
//  }
  ...
  else if (!strcmp(option, "AVERAGE") )
  {
     ...
     nargs = 1;
     cout << "Using Method: " << P.average << " for template computation." <<  endl;
  }
...
//  if (P.mov.size() == 2) cout << " !!! Consider using a direct registration and mapping to halfway space with mri_robust_register !!! " << endl;
  ...
  {
      ntest = false;
      cerr << " No. of filnames for --warp should agree with no. of inputs!" << endl;
      exit(1);
  }
  if (P.nltas.size() > 0 && P.mov.size() != P.nltas.size())
  {
      ntest = false;
      cerr << " No. of filnames for --lta should agree with no. of inputs!" << endl;
      exit(1);
  }
  if (P.nweights.size() > 0 && P.mov.size() != P.nweights.size())
  {
     ntest = false;
      cerr << " No. of filnames for --weights should agree with no. of inputs!" << endl;
      exit(1);
  }
