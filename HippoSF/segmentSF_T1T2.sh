...
set HSFLOG = ($SUBJECTS_DIR/$SUBJECTNAME/scripts/hippocampal-subfields-T1T2.log)
rm -f $HSFLOG

# command
foreach SIDE ( left right )
  set cmd="$SCRIPTPATH/run_segmentSubjectT1T2_autoEstimateAlveusML.sh $RUNTIME $SUBJECTNAME $SUBJECTS_DIR $T2VOL $RESOLUTION $ATLASMESH $ATLASDUMP $LUT $K $SIDE $OPTIMIZER $SUFFIX $USERSUFFIX ${FREESURFER_HOME}/bin/ $MRFCONSTANT $BYPASSBF $USEWHOLEBRAININHP"
