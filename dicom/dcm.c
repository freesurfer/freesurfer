...
** Last Update:		$Author: inverse $, $Date: 2001/06/26 14:59:53 $
** Source File:		$RCSfile: dcm.c,v $
** Revision:		$Revision: 1.2 $
** Status:		$State: Exp $
*/

static char rcsid[] = "$Revision: 1.2 $ $RCSfile: dcm.c,v $";
	     ...
	     PRIVATE_OBJECT ** object, U32 * scannedLength)
{
    ...
    if (*ptr == NULL) {
		if (fd != -1) {
			...
			if (strncmp(label, "DICM", 4) != 0) {
				...
				labelsize=0;
				pos=_tell(fd);
				_lseek(fd, pos-sizeof(label), SEEK_SET);
			}
				else
		...
		if (nBytes != DCM_PREAMBLELENGTH + labelsize)
			return COND_PushCondition(DCM_FILEACCESSERROR,
			DCM_Message(DCM_FILEACCESSERROR), name,
			"readPreamble");
    } else {
    ...
    if (knownLength)
		*size -= DCM_PREAMBLELENGTH + labelsize;
    *fileOffset += (off_t) DCM_PREAMBLELENGTH + labelsize;
    if (*ptr != NULL)
		*ptr += DCM_PREAMBLELENGTH + labelsize;
    (*object)->objectSize += DCM_PREAMBLELENGTH + labelsize;
