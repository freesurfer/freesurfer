...
int main(int argc, char *argv[]) {
  int nargs, ng, ig, i, nx, ny, nz, nf, navg, ix, iy, iz, id;
  float smax, ssum, norm, mean;
  float *grads, *gp, *eval;
  MATRIX *B, *Bpseudo, *dwi, *tensor, *Tensor, *Evec;
  MRI *invol, *mask, *lowb, *avgdwi,
  *tenstack, *eigval, *eigvec1, *eigvec2, *eigvec3, *trace, *fa;
  FILE *fp;
  char outfile[1024];
    ...
    for (iy=0; iy<ny; iy++)
      for (ix=0; ix<nx; ix++)
        if ( MRIgetVoxVal(mask, ix, iy, iz, 0) ) {
          ...
          MRIsetVoxVal(eigval, ix, iy, iz, 1, eval[1]);
          MRIsetVoxVal(eigval, ix, iy, iz, 2, eval[2]);
          MRIsetVoxVal(eigvec1, ix, iy, iz, 0, Evec->rptr[1][1]);
          MRIsetVoxVal(eigvec1, ix, iy, iz, 1, Evec->rptr[2][1]);
          MRIsetVoxVal(eigvec1, ix, iy, iz, 2, Evec->rptr[3][1]);
          MRIsetVoxVal(eigvec2, ix, iy, iz, 0, Evec->rptr[1][2]);
          MRIsetVoxVal(eigvec2, ix, iy, iz, 1, Evec->rptr[2][2]);
          MRIsetVoxVal(eigvec2, ix, iy, iz, 2, Evec->rptr[3][2]);
          MRIsetVoxVal(eigvec3, ix, iy, iz, 0, Evec->rptr[1][3]);
          MRIsetVoxVal(eigvec3, ix, iy, iz, 1, Evec->rptr[2][3]);
          MRIsetVoxVal(eigvec3, ix, iy, iz, 2, Evec->rptr[3][3]);
