...
// Revision Date  : $Date: 2006/09/25 16:58:08 $
...
//static char vcid[] = "$Id: mri_cc_medial_axis.c,v 1.2 2006/09/25 16:58:08 pengyu Exp $";
...
rotatevolume()
{
  ...
  mri_cc_tal = MRIcopy(mri_tal, NULL) ;  
	...
	if(!NO_CORRECTION)
	{
		...
		y_center = cc_tal_y ; z_center = cc_tal_z ;  
		...
		for ( x = -xshift;   x <= xshift  ; x++)  /* shift centoid point in x direction */ 
			for ( y_alpha = -1*y_rotate; y_alpha <= y_rotate; y_alpha++)
				...
				{	
					...
					if ( counter < number ) { number = counter; p_x = x_center; p_y = y_alpha; p_z = z_alpha; }  
				}
...
static MEDATOM *compute_normal(MEDATOM *medial_axis, int length)
{
	...
	for (i=1; i < length-1; i++)
	{
		...
		do
		{
			...
			if (dist)
			{
				dx = medial_axis[i].dy/dist;	dy = -1*medial_axis[i].dx/dist;
				n++;
				sx+=dx; sy+=dy;
			}
...
{	float val1, val2;
 ...
 if (0)
 {
	...
	if((fp = fopen(ofname, "w")) == NULL)
  {
	...
	{
		Real upmid, downmid; float radius;
		dist = n*space;
