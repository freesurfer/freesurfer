...
% w = .009;
T0 = -10;
k1 = totalMinVar{find(strcmp(varNames, 'k1'))};    % rate at which carbs are metabolized from stomach to blood (grams/min)
SI = totalMinVar{find(strcmp(varNames, 'SI'))};    % insulin sensitivity (fit from medronic IOB table- amount BG is lowered by insulin)
SC = totalMinVar{find(strcmp(varNames, 'SC'))};    % Carb sensitivity
bTime = totalMinVar{find(strcmp(varNames, 'bTime'))};
k2 = SI * (totalMinVar{find(strcmp(varNames, 'b1'))}*(t<=bTime-T0) + totalMinVar{find(strcmp(varNames, 'b2'))}*(t>bTime-T0)); %1e-6;    % rate at which liver drips glucose into blood plasma (glucose/min)

k4 = totalMinVar{find(strcmp(varNames, 'k45'))}; %sqrt(a)*w;   % rate at which insulin moves from plasma to cells (fit from medronic IOB table, unit/min)
k5 = k4; %(a*w^2)/k4;   % rate at which insulin moves from fluid to plasma (units/min)
k3 = totalMinVar{find(strcmp(varNames, 'k3'))}; %(a+1)*w - k4 - k5;   % rate at which insulin moves from plasma to interstitial fluid (units/min)
k6 = .05;    % relative rate at which muscle glucose is refilled (1/min)
G_target = 125;   % desired blood glucose (Gp) level
low_limit = 70;
high_limit = 180;

carb_delay = totalMinVar{find(strcmp(varNames, 'cTime'))} - T0;
carb_grams = 50.1;
allG0 = 70:5:180;
clear dispStr
%% Define operators
%totalInsulin = (sum(k2) + sum(carb_grams)*SC + G0-G_target)/SI;
...
for index = 1:length(allG0)
    ...
    K = SI*k4*OP_int;
    l = G0*allOnes + OP_int*k2; % + exp(-k6*t).*(OP_int*(exe.*exp(k6*t))) - OP_int*exe;
...
%     beq1 = [1; 1; 1]*k2/(k4*SI);
    ...
    dispStr{index} = ['For initial BG of ' num2str(G0) ' mg/dL, bolus ' num2str(bolus) ' units at t = ' num2str(t(ind)+T0) ' minutes.'];
    disp(dispStr{index})
end
disp(' ')
for index = 1:length(allG0)
