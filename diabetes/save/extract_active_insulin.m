...
for t=1:ntps
    ...
    basal = 0 ;
    if (t > 1)
        for t1=time_series(t-1):time_series(t)
	  dt = round(time_series(t)-t1+1) ;
	  if (dt <= length(iob_table))
	     basal = basal + ((dt-1)/60)*basal_vec(t-1)*iob_table(dt) ;
	  end
	end
    end
    insulin = basal + bolus_vec(t) ;
    if (insulin > 0)
       for bolus_ind = 1:num_active_boluses
