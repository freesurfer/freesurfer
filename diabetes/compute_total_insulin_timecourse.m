function [insulin_active, insulin_absorbed] = compute_total_insulin_timecourse(insulin_schedule, time,K_fluid_to_blood, K_blood_to_fluid, K_blood_to_cells, dt,bolus_time,bolus_amount)
...
for t=time(1):dt:time(end)
    if (t < bolus_time & ((t+dt) >= bolus_time))
      insulin_fluid = insulin_fluid +  bolus_amount ;
    end
