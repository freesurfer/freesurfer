...
switch optimCF
    ...
    case 2
        ...
        A = [A1; A2; A3];
        Aeq = [Aeq1; Aeq2; Aeq3];
        Ip_absIp = linprog([0*allOnes; allOnes], [A zeros(size(A,1),T2); K, -eye(T2); -K, -eye(T2)], [b1; b2; b3; l-parms.G_target*allOnes; -l+parms.G_target*allOnes], [Aeq zeros(size(Aeq,1),T2)], [beq1; beq2; beq3], [Ip_min; 0*allOnes], [], [], optimoptions('linprog', 'Algorithm', 'interior-point','maxiter',1000));
        Ip = Ip_absIp(1:T2);
    case 3
