...
rms_table = zeros(length(C_range), length(SI_range)) ;
pool = parpool(20) ;

for b1=.65*dt:.05*dt:1.1*dt
  for b2=.5*dt:.05*dt:1*dt
...
%       carbs_metabolized = predict_carb_uptake(carbs, .0, k_stomach_to_blood, time) ;
        ...
        parfor CRi=1:length(C_range)
          C = C_range(CRi) ;
          for SIi=1:length(SI_range)
	    SI = SI_range(SIi) ;
	    SC = SI / C ;
