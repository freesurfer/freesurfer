pct_change_thresh = .005 ;
pct_change_thresh = .15 ;
delay_index = 1 ;
...
parms.carb_grams = 50 ;
disp(sprintf('computing schedule for delay = %2.0f', delay)) ;
parms.insulin_delay = delay+1 ;
...
parms.carb_delay = 15+parms.insulin_delay ;  % 15 min after insulin
baseline = simulate_timecourse(parms) ;
for delay=0:5:60
if (delay == 5)
   keyboard
...
for iter=1:20000
    ...
    for t1=1:endt
	    ...
	    rms = compute_BG_rms(parms.G_target, Gp_t) ;
            del_rms = start_rms - rms ;
            if (rms < best_rms & (parms.insulin(t1) > 0) & (min(Gp_t) > parms.low_limit))
                best_t1 = t1 ;
                best_t2 = t2 ;
                best_rms = rms ;
            elseif (start_rms + del_rms < best_rms & (parms.insulin(t2)>0) & (min(Gp_t_r) > parms.low_limit))  % transfer in other direction is best
                best_t1 = t2 ;
                best_t2 = t1 ;
    ...
    if (best_t1 > 0)
       delta = min(parms.insulin(best_t1), del_insulin) ;
       parms.insulin(best_t1) = parms.insulin(best_t1) - delta ;
       parms.insulin(best_t2) = parms.insulin(best_t2) + delta ;
    ...
    if (mod(iter,10) ==0)
        ...
        prev_rms = end_rms ;
	if (pct_change < pct_change_thresh)
       	   disp(sprintf('terminating integration due to pct change %2.3f < thresh %2.3f', pct_change, pct_change_thresh)) ;
       	   break ;
    	end
    end
    if (best_t1 <= 0)
       disp('terminating integration due to lack of progress') ;
       break ;
    end
end

figure('name', sprintf('delay = %2.1f, mean BG = %2.1f (min = %2.1f, max = %2.1f)', delay,mean(outputs.Gp_t),min(outputs.Gp_t),max(outputs.Gp_t))) ;
...
delay_timecourse(delay_index, :) = outputs.Gp_t ;
