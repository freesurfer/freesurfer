...
Gp = 2*G_target ;                  % glucose in bloodparms.k6

Ip = steady_state_basal_in_plasma ;
If = steady_state_basal_in_fluid + (Gp-G_target)/insulin_sensitivity ;
...
disp(sprintf('mean BG = %2.1f (min = %2.1f, max = %2.1f)', mean(Gp_t),min(Gp_t),max(Gp_t))) ;
