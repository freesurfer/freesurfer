...
BG_t = zeros(true_parms.max_BG, parms.T1) ;

nsamples = true_parms.nsamples ;
for n=1:nsamples
    parms = true_parms ;
    parms.basal = parms.basal*(1+hyper_parms.basal_std*randn) ;
    parms.insulin_sensitivity = parms.insulin_sensitivity * (1+hyper_parms.insulin_sensitivity_std*randn);	
    parms.carb_sensitivity = parms.carb_sensitivity * (1+hyper_parms.carb_sensitivity_std*randn);	
    parms.carb_delay(1) = parms.carb_delay(1) * (1+hyper_parms.carb_delay_std*randn);	
    parms.carb_grams(1) = parms.carb_grams(1) * (1+hyper_parms.carb_grams_std*randn);	
    parms.G0 = parms.G0 * (1+hyper_parms.G0_std*randn);	
    carb_ratio = parms.insulin_sensitivity/parms.carb_sensitivity ;
    
    timecourse = simulate_timecourse(parms) ;
    for t=1:length(timecourse.Gp_t)
