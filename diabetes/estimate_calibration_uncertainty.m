...
for n=1:nsamples
    ...
    parms = true_parms ;
    parms.basal = parms.basal*(1+parameter_error*rand) ;
    parms.insulin_sensitivity = parms.insulin_sensitivity * (1+parameter_error*rand);	
    parms.carb_sensitivity = parms.carb_sensitivity * (1+parameter_error*rand);	
    carb_ratio = parms.insulin_sensitivity/parms.carb_sensitivity ;
    ...
    measured_timecourse.Gp_t = true_timecourse.Gp_t + randn(1,size(true_timecourse.Gp_t,2)).*sigma.*true_timecourse.Gp_t;
%    measured_timecourse.Gp_t = true_timecourse.Gp_t ;
	...
	if (t0 == 1)
		...
		F(i) = parms.carb_grams ;
	        ...
	        A(i,1) = measured_timecourse.Ip_t(t0);
		F(i) = 0 ;
        end
        A(i,2) = measured_timecourse.Ip_t(t) ;
        M(i,1) = measured_timecourse.Gp_t(t0) ;	
...
if 0
    ...
    ebest = calibrate_basal(A,M,180,Ibest,dt, 1);
end
    disp(sprintf('basal error = %2.3f%%', 100*(true_parms.basal - (parms.basal-ebest))/true_parms.basal));
    ...
    nS(n) = Sbest ; nI(n) = Ibest ; ne(n) = parms.basal-ebest ; nC(n) = Cbest ;
    ncr(n,:) = noisy_carb_ratio(n) ;
    nt(n,:) = noisy_timecourse(n).Gp_t ;
end

% convert to percentages
carb_ratio = true_parms.insulin_sensitivity/true_parms.carb_sensitivity ;
pC = (nC - carb_ratio) / carb_ratio ;
pI = (nI - true_parms.carb_sensitivity) ./true_parms.carb_sensitivity ;
pS = (nS - true_parms.insulin_sensitivity) ./true_parms.insulin_sensitivity ;
pbasal = (ne - true_parms.basal) ./ true_parms.basal ;
