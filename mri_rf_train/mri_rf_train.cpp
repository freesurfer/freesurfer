...
static RANDOM_FOREST *train_rforest(MRI **mri_inputs, MRI **mri_segs, TRANSFORM **transforms, 
				    int nsubjects, GCA *gca, RFA_PARMS *parms, float wm_thresh,
				    int wmsa_whalf) ;
static RANDOM_FOREST *train_rforest_with_wmsa_nbrs(MRI **mri_inputs, MRI **mri_segs, TRANSFORM **transforms, 
						   int nsubjects, GCA *gca, RFA_PARMS *parms) ;
...
//        GCAreinit(mri_in, gca);
	...
	(mri_in, NULL, TRs, TEs, FAs, parms.nvols) ;
      MRIfree(&mri_in) ;
      mri_in = mri_tmp ;
    }
  ...
  {
    if (only_nbrs)
      rf = train_rforest_with_wmsa_nbrs(mri_inputs, mri_segs, transforms, nsubjects, gca, &parms) ;
    else
      rf = train_rforest(mri_inputs, mri_segs, transforms, nsubjects, gca, &parms, wm_thresh,wmsa_whalf) ;
	      ...
	      GCA *gca, RFA_PARMS *parms, float wm_thresh, int wmsa_whalf)
{
  ...
  {
    mri_in = mri_inputs[n] ; transform = transforms[n] ; mri_seg = mri_segs[n] ;
			     ...
			     GCA *gca, RFA_PARMS *parms)
{
  ...
  if (ignore_indices == NULL)
    ErrorExit(ERROR_NOMEMORY, "train_rforest: could not allocate %d length index vector", ntraining) ;
