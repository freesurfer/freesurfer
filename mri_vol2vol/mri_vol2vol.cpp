  ...
  while (nargc > 0) {
	...
	nargsused++;
	 ...
	 pargv[0][0] != '+' && pargv[0][0] != '.'){
      ...
      int LRRev=0;
      if(nargc < 6){
	printf("  --spm-warp mov movlta warp lrrev interp out\n");
	argnerr(option,6);
      }
      mov = MRIread(pargv[0]);
      if(mov == NULL) exit(1);
	...
	if(srclta == NULL) exit(1);
      ...
      if(warp == NULL) exit(1);
      sscanf(pargv[3],"%d",&LRRev);
      sscanf(pargv[4],"%d",&interpcode);
      printf("Running MRIapplySpmVbmWarp() interp = %d, lrrev=%d\n",interpcode,LRRev);
      out = MRIapplySpmWarp(mov, srclta, warp, LRRev, interpcode, NULL);
      if(out == NULL) exit(1);
      printf("Writing to %s\n",pargv[5]);
      err = MRIwrite(out,pargv[5]);
      if(err) exit(1);
      MRIfree(&warp);
...
printf("  --spm-warp mov movlta warp lrrev interp output\n");
...
printf("     lrrev=0,1 indicates if the warp was generated by left-right reversing anat\n");
