...
PetscSolver::SetupIndexCorrespondence()
{
  ...
  MaskConstIteratorType maskCiter( mask,
				   mask->GetRequestedRegion() );
  IndexIterator indexIter( indexImage,
...
{
  typedef itk::ConstNeighborhoodIterator<MaskImageType>
    MaskCNIterator;
  typedef itk::ConstNeighborhoodIterator<IndexImageType>
    IndexCNIterator;

  MaskCNIterator::RadiusType radius;
  radius.Fill(1);

  MaskCNIterator mask_cni( radius,
			   mask,
			   mask->GetRequestedRegion()
			   );
  IndexCNIterator index_cni( radius,
			     indexImage,
			     indexImage->GetRequestedRegion()
			     );
  ConstNeighborhoodIterator data_cni( radius,
				      data,
				      data->GetRequestedRegion()
				      );
