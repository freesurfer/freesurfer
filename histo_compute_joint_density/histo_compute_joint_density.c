...
main(int argc, char *argv[])
{
  ...
  if (argc < 4)
	...
	if (valid1_min >= 0)
	{
		...
		MRIfree(&mri1) ; mri1 = mri_tmp ;
		mri_tmp = MRIsegmentToImage(mri2, NULL, mriseg, 0) ;
		MRIfree(&mri2) ; mri2 = mri_tmp ;
	...
	for (level = 1 ; level < MAX_LEVELS ; level++)
	{
		...
		sprintf(fname, "h%d.mgz", level) ; MRIwrite(mri1_pyramid[level], fname) ; 
		sprintf(fname, "b%d.mgz", level) ; MRIwrite(mri2_pyramid[level], fname) ;
	}
