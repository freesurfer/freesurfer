...
int main(int argc, char *argv[])
{
  ...
  printf("USE_LOCAL_MINC is defined, hence can't compare test_convert_transform_to_starts_and_steps with minc, but deleter=%p\n",deleter);
#else
  ...
  if (status != OK)
  {
    printf("input_transform_file failed\n");
    return 1;
  }
