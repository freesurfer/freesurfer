 ...
 * @brief program to fuse a group of cross sectional segmentations into an initial estimate
 * of a longitudinal one
 *
...
static MRI *MRIfuseSegmentations(MRI *mri_in, MRI *mri_fused, int nvols, LTA *xforms[], MRI *mri_asegs[], MRI *mri_norms[], double std) ;
...
main(int argc, char *argv[]) {
  ...
  struct timeb start ;
  MRI         *mri_in, *mri_fused, *mri_norms[MAX_VOLUMES], *mri_asegs[MAX_VOLUMES] ;
  double      std = 3.0 ;
  ...
  for (i = 0 ; i < ntimepoints ; i++)
  {
    ...
    sprintf(fname, "%s/%s/mri/transforms/%s_to_%s.lta", sdir, subject, argv[i+2], subject) ;
    xforms[i] = LTAread(fname) ;
    if (xforms[i] == NULL)
      ErrorExit(ERROR_NOFILE, "%s: could not read input transform %s", Progname, fname) ;
  }

  mri_fused = MRIfuseSegmentations(mri_in, NULL, ntimepoints, xforms, mri_asegs, mri_norms, std) ;
  ...
  minutes = seconds / 60 ;
  seconds = seconds % 60 ;
  fprintf(stderr, "fusing took %d minutes and %d seconds.\n", minutes, seconds) ;
  exit(0) ;
  return(0) ;
}
/*----------------------------------------------------------------------
...
static void
usage_exit(int code) {
  printf("usage: %s [options] <subject to segment> <subject 1> <subjet 2> <...> <output volume>",
         Progname) ;
  printf("\ta - name of aseg file to use (default=aseg.mgz)") ;
  printf("\tn - name of norm file to use (default=norm.mgz)") ;
                     ...
                     *mri_norms[], double sigma)
{
  ...
  for (i = 0 ; i < nvols ; i++)
  {
    ...
    case LINEAR_RAS_TO_RAS:
      m = MRIrasXformToVoxelXform(mri_norms[i], mri_in, xforms[i]->xforms[0].m_L, NULL);
      break ;
    case LINEAR_VOX_TO_VOX:
      m = MatrixCopy(xforms[i]->xforms[0].m_L, NULL);
      break ;
    default:
      ErrorExit(ERROR_UNSUPPORTED, "unsupported transform type %d\n", xforms[i]->type) ;
      break ;
    }
    ...
    for (y = 0 ; y < mri_fused->height ; y++)
    {
      ...
      for (z = 0 ; z < mri_fused->depth ; z++)
      {
        ...
        for (i = 0 ; i < nvols ; i++)
        {
          ...
          if (xd < 0 || yd < 0 || zd < 0 || xd > mri_asegs[i]->width-1
              || yd > mri_asegs[i]->height-1  || zd > mri_asegs[i]->depth-1)
