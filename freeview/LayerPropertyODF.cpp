...
LayerPropertyODF::LayerPropertyODF(QObject *parent) : LayerPropertyMRI(parent) {
  m_dOdfScale     = 1;
  m_nOdfSkip      = 0;
  m_nOdfInversion = 0;
...
vtkColorTransferFunction *LayerPropertyODF::GetOdfLut() { return m_odfLut; }
...
void LayerPropertyODF::UpdateOdfLut() {
  ...
  foreach (double val, keys) {
    ...
    m_odfLut->AddRGBPoint(th[0] + (th[1] - th[0]) * val, c.redF(), c.greenF(),
                          c.blueF());
  }
  m_odfLut->Build();
}
