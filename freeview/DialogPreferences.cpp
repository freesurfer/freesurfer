...
//  connect(ui->checkBoxMacUnified, SIGNAL(toggled(bool)), mainwnd, SLOT(SetUnifiedTitleAndToolBar(bool)));
  ...
  connect(ui->sliderOutlineThickness, SIGNAL(valueChanged(int)), mainwnd, SLOT(UpdateSettings()));
...
void DialogPreferences::SetSettings(const QVariantMap &map)
{
  ...
  ui->sliderOutlineThickness->setValue(map["OutlineThickness"].toInt());
...
QVariantMap DialogPreferences::GetSettings()
{
  ...
  map["OutlineThickness"] = ui->sliderOutlineThickness->value();
  return map;
}
