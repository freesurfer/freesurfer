...
LayerODF::LayerODF(LayerMRI *ref, QObject *parent)
  ...
  connect(mProperty, SIGNAL(OdfPropertyChanged()), this,
          SIGNAL(UpdateActorRequested()));
  connect(this, SIGNAL(UpdateActorRequested(int)), SLOT(UpdateActors(int)),
          Qt::QueuedConnection);
  connect(mProperty, SIGNAL(ColorCodeChanged()), SLOT(OnColorCodeChanged()),
          Qt::QueuedConnection);
...
bool LayerODF::Load(const QString &fn, const QString &vertex_fn,
                    const QString &face_fn, bool bPermute) {
  ...
  if (m_bDtkFormat && bPermute) {
    MRI *mri2    = ::MRIallocSequence(mri->height, mri->depth, mri->nframes,
                                   mri->type, mri->width);
...
void LayerODF::BuildSlice(int nPlane) {
      ...
      for (z = slice_range[2][0]; z <= slice_range[2][1]; z += nskip) {
        if (maskdata) {
          double val = MyVTKUtils::GetImageDataComponent(
              mask_ptr, dim, 1, x, y, z, 0, mask_scalar_type);
  ...
  polydata->GetPointData()->SetActiveScalars(qPrintable(
      GetProperty()->GetOdfColorCode() == 0 ? "Directional" : "Magnitude"));
...
void LayerODF::OnColorCodeChanged() {
  ...
  for (int i = 0; i < 3; i++) {
    vtkSmartPointer<vtkPolyDataMapper> mapper =
        vtkPolyDataMapper::SafeDownCast(m_glyphActor2D[i]->GetMapper());
    if (m_polydata[i].GetPointer()) {
      m_polydata[i]->GetPointData()->SetActiveScalars(
          qPrintable(nCode == 0 ? "Directional" : "Magnitude"));
