...
LUTDataHolder::LUTDataHolder()
{
  ...
  if (fi.exists())
  {
    ...
    ctd.table = CTABreadASCII( ctd.filename.toUtf8().data() );
  ...
  if ( m_tables.isEmpty() )
  {
    ...
    ctd.table = CTABreadASCII( tempfn.toUtf8().constData() );
    ctd.name = "FreeSurferColorLUT";
    if ( ctd.table )
    {
      m_tables.push_back( ctd );
...
COLOR_TABLE* LUTDataHolder::LoadColorTable( const QString& filename )
{
  ...
  if ( ct )
  {
    ...
    ct = CTABreadASCII( m_tables[nId].filename.toUtf8().data() );
    if ( ct )
    {
  ...
  {
    ct = CTABreadASCII2( filename_full.toUtf8().data(), 0);  // do not check duplicate names because it could take a long time
    if ( ct )
    {
      ColorTableData ctd;
      ctd.table = ct;
