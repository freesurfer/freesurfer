...
PanelODF::PanelODF(QWidget *parent)
  ...
  connect(ui->comboBoxMask, SIGNAL(currentIndexChanged(int)), this,
          SLOT(OnComboMask(int)), Qt::QueuedConnection);
  connect(ui->lineEditScale, SIGNAL(textChanged(QString)),
          SLOT(OnLineEditScale(QString)), Qt::QueuedConnection);
  connect(ui->lineEditMaskThreshold, SIGNAL(returnPressed()),
          SLOT(OnLineEditMaskThresholdChanged()), Qt::QueuedConnection);
}
...
void PanelODF::ConnectLayer(Layer *layer_in) {
  ...
  connect(p, SIGNAL(OdfPropertyChanged()), this, SLOT(UpdateWidgets()),
          Qt::QueuedConnection);
  connect(p, SIGNAL(ColorCodeChanged()), this, SLOT(UpdateWidgets()),
          Qt::QueuedConnection);
  connect(ui->comboBoxInversion, SIGNAL(currentIndexChanged(int)), p,
          SLOT(SetOdfInversion(int)));
  connect(ui->spinBoxSkip, SIGNAL(valueChanged(int)), p, SLOT(SetOdfSkip(int)),
          Qt::QueuedConnection);
  connect(ui->comboBoxColorCode, SIGNAL(currentIndexChanged(int)), p,
          SLOT(SetOdfColorCode(int)));
}
...
void PanelODF::DoUpdateWidgets() {
  ...
  if (layer) {
    ...
    ChangeLineEditNumber(ui->lineEditScale,
                         layer->GetProperty()->GetOdfScale());
    ui->spinBoxSkip->setValue(layer->GetProperty()->GetOdfSkip());
    ui->comboBoxInversion->setCurrentIndex(
        layer->GetProperty()->GetOdfInversion());
    ui->comboBoxColorCode->setCurrentIndex(
        layer->GetProperty()->GetOdfColorCode());
    for (int i = 0; i < m_colorThresholdWidgets.size(); i++)
      m_colorThresholdWidgets[i]->setVisible(
          layer->GetProperty()->GetOdfColorCode() == 1);
    ...
    ui->sliderColorMin->setValue(
        (int)((th[0] - range_min) / (range_max - range_min) * 100));
    ui->sliderColorMax->setValue(
        (int)((th[1] - range_min) / (range_max - range_min) * 100));
  }
...
void PanelODF::OnLineEditColorThreshold(const QString &strg) {
  ...
  if (layer) {
    ...
    ui->sliderColorMin->setValue(
        (int)((th[n] - range_min) / (range_max - range_min) * 100));
...
void PanelODF::OnSliderColorThreshold(int nVal) {
  ...
  if (layer) {
    ...
    layer->GetProperty()->SetMagnitudeThreshold(th);
    ChangeLineEditNumber(n == 0 ? ui->lineEditColorMin : ui->lineEditColorMax,
                         th[n]);
  }
}
