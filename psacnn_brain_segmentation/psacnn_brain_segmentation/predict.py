...
def predict_segmentation(input_file, output_dir, contrast = 't1w',
                         model_file=None, output_soft=False,
                         patch_dim=96):
    ...
    curr_unet.predict_segmentation(test_files, test_channel_names, out_membership_file, out_hard_file,
                                   step_size=[16, 16, 16], batch_size=4, center_voxel=True, sampling_rate=20000,
...
class PSACNNInputSpec(BaseInterfaceInputSpec):
    ...
    save_prob_output = traits.Bool(False, desc='saves probability map. can be 1.5GB', argstr='-os', position=5,
                                   mandatory=False)
...
class PSACNN(BaseInterface):
   ...
   '
    ...
    def _run_interface(self, runtime):
        ...
        predict_segmentation(self.inputs.input_image, self.inputs.output_dir, contrast=self.inputs.contrast,
                             model_file=self.inputs.model_file, output_soft=self.inputs.output_soft,
...
if __name__ == "__main__":
    ...
    psacnn = Node(PSACNN(output_dir=output_dir, contrast='t2w', gpu=0, patch_size=96, save_prob_output=False), name='psacnn')
