...
//      MRIScomputeClassModes(mris, mri_T1, &white_mode, &gray_mode, NULL, &white_std, &gray_std, NULL);
    ...
    if (use_histos)
      MRIScomputeWhiteSurfaceClassBoundaries(mris, mri_T1, &max_border_white, &min_border_white, &max_gray, &min_gray_at_white_border, white_scale) ;
       ...
       n_averages /= 2, current_sigma /= 2, i++)
  {
    ...
    */
    ...
    if (write_vals)
    {
      ...
      printf("writing target values to %s\n", fname) ;
...
get_option(int argc, char *argv[])
{
  ...
  else if (!stricmp(option, "white_scale"))
  {
    white_scale = atof(argv[2]) ;
    nargs = 1 ;
    fprintf(stderr,  "using scale %2.1f (default=2) for finding histogram thresholds (bigger is more liberal)\n",white_scale);
  }
