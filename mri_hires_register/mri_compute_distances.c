...
static float MRIcomputeAverageHausdorffDistance(MRI *mri1, MRI *mri2, int label, double *psigma) ;
static float MRIcomputeSegmentPairHausdorffDistance(MRI *mri1, MRI *mri2, 
																										MRI_SEGMENT *mseg1, MRI_SEGMENT *mseg2) ;
...
static float
MRIcomputeAverageHausdorffDistance(MRI *mri1, MRI *mri2, int label, double *psigma)
{
	...
	if (mriseg1->nsegments == 0 || mriseg2->nsegments == 0)
		ErrorReturn(-1.0, (ERROR_BADPARM, 
											 "MRIcomputeAverageHausdorffDistance: invalid nseg %d or %d",
	...
	sigma = sqrt(dsq/mriseg1->nsegments - total_dist*total_dist) ; // standard dev
	if (psigma)
		*psigma = sigma / sqrt(mriseg1->nsegments) ;

	MRIsegmentFree(&mriseg1) ; MRIsegmentFree(&mriseg2) ;


	return(total_dist) ;
}
static float
MRIcomputeSegmentPairHausdorffDistance(MRI *mri1, MRI *mri2, MRI_SEGMENT *mseg1, MRI_SEGMENT *mseg2)
{
	...
	*MATRIX_RELT(vvox,4,1) = 1.0 ; *MATRIX_RELT(vras,4,1) = 1.0 ;
	...
	for (i1 = 0 ; i1 < mseg1->nvoxels ; i1++)
	{
		...
		x1 = V3_X(vras) ; y1 = V3_Y(vras) ; z1 = V3_Z(vras) ; 
		for (i2 = 0 ; i2 < mseg2->nvoxels ; i2++)
		{
			...
			x2 = V3_X(vras) ; y2 = V3_Y(vras) ; z2 = V3_Z(vras) ; 
...
static float
MRIcomputeSegmentPairCentroidDistance(MRI *mri1, MRI *mri2, MRI_SEGMENT *mseg1, MRI_SEGMENT *mseg2)
{
