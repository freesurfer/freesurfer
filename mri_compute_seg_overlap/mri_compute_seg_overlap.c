...
//static int num_labels  = 12;
//static int labels_of_interest[12] = {10, 11, 12, 13, 17, 18, 49, 50, 51, 52, 53, 54};

static int num_labels  = 24;

static int labels_of_interest[24] = {2, 41, 3, 42, 4, 43, 17, 53, 10, 49, 11, 50, 12, 51, 13, 52, 18, 54, 26, 58, 14, 15, 5, 44};

//static int num_labels  = 22;

//static int labels_of_interest[22] = {2, 41, 3, 42, 4, 43, 17, 53, 10, 49, 11, 50, 12, 51, 13, 52, 18, 54, 14, 15, 5, 44};
...
int main(int argc, char *argv[])
{
  ...
  nargs = handle_version_option (argc, argv, "$Id: mri_compute_seg_overlap.c,v 1.1 2006/02/02 19:53:40 xhan Exp $", "$Name:  $");
  argc -= nargs ;
  if (1 == argc)
    exit (0);
  ...
  if((mri_seg1->width != mri_seg2->width) ||
    ...
    ErrorExit(ERROR_BADPARM, "%s: two input label volumes have different sizes \n", Progname);
  ...
  for (f = 0 ; f < nframes ; f++){
	...
	for (x = 0 ; x < width ; x++){
	  ...
	  if(v1 == v2){
	    if((v1>=4 && v1 <= 5) || (v1 >= 10 && v1 <= 15) || (v1 >= 17 && v1 <= 18) || (v1>=43 && v1 <= 44) || (v1 >= 49 && v1 <= 54 ) )
	  ...
	  if((v2>=4 && v2 <= 5) || (v2 >= 10 && v2 <= 15) || (v2 >= 17 && v2 <= 18) || (v2>=43 && v2 <= 44) || (v2 >= 49 && v2 <= 54 ) )
	    subcorvolume2++;
  ...
  for(i=0; i < MAX_CLASSES; i++){
    correct_ratio[i] = (double)Volume_overlap[i]/((double)Volume_union[i] + 1e-10);
    correct_ratio2[i] = (double)Volume_overlap[i]*2.0/((double)Volume_from1[i] + (double)Volume_from2[i] + 1e-10);
    //correct_ratio2[i] = (double)Volume_overlap[i]/((double)Volume_from1[i] + 1e-10);
  }
  ...
  for(i=0; i < num_labels; i++){
    ...
    std1 += correct_ratio[labels_of_interest[i]]*correct_ratio[labels_of_interest[i]];
  }
  mean1 /= num_labels;
  std1 /= num_labels;
...
void usage(int exit_val)
{
  ...
  fprintf(fout, "this program compares two segmentation volumes and computes the DICE and Jaccard Coefficients. It considers only 12 major structures. \n") ;
