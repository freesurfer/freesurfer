...
MRI *MRIremoveFace(MRI *mri_src, MRI *mri_dst, LTA *lta, GCA *gca, GCA *gca_face, int radius,
									 int fill_val) ;
...
main(int argc, char *argv[])
{
  ...
  nargs = handle_version_option (argc, argv, "$Id: mri_deface.c,v 1.12 2004/02/03 15:44:35 tosa Exp $", "$Name:  $");
  argc -= nargs ;
  if (1 == argc)
...
#define DELTA_SCALE 0.01
  ...
  mri_out = MRIremoveFace(mri_in, NULL, parms.lta, gca, gca_face, radius, 255) ;
  {
    MRI *mri_tmp ;
...
#define MIN_SCALES 2
  ...
  do
  {
    ...
    /* search a finer nbhd (if do-while continues) */
		if ((spacing < 8) || (max_log_p < old_max-tol*old_max)) /* couldn't take a step */
		{
  ...
  for (i = 0 ; i < nreductions ; i++)
  {
    ...
    if (Gdiag & DIAG_SHOW)
    {
      printf("scanning %2.2f degree nbhd (%2.1f), "
             "scale %2.3f->%2.3f (%2.3f), and trans %2.2f->%2.2f (%2.2f)\n",
...
MRI *
MRIremoveFace(MRI *mri_src, MRI *mri_dst, LTA *lta, GCA *gca, GCA *gca_face, int radius, int fill_val)
{
