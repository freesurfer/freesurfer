...
#define DEFAULT_GCS  2
  ...
  if (!gcsa->cp_nodes)
    ErrorExit(ERROR_NOMEMORY,
              "GCSAalloc(%d, %d): could not allocate cp nodes",
...
int GCSAsourceToPriorVertexNo(GCSA *gcsa, VERTEX *v)
{
  int vdstno;
  float dmin;
  vdstno = MHTfindClosestVertexNo
    (gcsa->mht_priors, gcsa->mris_priors, v, &dmin) ;
  return(vdstno) ;
}
      ...
      if (n >= cp->nlabels[i])   /* not there - reallocate stuff */
        {
          ...
          /* allocate new ones */
          cp->label_priors[i] =
            (float *)calloc(cp->nlabels[i]+1, sizeof(float));
          if (!cp->label_priors[i])
          ...
          for (n = 0 ; n < cpn->nlabels ; n++)
            {
              label = cpn->labels[n] ;
              ll =
                gcsaNbhdGibbsLogLikelihood(gcsa, mris,
                                           v_inputs, vno, 1.0,label) ;
              if (vno == Gdiag_no)
          ...
          if (best_label != old_label)
            {
              if (vno == Gdiag_no)
                printf("v %d: label changed from %s (%d) to %s (%d)\n",
                       vno, annotation_to_name(old_label, NULL),
                       old_label, annotation_to_name(best_label, NULL),
                       best_label) ;
              v->marked = 1 ;
      ...
      for (vno = 0 ; vno < gcsa->mris_priors->nvertices ; vno++)
        {
          ...
          *cpn = *cpn_nbr ;
          ...
          if (!cpn->cps)
            ErrorExit(ERROR_NOMEMORY,
                      "fill_cpn_holes(): could not allocate cps %d",
                      vno) ;
          cpn->labels = (int *)calloc(cpn->nlabels, sizeof(int)) ;
          if (!cpn->labels)
            ErrorExit(ERROR_NOMEMORY,
                  ...
                  if (cp->nlabels[i])
                    {
                      ...
                      if (!cp->label_priors[i])
                        ErrorExit(ERROR_NOMEMORY, "fill_cpn_holes: "
                                  ...
                                  cp->nlabels[i]) ;
                      cp->labels[i] =
                        (int *)calloc(cp->nlabels[i]+1, sizeof(int)) ;
                      if (!cp->labels[i])
                        ErrorExit(ERROR_NOMEMORY,
              ...
              if (!gcs) /* couldn't find one for this label - fix */
                {
                  ...
                  vno_nbr_classifier =
                    vn_classifier-gcsa->mris_classifiers->vertices;
                  ...
                  if (!gcs)
                    ErrorPrintf(ERROR_BADPARM,
                                "fill_cpn_holes: could not find cp!\n");
                  else
  ...
  if (ndst >= gcsan_dst->nlabels)   /* reallocate stuff */
    {
      ...
      if (old_labels)
        {
          ...
          memmove(gcsan_dst->gcs,
                  old_gcs,
                  (gcsan_dst->nlabels-1)*sizeof(GCS)) ;
          free(old_gcs) ;
        }
      ...
      gcsan_dst->gcs[ndst].m_cov =
        MatrixCopy(gcsan_src->gcs[nsrc].m_cov, NULL);
      gcsan_dst->gcs[ndst].v_means =
        MatrixCopy(gcsan_src->gcs[nsrc].v_means, NULL);
      gcsan_dst->max_labels = gcsan_dst->nlabels ;
    }
  else   /* add to existing GCS */
    {
      ...
      m_src =
        MatrixScalarMul(gcs_src->v_means, gcs_src->total_training, NULL) ;
      m_dst =
        MatrixScalarMul(gcs_dst->v_means, gcs_dst->total_training, NULL) ;
      MatrixAdd(m_src, m_dst, gcs_dst->v_means) ;
      MatrixFree(&m_src) ; MatrixFree(&m_dst) ;

      m_src =
        MatrixScalarMul(gcs_src->m_cov, gcs_src->total_training-1, NULL) ;
      m_dst =
        MatrixScalarMul(gcs_dst->m_cov, gcs_dst->total_training-1, NULL) ;
