...
void mgh_ludcmp(FLOATTYPE **a, int n, int *indx, FLOATTYPE *d) {
  ...
  *d = 1.0;
  for (i = 0; i < n; i++) {
    ...
    if (big == 0.0) ErrorExit(ERROR_BADPARM, "LU decomposition: matrix in singular");
    vv[i] = 1.0 / big;
  }
  for (j = 0; j < n; j++) {
    for (i = 0; i < j; i++) {
...
void mgh_svdcmp(FLOATTYPE **a, FLOATTYPE *w, FLOATTYPE **v, int m, int n) {
  ...
  if (m < n) ErrorExit(ERROR_BADPARM, "Singular value decompostion: input matrix has too few rows");
  rv1 = MGH_vector(n);
  for (i = 0; i < n; i++) {
    l = i + 1;
    rv1[i] = scale * g;
  ...
  for (k = n - 1; k >= 0; k--) {
    for (its = 1; its <= 30; its++) {
      ...
      if (its == 30) ErrorExit(ERROR_BADPARM, "Singular value decomposition failed to converge");
      x = w[l];
      nm = k - 1;
      y = w[nm];
      g = rv1[nm];
