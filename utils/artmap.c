...
int ArtmapProcess(ARTMAP *artmap, double *I)
{
  ...
  if (class == 4) {
    ...
    if (Gdiag & DIAG_WRITE)
      for (k = 0; k < artmap->noutputs; k++) printf("Mwj(4, %d) = %d\n", k, *Mwj(artmap, 4, k));
  }
...
int artProcess(ARTMAP *artmap, double *I)
{
  ...
  for (i = 0; i < artmap->f2nodes; i++) artmap->flags[i] &= ~ARTMAP_RESET;
  ...
  do {
    ...
    if (artClass < 0) /* commit new node */
    {
      if (artmap->f2nodes < artmap->max_f2) {
        if (Gdiag & DIAG_WRITE) printf("committing new f2 node %d to class %d\n", artmap->f2nodes, class);
    ...
    if (artClass != class) {
      if (Gdiag & DIAG_WRITE) printf("artClass %d != class %d --> match tracking\n", artClass, class);
    }
...
static int artFeedBack(ARTMAP *artmap, int class)
{
  ...
  if (Gdiag & DIAG_WRITE)
    printf(
        "artFeedBack(%d) --> match %2.3f, n(I) %2.3f, n(I^Zj) %2.3f, "
        "rho %2.3f %s\n",
        class,
        match,
        norm_I,
...
static int artFeedForward(ARTMAP *artmap)
{
  ...
  for (j = 0; j < artmap->f2nodes; j++) {
    if (artmap->flags[j] & ARTMAP_RESET) continue; /* this node was previous winner */
...
int mapFeedForward(ARTMAP *artmap, int class)
{
  ...
  if (Gdiag & DIAG_WRITE) printf("mapFeedForward(%d) --> %d\n", class, apredict);
  return (apredict);
}
...
void mapLearn(ARTMAP *artmap, int aclass, int mapclass)
{
  ...
  for (k = 0; k < artmap->noutputs; k++) {
    ...
    if (k != mapclass) {
      if (Gdiag & DIAG_WRITE) printf("deleting map connection %d --> %d\n", aclass, k);
      *wj = 0;
    }
  }
}
