...
IC_VERTEX ic12_vertices[ICO_NVERTICES] = {{.0000, .0000, 1.0000},
                                          {.8944, .0000, .4472},
                                          {.2764, .8507, .4472},
                                          {-.7236, .5257, .4472},
                                          {-.7236, -.5257, .4472},
...
IC_FACE ic12_faces[ICO_NFACES] = {{{1, 5, 4}},   {{1, 6, 5}},    {{1, 2, 6}},   {{1, 3, 2}},   {{1, 4, 3}},
...
MRI_SURFACE *ic12_make_surface(int max_vertices, int max_faces) {
  ...
  for (vno = 0; vno < ICO_NVERTICES; vno++) {
    ...
    if (!v->v) ErrorExit(ERROR_NOMEMORY, "ic12: could not allocate %dth vertex list.", vno);
    v->vnum = 0;
  }
  ...
  for (vno = 0; vno < ICO_NVERTICES; vno++) {
    ...
    if (!v->f) ErrorExit(ERROR_NO_MEMORY, "ic12: could not allocate %d faces", v->num);
    v->n = (uchar *)calloc(v->num, sizeof(uchar));
    if (!v->n) ErrorExit(ERROR_NO_MEMORY, "ic12: could not allocate %d nbrs", v->n);
