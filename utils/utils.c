...
void printContents(xmlDocPtr doc, xmlNodePtr cur)
{
  xmlChar *contents;
  ...
  while(i<strlen((char *)contents))
  {
    ...
    if (*(contents+i)==' ')
      i++;
    for(j=i; j<78-tabNum*8+i&&j<strlen((char *)contents)&&(wrdLen((char *)(contents+j))>78-tabNum*8||wrdLen((char *)(contents+j))<=78-tabNum*8+i-j); j++)
    {
      if (*(contents+j)=='\n')
      {
        j++;
