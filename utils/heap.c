...
Xheap xhInit(XheapElement *array, int N) {
  ...
  /* down build */
  for (i = N / 2; i >= 1; i--) xhDownHeap(i, H);
...
int xhDownHeap(int k, Xheap H) {
  ...
  while (k <= N / 2) {
    k_minson = k + k;
    if (k_minson < N) {
      if (a[k_minson].value > a[k_minson + 1].value) k_minson = k_minson + 1; /* always locate the smallest son */
    }
    if (v.value <= a[k_minson].value) break; /* break out the loop */
    a[k] = a[k_minson];
    p = a[k].p;
    *p = k;
