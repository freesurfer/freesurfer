...
void mincGetVolInfo(char *infoline, VOL_GEOM *vgSrc, VOL_GEOM *vgDst)
{
  char *psrcVol=0;
  char *pdstVol=0;
  int retVal;
...
LTA *ltaMNIreadEx(const char *fname)
{
  LTA *lta = 0;
  LINEAR_TRANSFORM *lt ;
  char             *cp, line[1000], infoline[1024];
  FILE             *fp ;
  int              row ;
  MATRIX           *m_L ;
  ...
  mincGetVolInfo(infoline, &lta->xforms[0].src, &lta->xforms[0].dst);
  lta->type = LINEAR_RAS_TO_RAS;
  return lta;
}
