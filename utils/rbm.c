...
int
RBMsortLabelProbabilities(RBM *rbm)
{
  int  l ;
...
static MRI *
layer_weights_to_mri(DBN *dbn, int layer) 
{
  ...
  if (rbm_first->input_type == RBM_INPUT_IMAGE)
  {
    ...
    */
    ...
    MRIfree(&mri_tmp) ; MRIfree(&mri_previous_layer) ;
    MRIcopyHeader(rbm->mri_inputs, mri) ;
  }
  else
  {
...
static int
dump_gradients(RBM *rbm, double *dvisible_bias, double *dvariance, double *dhidden_bias, double **dw, RBM_PARMS *parms, int step) 
{
  int  v, h ;
...
#endif
double
RBMfreeEnergy(RBM *rbm, double *visible) 
{
...
int
RBMwriteNetwork(RBM *rbm, int n, RBM_PARMS *parms, int layer) 
{
  MRI *mri ;
  char fname[STRLEN] ;
...
int
RBMprintNetworkActivations(RBM *rbm, FILE *fp, int n, RBM_PARMS *parms) 
{
  int  v, h ;
...
RBM  *
RBMcopy(RBM *rbm_src, RBM *rbm_dst)
{
  int l, v, h ;
...
RBM  *
RBMalloc(int type, int nvisible, int nhidden, int nlabels, int input_type) 
{
  RBM   *rbm ;
  int   v, h ;
  double wt_lim;
...
int
RBMactivateForward(RBM *rbm, double *visible)
{
  int    h ;
...
int
RBMfillVisible(RBM *rbm, MRI *mri_inputs, double *visible, int x0, int y0, int z0, int f0, int ksize)
{
...
int
RBMmostLikelyLabel(RBM *rbm) 
{
  int    best_label, label ;
  double pbest ;

  pbest = rbm->labels[0] ; best_label = 0 ;  
...
double
RBMvoxlistRMS(RBM *rbm, VOXLIST *vl, RBM_PARMS *parms, int *indices, int index, int num)
{
  ...
  if (indices)
  {
    ...
    {
      i = indices[ind] ;
      x = vl->xi[i] ;  y = vl->yi[i] ;  z = vl->zi[i] ; f = vl->fi[i] ;
...
double
RBMvoxlistHistoRMS(RBM *rbm, VOXLIST *vl, RBM_PARMS *parms, int *indices, int index, int num)
{
...
//    rms += HISTOrmsDifference(histo_data[v], histo_recon[v]) ;
  ...
  rms /= rbm->nvisible ;
  free(visible) ; free(histo_recon) ; free(histo_data) ;
  callno++ ;
  return(rms) ;
}

int
RBMcomputeGradients(RBM *rbm, VOXLIST *vl, double **dw, double *dvisible_bias, double *dhidden_bias, double *dvariance, double *dlabel_bias, double **dlabel_weights, RBM_PARMS *parms, int *indices, int index) 
{
...
//    dhidden_bias[h] += parms->l_sparsity*(parms->sparsity - db_sparsity[h]) ;
  ...
  free(active) ; free(visible) ; free(hidden0) ; free(db_sparsity) ;
  return(NO_ERROR) ;
}

int
RBMtrainFromImage(RBM *rbm, MRI *mri_inputs, MRI *mri_labels, RBM_PARMS *parms)
{
  VOXLIST  *vl ;
...
int
RBMtrainFromVoxlistImage(RBM *rbm, VOXLIST *vl, RBM_PARMS *parms) 
{
  double   sparsity, training_rate, last_rms, pct_diff, rms, momentum, delta,min_rms,
    **dw, *dvisible_bias, *dhidden_bias, *dvariance, *last_dvariance, *dlabel_bias = NULL, **dlabel_weights = NULL,
    **last_dw, *last_dvisible_bias, *last_dhidden_bias, *last_dlabel_bias = NULL, **last_dlabel_weights = NULL,
    mean, var ;
  ...
  printf("training on %d voxels, mini batch size %d (%d), held out %d\n", 
	 vl->nvox, parms->mini_batch_size,  parms->batches_per_step, parms->held_out) ;
...
//  held_out_index = 0 ;
  ...
  sparsity = parms->sparsity[0] ; training_rate = parms->training_rates[0] ; momentum = parms->momentum[0] ;
  ...
  for (nbad = step = 0 ; step < parms->nsteps ; step++)
  {
    for (b = 0 ; b < parms->batches_per_step ; b++)
    {
      ...
      RBMcomputeGradients(rbm, vl, dw, dvisible_bias, dhidden_bias, dvariance, dlabel_bias, dlabel_weights,
			  parms, indices, index) ;
...
//	memset(last_dw[v], 0, rbm->nhidden*sizeof(last_dw[v][0])) ;
  ...
  for (v = 0 ; v < rbm->nvisible ; v++)
  {
    free(dw[v]) ; free(last_dw[v]) ;
  }
  free(dw) ; free(last_dw) ; 
  free(dvisible_bias) ; free(last_dvisible_bias) ;  free(dvariance) ;
  free(dhidden_bias) ; free(last_dhidden_bias) ;  free(last_dvariance) ;
  return(NO_ERROR) ;
}
int
RBMcountHiddenActive(RBM *rbm)
{
  int hidden_active, h ;
...
MRI *
RBMaverageActiveHiddenReceptiveFields(RBM *rbm, MRI *mri_receptive_fields, MRI *mri_inputs, int x0, int y0, int z0)
{
...
MRI *
RBMreconstruct(RBM *rbm, MRI *mri_inputs, MRI *mri_reconstructed, MRI **pmri_labeled, RBM_PARMS *parms)
{
  ...
  if (rbm->input_type == RBM_INPUT_IMAGE)
  {
    center = (parms->ksize*parms->ksize-1)/2 ; whalf = (parms->ksize-1)/2 ;
...
*/
    ...
    sprintf(fname, "%s.hidden.plt", parms->base_name) ; printf("saving %s\n", fname) ;
    HISTOplot(histo, fname) ;
    sprintf(fname, "%s.hidden_labels.plt", parms->base_name) ; printf("saving %s\n", fname) ;
  ...
  HISTOfree(&histo) ; MRIfree(&mri_receptive_fields) ; HISTO2Dfree(&histo_labels) ;
  free(hidden_counts) ;
  if (pmri_labeled)
...
DBN *
DBNalloc(int type, int nlayers, int nvisible, int *nhidden, int nlabels, int input_type) 
{
  DBN   *dbn ;
  int    itype, nl, layer ;
...
int
DBNwrite(DBN *dbn, char *fname)
{
  return(NO_ERROR) ;
}

int
DBNactivateForward(DBN *dbn, double *visible, int nlayers)
{
  int l  ;
...
int
DBNactivateBackward(DBN *dbn, int last_layer, int first_layer)
{
  int l  ;
  ...
  {
    memcpy(dbn->rbms[l-1]->hidden_state, dbn->rbms[l]->visible, dbn->rbms[l]->nvisible*sizeof(dbn->rbms[0]->visible[0])) ;
...
int
DBNtrainFromImage(DBN *dbn, MRI *mri_inputs, MRI *mri_labels, RBM_PARMS *parms)
{
  VOXLIST  *vl ;
...
int
DBNtrainFromVoxlistImage(DBN *dbn, VOXLIST *vl, RBM_PARMS *parms) 
{
  double   sparsity, training_rate, last_rms, pct_diff, rms, momentum, delta,min_rms,
    **dw, *dvisible_bias, *dhidden_bias, *dvariance, *last_dvariance, *dlabel_bias = NULL, **dlabel_weights = NULL,
    **last_dw, *last_dvisible_bias, *last_dhidden_bias, *last_dlabel_bias = NULL, **last_dlabel_weights = NULL,
...
//  held_out_index = 0 ;
  ...
  for (l = 0 ; l < dbn->nlayers ; l++)
  {
    ...
    for (nbad = step = 0 ; step < parms->nsteps ; step++)
    {
      ...
      {
	index = (int)nint(randomNumber(0, held_out_index-(parms->mini_batch_size+1))) ;
	DBNcomputeGradients(dbn, l, vl, dw, dvisible_bias, dhidden_bias, dvariance, dlabel_bias, dlabel_weights,
			    parms, indices, index) ;
...
int
DBNwriteNetwork(DBN *dbn, int n, RBM_PARMS *parms) 
{
  int  layer ;
  MRI *mri ;
  char fname[STRLEN] ;
...
int
DBNcomputeGradients(DBN *dbn, int layer, VOXLIST *vl, double **dw, 
...
MRI *
DBNreconstruct(DBN *dbn, MRI *mri_inputs, MRI *mri_reconstructed, MRI **pmri_labeled, RBM_PARMS *parms)
{
  ...
  histo_labels = HISTO2Dinit(NULL, rbm_last->nhidden, rbm_last->nlabels, 0, rbm_last->nhidden-1,0,rbm_last->nlabels-1) ;
  ...
  if (rbm_first->input_type == RBM_INPUT_IMAGE)
  {
    ...
    for (n = 0 ; n < dbn->nlayers ; n++)
    {
      sprintf(fname, "%s.hidden.layer%d.plt", parms->base_name,n) ; printf("saving %s\n", fname) ;
...
double
DBNvoxlistRMS(DBN *dbn, int layer, VOXLIST *vl, RBM_PARMS *parms, int *indices, int index, int num)
{
...
int
CDBNtrainFromImage(CDBN *cdbn, MRI *mri_inputs, MRI *mri_labels, RBM_PARMS *parms)
{
  VOXLIST  *vl ;
...
CDBN *
CDBNalloc(int type, int nlayers, int *ksizes, int *ngroups, int nlabels, MRI *mri_inputs) 
{
  CDBN   *cdbn ;
  ...
  {
    cdbn->mri_outputs[layer] = MRIallocSequence(mri_inputs->width, mri_inputs->height, mri_inputs->depth, MRI_FLOAT, ngroups[layer]);
...
static int
reset_hidden_nodes(CDBN *cdbn, int layer, double min_active, double max_active) 
{
...
int
CDBNtrainFromVoxlistImage(CDBN *cdbn, VOXLIST *vl, RBM_PARMS *parms, MRI *mri_inputs, MRI *mri_labels) 
{
  double   sparsity, training_rate, last_rms, pct_diff, rms, momentum, delta,min_rms,
    **dw, *dvisible_bias, *dhidden_bias, *dvariance, *last_dvariance, *dlabel_bias = NULL, **dlabel_weights = NULL,
    **last_dw, *last_dvisible_bias, *last_dhidden_bias, *last_dlabel_bias = NULL, **last_dlabel_weights = NULL,
    mean, var, label_rms, min_label_rms, last_label_rms, label_pct_diff, saved_rms ;
...
//  held_out_index = 0 ;
  ...
  for (layer = 0 ; layer < cdbn->nlayers ; layer++)
  {
    ...
    printf("************* TRAINING LAYER %d, sparsity = %2.2f wt %2.2f, momentum = %2.1f, dt = %2.4f, weight decay %2.5f ************\n", 
	   layer,sparsity, parms->l_sparsity[layer],momentum,training_rate,parms->weight_decays[layer]) ;
    rbm = cdbn->rbms[layer] ;
    rbm_min = RBMcopy(rbm, NULL) ; rbm_min_label = RBMcopy(rbm,NULL) ;
    ...
    min_rms = last_rms = CDBNvoxlistRMS(cdbn, layer, vl, parms, indices, held_out_index, parms->held_out, &min_label_rms) ; last_label_rms = min_label_rms ;
    if (rbm->nlabels > 0)
    ...
    for (nbad = step = 0 ; step < parms->nsteps ; step++)
    {
      ...
      {
	index = (int)nint(randomNumber(0, held_out_index-(parms->mini_batch_size+1))) ;
	CDBNcomputeGradients(cdbn, layer, vl, dw, dvisible_bias, dhidden_bias, dvariance, dlabel_bias, dlabel_weights,
			    parms, indices, index) ;
	...
	printf("iter %3.3d: rms = %2.5f (%2.3f%%), label rms = %2.5f (%2.3f%%) %s\n", step+1, rms, pct_diff, label_rms, label_pct_diff, new_min ? "****" : "") ;
      }
      else
...
//	memset(last_dw[v], 0, rbm->nhidden*sizeof(last_dw[v][0])) ;
	...
	{
	  index = (int)nint(randomNumber(0, held_out_index-(parms->mini_batch_size+1))) ;
	  CDBNcomputeDiscriminativeGradients(cdbn, layer, vl, dw, dhidden_bias, dvariance, dlabel_bias, dlabel_weights,
					     parms, indices, index) ;
...
//	memset(last_dw[v], 0, rbm->nhidden*sizeof(last_dw[v][0])) ;
    ...
    if (rbm->nlabels  > 0)
    {
      for (v = 0 ; v < rbm->nlabels ; v++)
      {
	free(dlabel_weights[v]) ; free(last_dlabel_weights[v]) ;
      }
      free(last_dlabel_bias) ; free(last_dlabel_weights) ;
    }
    for (v = 0 ; v < rbm->nvisible ; v++)
    {
      free(dw[v]) ; free(last_dw[v]) ;
...
double
CDBNvoxlistRMS(CDBN *cdbn, int layer, VOXLIST *vl, RBM_PARMS *parms, int *indices, int index, int num, double *plabel_rms)
{
...
int
CDBNcomputeGradients(CDBN *cdbn, int layer, VOXLIST *vl, double **dw, 
		     double *dvisible_bias, double *dhidden_bias, double *dvariance,
		     double *dlabel_bias, double **dlabel_weights,
		     RBM_PARMS *parms, int *indices, int index) 
{
  ...
  for (ind = index ; ind < index+parms->mini_batch_size; ind++)
  {
    ...
    memcpy(visible, rbm->visible, rbm->nvisible*sizeof(rbm->visible[0])) ;
    memcpy(hidden0, rbm->hidden, rbm->nhidden*sizeof(rbm->hidden[0])) ; // Bengio says use hidden_state here, but Lee and Ng posterior
    RBMactivateBackward(rbm) ;
    if (parms->debug > 1)
    {
      printf("index %d: %2.3f\n", i, visible[0]) ;
...
#if 1
int
CDBNcomputeDiscriminativeGradients(CDBN *cdbn, int layer, VOXLIST *vl, double **dw, 
...
int
CDBNcomputeLabelGradients(CDBN *cdbn, int layer, VOXLIST *vl, double **dw, 
				   double *dhidden_bias, double *dvariance,
				   double *dlabel_bias, double **dlabel_weights,
				   RBM_PARMS *parms, int *indices, int index) 
{
  ...
  for (ind = index ; ind < index+parms->mini_batch_size; ind++)
  {
	...
	if (compute_weights)
	{
	  ...
	  {
	    var = exp(rbm->variance[v]) ;
	    dw[v][h] += (l0 - rbm->labels[l]) * rbm->label_weights[l][h] * rbm->hidden[h] * (1 - rbm->hidden[h]) * visible[v]/var;
	  }
	}
      }
    }
...
#endif
int
CDBNactivateForward(CDBN *cdbn, double *visible, int nlayers)
{
  int l  ;
...
int
CDBNactivateBackward(CDBN *cdbn, int last_layer, int first_layer)
{
  int l  ;
  ...
  for (l = last_layer ; l > first_layer ; l--)
  {
    memcpy(cdbn->rbms[l-1]->hidden_state, cdbn->rbms[l]->visible, cdbn->rbms[l]->nvisible*sizeof(cdbn->rbms[0]->visible[0])) ;
...
int
CDBNfillVisible(CDBN *cdbn, MRI *mri_inputs, double *visible, int x0, int y0, int z0, int ksize)
{
...
MRI *
CDBNcreateOutputs(CDBN *cdbn, RBM_PARMS *parms, MRI *mri_inputs, int first_layer, int last_layer, MRI **pmri_labeled) 
{
  ...
  for (layer = first_layer, mri_layer_inputs = mri_inputs ; layer <= last_layer ; layer++)
  {
    ...
    mri_layer_inputs = mri_outputs ;
    {
      ...
      fp = fopen(fname, "w") ;
      always = never = 0 ; nvox = mri_inputs->height*mri_inputs->width*mri_inputs->depth ;
      for (h = 0 ; h < rbm->nhidden ; h++)
      {
	if (!hidden_counts[h])
	  never++ ;
	...
	fprintf(fp, "%d %d\n", h, hidden_counts[h]) ;
      ...
      printf("writing hidden counts to %s (%d/%2.1f%% always active, %d/%2.1f%% never)\n",
	     fname,  always, 100.0*always/rbm->nhidden, never, 100.0f*never/rbm->nhidden) ;
    }
...
int
CDBNwriteNetwork(CDBN *cdbn, int n, RBM_PARMS *parms, int layer) 
{
  MRI *mri ;
  char fname[STRLEN] ;
...
int
dump_visible(RBM *rbm, char *fname)
{
...
int
dump_hidden(RBM *rbm, char *fname)
{
  FILE *fp = fopen(fname, "w") ;
  int  v ;
