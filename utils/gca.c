...
static HISTOGRAM *gcaGetLabelHistogram(GCA *gca, 
                                       int label,
                                       int frame,
                                       int border) ;
...
static int mriFillRegion(MRI *mri,
                         int x,int y,int z,
                         int frame,
                         float fill_val,
                         int whalf);
                            ...
                            float *pxv, float *pyv, float *pzv )
{
  ...
  if (transform->type == LINEAR_VOX_TO_VOX)
  {
    ...
    *MATRIX_RELT(v_dst, 4, 1) = 1.0 ;
    ...
    V3_X(v_src) = xp ;  V3_Y(v_src) = yp ;  V3_Z(v_src) = zp ; 
    MatrixMultiply(m, v_src, v_dst) ;
    xc = V3_X(v_dst) ; yc = V3_Y(v_dst) ; zc = V3_Z(v_dst) ;
    MatrixFree(&m) ; VectorFree(&v_src) ; VectorFree(&v_dst) ;
...
GCAwrite(GCA *gca, const char *fname)
{
  ...
  if (znz_isnull(file))
  {
    ...
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM,
                 "GCAwrite(%s): could not open file",fname)) ;
  }
...
GCAread(const char *fname)
{
  ...
  if (znz_isnull(file))
  {
    ErrorReturn(NULL, 
                (ERROR_BADPARM,
                 "GCAread(%s): could not open file", fname)) ;
  }
...
float
GCAnormalizedLogSampleProbability(GCA *gca,
                                  GCA_SAMPLE *gcas,
                                  MRI *mri_inputs,
                                  TRANSFORM *transform,
                                  int nsamples,
...
*/
float
GCAcomputeLogImageProbability(GCA *gca,
                              MRI *mri_inputs,
                              MRI *mri_labels,
                              TRANSFORM *transform)
{
                       ...
                       int unknown_nbr_spacing, int use_ventricles)
{
  ...
  float min_unknown[MAX_GCA_INPUTS], max_unknown[MAX_GCA_INPUTS],
    ...
    means[MAX_DIFFERENT_LABELS][MAX_GCA_INPUTS], 
                                ...
                                const char *fname,TRANSFORM *transform)
{
  ...
  mri_dst = MRIallocSequence(mri->width,
                             mri->height,
                             mri->depth,
                             MRI_FLOAT,
                             gca->ninputs) ;
...
double
GCAimagePosteriorLogProbability(GCA *gca,
                                MRI *mri_labels,
                                MRI *mri_inputs,
                                TRANSFORM *transform)
{
...
#define MAX_PEAK_BINS 40
    ...
    printf("skull bounding box = (%d, %d, %d) --> (%d, %d, %d)\n",
           ...
           tbox.x+tbox.dx-1, tbox.y+tbox.dy-1, tbox.z+tbox.dz-1);
    box.x += tbox.x ;
    box.y += tbox.y ;
    box.z += tbox.z ;
    box.dx += tbox.dx ;
...
void
load_vals( const MRI *mri_inputs,
...
int
GCAmapRenormalizeWithAlignmentLongitudinal(GCA *gca,
					   MRI *mri,
					   TRANSFORM *transform,
					   FILE *logfp,
					   const char *base_name,
					   ...
					   int handle_expanded_ventricles)
{
  ...
  return(GCAcomputeRenormalizationWithAlignmentLongitudinal(gca, mri, transform,
         logfp, base_name, plta,
         handle_expanded_ventricles,
         NULL, NULL, NULL, NULL));
}
                               ...
                               int *old_label_computed)
{
  ...
  float plabel_scales[MAX_CMA_LABELS],
    plabel_offsets[MAX_CMA_LABELS],plabel_peaks[MAX_CMA_LABELS];
...
MRI *
MRImarkPossibleBorders(MRI *mri, MRI *mri_borders, float border_thresh)
{
                                ...
                                float *plabel_peaks, int* plabel_computed)
{
  ...
  if (base_name)
  {
    ...
    for (l = 0 ; l < MAX_CMA_LABELS ; l++)
      fprintf(fp, "%d %s %2.2f %2.1f %2.0f\n",
              l, cma_label_to_name(l), label_scales[l], 
              label_offsets[l], label_peaks[l]) ;
...
*/
int
GCAcomputeRenormalizationWithAlignmentLongitudinal(
  GCA *gca,
  MRI *mri, TRANSFORM *transform,
  FILE *logfp,const char *base_name, LTA **plta,
  int handle_expanded_ventricles,
...
{
  HISTOGRAM *h_mri, *h_gca, *h_mtl = NULL, *h_caudate ;
  int l, nbins, i, x, y, z, num, frame, bin, j, computed[MAX_CMA_LABELS],
    label, k, gca_peak, mri_peak ;
  float fmin, fmax, label_scales[MAX_CMA_LABELS], overlap,
    mean_gm_scale, mean_wm_scale, mean_csf_scale, label_peaks[MAX_CMA_LABELS],
    label_offsets[MAX_CMA_LABELS],
    mean_wm_offset, mean_csf_offset, mean_gm_offset,
  ...
  nbins = 256 ;
  h_mri = HISTOalloc(nbins) ; h_mtl = HISTOalloc(nbins) ; h_caudate = HISTOalloc(nbins) ;
  for (j = 0 ; j < NALIGN_LABELS ; j++)
  {
    l = align_labels[j] ;
    if (l == Gdiag_no)
    ...
    mri_seg = MRIalloc(mri->width, mri->height, mri->depth, MRI_UCHAR) ; MRIcopyHeader(mri, mri_seg) ;
  ...
  HISTOfree(&h_gca) ; HISTOfree(&h_mri) ; HISTOfree(&h_mtl) ; HISTOfree(&h_caudate) ;
                  ...
                  MRI *mri_dst, MRI_SEGMENT *mseg)
{
  ...
  for (max_posterior = 0.0, old_label = max_label = i = 0 ; 
       i < mseg->nvoxels ; i++)
  {
    x = mseg->voxels[i].x ;
    y = mseg->voxels[i].y ;
...
COLOR_TABLE *GCAcolorTableCMA(GCA *gca)
{
  ...
  nmax = 0;
  for (n=0; n<1000; n++) if (labelhitlist[n])
    {
      nmax=n;
    }
...
#endif
float
GCAcomputeLabelPosterior(GCA *gca,
                         TRANSFORM *transform,
                         MRI *mri,
                         float x, float y, float z,
                         int label)
...
float
GCAcomputeLabelLikelihood(GCA *gca,
                          TRANSFORM *transform,
                          MRI *mri,
                          float x, float y, float z,
                          int label)
...
gcaBuildNbhdGCAN(GCA *gca, int x, int y, int z, int wsize, float sigma)
{
  ...
  {
    ErrorExit(ERROR_NOMEMORY,
              "gcaBuildNbhdGCAN: could not allocate node gcs") ;
  }
  for (n = nlabels = 0 ; n <= max_label ; n++)
  {
    if (labels[n] > 0)
      ...
      for (y = 0 ; y < height ; y++)
      {
        for (z = 0 ; z < depth ; z++)
        {
                                    ...
                                    transform, x, y, z, &xn, &yn, &zn))
          {
            ...
            {
              GCAsourceVoxelToPrior(gca, mri_inputs, transform, 
                                    x, y, z, &xp, &yp, &zp);
              ...
              for (n = 0 ; n < gcap_total->nlabels ; n++)
              {
                ...
                p = gcaGibbsLogPosterior(gca,
                                         ...
                                         vals,
                                         gcap_total->labels[n],
                                         x, y, z, gcap_total, gcan_total,transform);
                if (p > max_p)
                {
                  max_p = p ;
...
MRI *
GCAcomputeOptimalScale(GCA *gca,
                       TRANSFORM *transform,
                       MRI *mri_inputs,
                       MRI *mri_aseg,
                       MRI *mri_sigma,
...
GCAupdateDistributions(GCA *gca, MRI *mri, TRANSFORM *transform)
{
  ...
  all_vals = (float *)calloc(gca->node_width*gca->node_height*gca->node_depth,
                             sizeof(float)) ;
  for (label = 0 ; label <= MAX_CMA_LABEL ; label++)
  {
...
is_possible_wmsa(GCA *gca, MRI *mri, TRANSFORM *transform, int x, int y, int z, int whalf)
{
  ...
  for (xi = -whalf ; xi <= whalf ; xi++)
    for (yi = -whalf ; yi <= whalf ; yi++)
      for (zi = -whalf ; zi <= whalf ; zi++)
      {
        GCAsourceVoxelToPrior( gca, mri, transform,
                               x+xi, y+yi,z+zi,
                               &xp, &yp, &zp) ;
        gcap = getGCAP(gca, mri, transform, x, y, z) ;
        if (gcap == NULL)
        ...
        if (left_wmsa > never_thresh || 
            right_wmsa > never_thresh || 
            wmsa > never_thresh)
        {
...
{ 
  ...
  for(i=0; i<nreftissues; i++) {
    ...
    MRIcomputeLabelMeansandCovariances2(wmsa->modalities,wmsa->seg,&reftissue_covs[i],&reftissue_means[i],&llabel[0],1,0);
  ...
  for (j=0; j<nreftissues; j++)   {
    for (k=0; k<nmodalities; k++) {
      if(reftissue_means[j]->rptr[1][k+1] < wmsa_means->rptr[1][k+1]) wmsacomp[j][k] = 0 ;
      else                                                         wmsacomp[j][k] = 1 ;
    }
  }
...
void
GCAinitLabelsFromMRI(GCA *gca, MRI *mri_labels)
{
  int x, y, z, label ;
  GCA_PRIOR *gcap ;
  GCA_NODE  *gcan ;
  
  if (gca->width != mri_labels->width ||
      gca->height != mri_labels->height ||
