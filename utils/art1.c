...
ART1 *Art1Read(char *fname) {
  ...
  for (j = 0; j < art1->noutputs; j++) art1->flags[j] |= ART1_COMMITTED;
...
int Art1Process(ART1 *art1, double *I) {
  ...
  for (i = 0; i < art1->ninputs; i++) art1->f0[i] = art1->f1[i] = I[i];
...
int artFeedBack(ART1 *art1, int class) {
  ...
  if (Gdiag & DIAG_WRITE)
    printf(
        "artFeedBack(%d) --> match %2.3f, n(I) %2.3f, "
...
int artFeedForward(ART1 *art1) {
  ...
  for (j = 0; j < art1->noutputs; j++) {
    if (art1->flags[j] & ART1_RESET) continue; /* this node was previous winner */
  ...
  if (Gdiag & DIAG_WRITE) printf("artFeedForward() --> %d (%2.3f)\n", max_j, max_out);
  return (max_j);
}
/*----------------------------------------------------------------------
    Parameters:
...
double norm(double huge *a, int len) {
  ...
  for (norm_val = 0.0, i = 0; i < len; i++, a++) norm_val += fabs(*a);
