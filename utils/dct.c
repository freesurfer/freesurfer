...
DCT *
DCTalloc(int ncoef, MRI *mri_source)
{
  DCT   *dct ;
  ...
  if (dct->x_inv == NULL || dct->y_inv == NULL || dct->z_inv == NULL ||
      dct->x == NULL || dct->y == NULL || dct->z == NULL)
...
DCTfree(DCT **pdct)
{
  ...
  *pdct = NULL ;
  ...
  if (dct->v_zk)
    VectorFree(&dct->v_zk) ;
  free(dct->x_inv) ; free(dct->y_inv) ; free(dct->z_inv) ;
  free(dct->x) ; free(dct->y) ; free(dct->z) ;
  free(dct) ;
  return(NO_ERROR) ;
}
int
DCTcreateMatrix(DCT *dct, MRI *mri, int skip)
{
  ...
  for (i = 0 ; i < 3 ; i++) // for each coordinate direction
  {
    ...
    {
    default:
    case 0: N = Nx ; m = dct->m_x_basis ; break ;
    case 1: N = Ny ; m = dct->m_y_basis ; break ;
    case 2: N = Nz ; m = dct->m_z_basis ; break ;
    }
    for (vox = 0 ; vox < N ; vox++)
    {
      dk = sqrt(1.0/N) ;
...
DCT *
DCTcopy(DCT *dct_src, DCT *dct_dst)
{
  if (dct_dst == NULL)
...
MRI *
DCTapplyInverse(DCT *dct, MRI *mri_src, MRI *mri_dst, int sample_type)
{
  int    x, y, z ;
  double xs, ys, zs ;
  double val ;
...
int
DCTtransformVoxlist(DCT *dct, VOXEL_LIST *vl)
{
  int    i ;
...
int
DCTinverseTransformVoxlist(DCT *dct, VOXEL_LIST *vl)
{
  int    i, x, y, z ;
	MATRIX *m_vox2vox ;
  ...
  if (v1 == NULL)
  {
    ...
    VECTOR_ELT(v1, 4) = 1.0 ; VECTOR_ELT(v1, 4) = 1.0 ;
  }
	m_vox2vox = MRIgetVoxelToVoxelXform(vl->mri, dct->mri_source) ;
  for (i = 0 ; i < vl->nvox ; i++)
  {
		V3_X(v1) = vl->xi[i] ; V3_Y(v1) = vl->yi[i] ; V3_Z(v1) = vl->zi[i] ; 
    MatrixMultiply(m_vox2vox, v1, v2) ;
                         ...
                         double *px, double *py,  double *pz)
{
  ...
  xi = nint(x/dct->res) ; yi = nint(y/dct->res) ; zi = nint(z/dct->res) ;
  if (xi < 0)
    xi = 0 ;
  if (yi < 0)
    yi = 0 ;
  ...
  xd = dct->x_inv[xi] ; yd = dct->y_inv[yi] ; zd = dct->z_inv[zi] ; 
  *px = xd ; *py = yd ; *pz = zd ;
  return(NO_ERROR) ;
}

int
DCTdump(DCT *dct, FILE *fp)
{
  int k ;
  fprintf(fp, "DCT (%d):\n", dct->ncoef) ;
  for (k = 1 ; k <= dct->ncoef ; k++)
    fprintf(fp, "\t(%2.3f, %2.3f, %2.3f)\n", 
            ...
            VECTOR_ELT(dct->v_yk, k),            
            VECTOR_ELT(dct->v_zk, k));
  fflush(fp) ;
  return(NO_ERROR) ;
}
...
DCTupdate(DCT *dct)
{
  ...
  for (i = 0 ; i < 3 ; i++)
  {
    ...
    for (j = 0 ; j < N ; j++)
    {
      ...
      if (jc >= 0 && jc < Ninv)
      {
        wts[jc] += jfd ; inv[jc] += jfd*j ;
      }
      if (jf >= 0 && jf < Ninv)
      {
        wts[jf] += jcd ; inv[jf] += jcd*j ;
      }
    }
    for (j = 0 ; j < Ninv ; j++)
      if (wts[j] > 0)
  ...
  free(x_wts) ; free(y_wts) ; free(z_wts) ;
  return(NO_ERROR) ;
}
static int
