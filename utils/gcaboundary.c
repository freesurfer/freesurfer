...
static int gcabUpdateDistributions(GCAB *gcab, MRI *mri_int, MRI *mri_seg, 
                                   MRI *mri_dist, TRANSFORM *transform,
                                   int x, int y, int z, float xn,
                                   float yn,float zn, int target_label);
...
static int gcabUpdateNode(GCAB *gcab, int xn, int yn, int zn,
                          double val1, double val2, float grad,int vno,float wt);
static int gcabFindClosestIcoVertex(GCAB *gcab, TRANSFORM *transform, 
                                    float x0, float y0, float z0, 
                                    float nx, float ny, float nz) ;
static int gcabValsToBins(GCAB *gcab, double val1, double val2, float grad, 
                          float *pi1f, float *pi2f, float *pgf) ;
GCAB *
GCABalloc(GCA *gca, int spacing, int ico_order, int nintensity_bins,
...
#define PAD 5
  ...
  gcab->bounding_box.x -= PAD ; gcab->bounding_box.y -= PAD ; gcab->bounding_box.z -= PAD ;
  gcab->bounding_box.dx += 2*PAD ; gcab->bounding_box.dy += 2*PAD ; gcab->bounding_box.dz += 2*PAD ;
  ...
  x1 = MIN(x1, gca->width) ; y1 = MIN(y1, gca->height) ; z1 = MIN(z1, gca->depth) ;
    ...
    for (y = 0 ; y < height ; y++)
    {
      ...
      if (!gcab->bs[x][y])
        ErrorExit(ERROR_NOMEMORY,
                  "GCABalloc: could not allocate %d,%dth *",x,y);
      for (z = 0 ; z < depth ; z++)
      {
        if (x == Gx && y == Gy && z == Gz)
          DiagBreak() ;
        ...
        for (vno = 0 ; vno < gcab->nvertices ; vno++)
        {
          ...
          gcabs->h_Iin_pdfs[vno] = HISTOinit(NULL, gcab->nintensity_bins, gcab->min_intensity, 
                                             gcab->max_intensity) ;
          gcabs->h_Iout_pdfs[vno] = HISTOinit(NULL, gcab->nintensity_bins, gcab->min_intensity, 
                                              gcab->max_intensity) ;
...
int
GCABcompleteTraining(GCAB *gcab)
{
  int    x, y, z, i1, i2, vno ;
  GCABS  *gcabs ;
...
double
GCABgetProbability(GCAB *gcab, MRI *mri_int, MRI *mri_dist, 
                   TRANSFORM *transform,
                   float x0, float y0, float z0,
                   float nx, float ny, float nz)
{
  float  xn, yn, zn, xi, yi, zi, xo, yo, zo, 
         xd, yd, zd, xi0, yi0, zi0, d, dx, dy, dz, xo0, yo0, zo0 ;
  int    vno, oob, xb, yb, zb, xu, yu, zu, x, y, z, num ;
  double grad, p, pdelI, mag, pI_total, val1, val2, pI_in, pI_out,
         e1x, e2x, e1y, e2y, e1z, e2z, d1, d2 ;
  GCABS  *gcabs ;

  oob = GCABsourceVoxelToNode(gcab,mri_int,transform,x0,y0,z0,&xn,&yn,&zn);
  if (oob != NO_ERROR)
    return(0.0) ;
  xo = x0 + SDIST*nx ; yo = y0 + SDIST*ny ; zo = z0 + SDIST*nz ;
  xi = x0 - SDIST*nx ; yi = y0 - SDIST*ny ; zi = z0 - SDIST*nz ;
  ...
  xb = (int)xn ; yb = (int)yn ; zb = (int)zn ;
  xd = xn-xb ; yd = yn-yb ; zd = zn-zb ;
  xu = xb+1 ;  yu = yb+1 ;  zu = zb+1 ; 

  pI_total = pdelI = 0 ;
#if 1
  x = nint(xn) ; y = nint(yn) ; z = nint(zn) ;
  gcabs = &gcab->bs[x][y][z] ;
  ...
  xo0 = xo ; yo0 = yo ; zo0 = zo ;
  for (d1 = -1 ; d1 <= 1.0 ; d1++)
    for (d2 = -1.0 ; d2 <= 1.0 ; d2++)
    {
      xo = xo0 + d1*e1x + d2*e2x ; yo = yo0 + d1*e1y + d2*e2y ; zo = zo0 + d1*e1z + d2*e2z ;
      MRIsampleVolume(mri_int, xo, yo, zo, &val2) ;
      if (val2 > gcab->max_intensity)
        val2 = gcab->max_intensity ;
      if (val2 < gcab->min_intensity)
    ...
    for (y = yb ; y <= yu ; y++)
    {
      for (z = zb ; z <= zu ; z++)
      {
        ...
        for (d = 0.0, num = 0 ; d > -10 ; d--)  // start 1/2mm inside border
        {
          ...
          i1b = (int)i1f ; i2b = (int)i2f ; gbinb = (int)gbinf ;
          i1u = i1b+1 ; i2u = i2b+1 ; gbinu = gbinb+1 ;
          i1d = i1f-i1b ;  i2d = i2f-i2b ;  gbind = gbinf-gbinb ; 
...
double
GCABgetPin(GCAB *gcab, MRI *mri_int, MRI *mri_dist, 
           TRANSFORM *transform,
           float x0, float y0, float z0,
                   float nx, float ny, float nz)
{
...
double
GCABgetPout(GCAB *gcab, MRI *mri_int, MRI *mri_dist, 
            TRANSFORM *transform,
            float x0, float y0, float z0,
            float nx, float ny, float nz)
{
...
double
GCABgetPgrad(GCAB *gcab, MRI *mri_int, MRI *mri_dist, 
             TRANSFORM *transform,
             float x0, float y0, float z0,
             float nx, float ny, float nz)
{
...
int
GCABtrain(GCAB *gcab, MRI *mri_int, MRI *mri_seg, TRANSFORM *transform, 
          int target_label)
{
    ...
    for (y = 0 ; y < mri_int->height ; y++)
    {
      for (z = 0 ; z < mri_int->depth ; z++)
      {
        ...
        if (oob != NO_ERROR)  // not in bounding box
          continue ;
        DiagBreak() ;
        gcabUpdateDistributions(gcab, mri_int, mri_seg, mri_dist, transform, x, y, z, 
                                xn, yn, zn, target_label) ;
      }
    }
  }

  MRIfree(&mri_border) ; MRIfree(&mri_dist) ;
  return(NO_ERROR) ;
}

int
GCABsourceVoxelToNode(GCAB *gcab, MRI *mri, TRANSFORM *transform,
...
////////////////////////////////////////////////////////////////
int
GCABvoxelToNode(GCAB *gcab, float xv, float yv, float zv,
                    float *pxn, float *pyn, float *pzn)
{
  float xn, yn, zn ;
  ...
  if (xn < 0 || yn < 0 || zn < 0 ||
      xn >= gcab->width || yn >= gcab->height || zn >= gcab->depth)
    return(ERROR_OUT_OF_BOUNDS) ;
  return NO_ERROR;
}
int
GCABwrite(GCAB *gcab, char *fname)
{
  FILE  *fp ;
  int   x, y, z, vno, i1, i2 ;
  GCABS *gcabs ;
...
GCABread(char *fname, GCA *gca) 
{
  ...
  if (version != GCAB_VERSION)
    ErrorReturn(NULL, (ERROR_BADFILE, "GCABread(%s): version %2.1f does not match expected version %2.1f",
  ...
  freadInt(fp) ; freadInt(fp) ; freadInt(fp) ;  // width, height and depth
...
static int
gcabUpdateDistributions(GCAB *gcab, MRI *mri_int, MRI *mri_seg, 
                        MRI *mri_dist, TRANSFORM *transform,
                        int x, int y, int z, float xn,float yn,float zn,
                        int target_label)
{
  float  nx, ny, nz, xi, yi, zi, xo, yo, zo, xd, yd, zd, dx, dy, dz, d, xi0, yi0, zi0,
         xo0, yo0, zo0, x0, y0, z0;
  ...
  x0 = (xi+xo)/2 ; y0 = (yi+yo)/2 ; z0 = (zi+zo)/2 ;
  xi = x0-SDIST*dx ; yi = y0-SDIST*dy ; zi = z0-SDIST*dz ;
  xo = x0+SDIST*dx ; yo = y0+SDIST*dy ; zo = z0+SDIST*dz ;
  MRIsampleVolumeDerivativeScale(mri_int, x0, y0, z0, nx, ny, nz, &grad,GRAD_SIGMA);
  if (grad > gcab->max_grad)
    grad = gcab->max_grad ;
  if (grad < gcab->min_grad)
...
static int
gcabUpdateNodeIout(GCAB *gcab, int xn, int yn, int zn, double val2,int vno,float wt)
{
  float  i1f, i2f, i2d, gbinf ;
  int    i2b, i2u ;
  GCABS  *gcabs ;

  if (xn < 0 || xn >= gcab->width ||
      yn < 0 || yn >= gcab->height ||
      zn < 0 || zn >= gcab->depth)
    return(NO_ERROR) ;
...
static int
gcabUpdateNodeIin(GCAB *gcab, int xn, int yn, int zn, double val1,int vno,float wt)
{
...
static int
gcabUpdateNode(GCAB *gcab, int xn, int yn, int zn,
               double val1, double val2, float grad,int vno,float wt)
{
...
static int
gcabComputeBorderNormal(GCAB *gcab, MRI *mri_seg, 
                        TRANSFORM *transform, 
...
static int
gcabFindClosestIcoVertex(GCAB *gcab, TRANSFORM *transform, 
                         float x0, float y0, float z0, 
                         float nx, float ny, float nz)
{
  int   n, max_n ;
  ...
  {
    dot = 
      nx * gcab->ico->vertices[n].x +
      ny * gcab->ico->vertices[n].y +
...
static int
gcabValsToBins(GCAB *gcab, double val1, double val2, float grad, 
               float *pi1f, float *pi2f, float *pgf)
{
  if (val1 < gcab->min_intensity)
    val1 = gcab->min_intensity ;
...
static MRI *
gcabWritePDFToMRI(GCAB *gcab, MRI *mri, int x, int y, int z)
{
  int    vno, i1, i2 ;
  GCABS  *gcabs ;
...
static MRI *
gcabWriteMRIToPDF(GCAB *gcab, MRI *mri, int x, int y, int z)
{
  int    vno, i1, i2 ;
  GCABS  *gcabs ;
...
int
GCABsmoothPDFs(GCAB *gcab, float sigma)
{
  int  x, y, z ;
  MRI  *mri_kernel ;
...
static int
gcabSmoothPDF(GCAB *gcab, int x, int y, int z, MRI *mri_kernel)
{
  MRI *mri_pdf, *mri_tmp ;
...
gcabSamplePDF(GCAB *gcab, GCABS *gcabs, int which, int vno, double val)
{
  ...
  index_b = (int)index_f ; index_d = index_f-index_b ; index_u = index_b+1 ;
  switch (which)
  {
  case GCAB_IIN:  h = gcabs->h_Iin_pdfs[vno] ; break ;
  case GCAB_IOUT: h = gcabs->h_Iout_pdfs[vno] ; break ;
  case GCAB_GRAD: h = gcabs->h_grad_pdfs[vno] ; break ;
  }
  p1 = h->counts[index_b] ; p2 = h->counts[index_u] ;
