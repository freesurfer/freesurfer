...
VOXEL_LIST *
VLSTcreate(MRI *mri, float low_val, float hi_val, VOXEL_LIST *vl, int skip, int border_only)
{
	int   x, y, z, nvox, i ;
	Real  val ;
	...
	for (nvox = x = 0 ; x < mri->width ; x+=skip)
	{
		for (y = 0 ; y < mri->height ; y+=skip)
		{
			for (z = 0 ; z < mri->depth ; z+=skip)
			{
				...
				if (val >= low_val && val <= hi_val)
				{
					if ((border_only == 0) || (MRIneighbors(mri, x, y, z, 0) > 0))
					{
						i = nvox++ ;
		...
		for (xk = -1 ; xk <= 1 ; xk++)
		{
			...
			for (yk = -1 ; yk <= 1 ; yk++)
			{
				...
				for (zk = -1 ; zk <= 1 ; zk++)
				{
					...
					if (MRIvox(mri_current, xi, yi, zi) == 0 && MRIvox(mri_new, xi, yi, zi) == 0 &&
			...
			for (yk = -1 ; yk <= 1 ; yk++)
			{
				...
				for (zk = -1 ; zk <= 1 ; zk++)
				{
					...
					if (MRIvox(mri_new, xi, yi, zi) == 1) // add it 
					{
						...
						vl_exp->xi[nvox] = xi ; vl_exp->yi[nvox] = yi ; vl_exp->zi[nvox] = zi ;
						nvox++ ;
					}
				}
			}
