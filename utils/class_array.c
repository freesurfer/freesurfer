...
static MATRIX *compute_ras_basis_vectors(MRI *mri_aseg_orig, 
                                         MRI *mri_aseg_edit, 
                                         int target_label, 
                                         int width, 
...
float **
CAbuildTrainingData(VOXEL_LIST *vl_total, int target_label, float *ca_classes, 
                    MRI **mri_smooth, MRI **mri_grad, MRI **mri_laplacian, MRI *mri_dtrans,
                    MRI **mri_2nd_deriv_s, int wsize, int nscales, int which_inputs)
{
  float **ca_inputs ;
  int   i, ninputs ;
  MRI   *mri_dtrans_grad ;
  ...
  for (i = 0 ; i < vl_total->nvox ; i++)
  {
    ...
    ca_inputs[i] = CAbuildInputsAtVoxel(vl_total, i,
                                         mri_smooth, mri_grad, mri_laplacian,
                                         mri_dtrans, mri_dtrans_grad, mri_2nd_deriv_s,
                                         wsize, nscales, NULL,
                                         which_inputs) ;
    if (ca_inputs[i] == NULL)
      ErrorExit(ERROR_NOMEMORY, "%s: could not allocate %dth CA input vector size %d", 
...
float *
CAbuildInputsAtVoxel(VOXEL_LIST *vl, int i, 
                     MRI **mri_smooth, MRI **mri_grad, MRI **mri_laplacian, MRI *mri_dtrans, 
                     MRI *mri_dtrans_grad, MRI **mri_2nd_deriv_s,
                     int wsize, int nscales, float *svm_inputs, int which_inputs)
{
...
float **
CAbuildInputs(VOXEL_LIST *vl_total, MRI *mri_intensity, MRI *mri_labels, int target_label, int which_inputs, 
              int wsize, int nscales, float *sigmas)
{
  float **svm_inputs ;
  ...
  svm_inputs = CAbuildTrainingData(vl_total, 0, NULL, mri_smooth, mri_grad, 
                                   mri_laplacian, mri_dtrans, mri_2nd_deriv_s, wsize, nscales, which_inputs) ;
...
CA *
CAalloc(int width, int height, int depth, MATRIX *m_vox2index, int type, int which_inputs,
        int wsize, int nscales,
        char *c1_name, char *c2_name, float *sigmas)
{
  CLASSIFIER_ATLAS *ca ;
  int              x, y, z, o, ninputs ;
  ...
  ca->ninputs = ninputs ;
  ca->width = width ; ca->height = height; ca->depth = depth ;
  ca->type = type ;
  ca->wsize = wsize ; ca->nscales = nscales ;
        ...
        for (o = 0 ; o < ICO0_NVERTICES ; o++)
        {
          ...
          case CA_GAUSSIAN:
            ...
            ca->classifiers[x][y][z][o].c2_vars = 
              (double *)calloc(ninputs, sizeof(double)) ;
            if (ca->classifiers[x][y][z][o].c1_means == NULL ||
                ca->classifiers[x][y][z][o].c1_vars == NULL ||
                ca->classifiers[x][y][z][o].c2_means == NULL ||
                ca->classifiers[x][y][z][o].c2_vars == NULL)
              ErrorExit(ERROR_NOMEMORY,
                        "CAalloc: could not alloc (%d, %d, %d, %d)",
                        x, y, z, o) ;
            break ;
          }
        }
...
int
CAtrain(CA *ca, VOXEL_LIST *vl, MRI *mri_norm, MRI *mri_aseg, 
        int source_label, int target_label)
{
  ...
  inputs = CAbuildInputs(vl, mri_norm, mri_aseg, target_label, ca->which_inputs, ca->wsize, ca->nscales, 
                         ca->sigmas) ;
  ...
  for (i = 0 ; i < vl->nvox ; i++)
  {
    xi = nint(vl->xd[i]) ; yi = nint(vl->yd[i]) ; zi = nint(vl->zd[i]) ;
    ...
    if (i == Gdiag_no)
      DiagBreak() ;
    if (xi < 0 || yi < 0 || zi < 0 || 
        xi >= ca->width || yi >= ca->height || zi >= ca->depth)
      continue ;
    ...
    IcoFindNClosestVertices(ic0_vertices, ICO0_NVERTICES, nx, ny, nz,
                            3, vertices);
    for (v = 0 ; v < 3 ; v++)
    {
      o = vertices[v] ;
  ...
  for (i = 0 ; i < vl->nvox ; i++)
    free(inputs[i]) ;
  free(classes) ; free(inputs) ;
  MRIfree(&mri_normals) ;
  return(NO_ERROR) ;
}

int 
CAsetSVMparms(CA *ca, double svm_C, double svm_tol, int svm_max_iter)
{
  ca->svm_C = svm_C ;
  ca->tol = svm_tol ;
...
int
CAvoxelToIndex(CA *ca, double x, double y, double z, double *pxd, double *pyd, double *pzd)
{
  ...
  *pxd = V3_X(v2) ; *pyd = V3_Y(v2) ; *pzd = V3_Z(v2) ;
  return(NO_ERROR) ;
}

  
static int
clUpdateStatistics(CA *ca, CLASSIFIER *cl, float *inputs, int output)
{
  int i ;
...
static int
clCompleteTraining(CA *ca, CLASSIFIER *cl)
{
  int i ;
...
MRI *
CAclassifyBorder(CA *ca, MRI *mri_norm, MRI *mri_aseg, MRI *mri_output,
...
float
CAclassifyVoxel(CA *ca, MRI *mri_normals, int x, int y, int z, 
                float *inputs)
{
  ...
  xi = nint(xd) ; yi = nint(yd) ; zi = nint(zd) ;
...
static MRI *
caComputeSurfaceNormals(MRI *mri_aseg, MRI *mri_normals, int label)
{
...
static MATRIX *
compute_ras_basis_vectors(MRI *mri_aseg_orig, MRI *mri_aseg_edit, 
                          int label, 
                          int width, int height, int depth, int pad)
{
  MRI_REGION box1, box2, box ;
  MRI        *mri_aligned, *mri_tmp ;
  MATRIX     *m_xform, *m_trans, *m_tmp, *m_id, *m_inv, *m_targ, *m_src, 
             *m_tmp2, *m_evectors ;
  int        i, j, max_col ;
  ...
  MRIprincipleComponentsRange(mri_aseg_orig, m_evectors, evalues,
                              means, label, label) ;
  ...
  *MATRIX_RELT(m_src, 3, 5) = box.z+box.dz ;
  ...
  MatrixFree(&m_targ) ; MatrixFree(&m_src) ; MatrixFree(&m_inv) ; 
  MatrixFree(&m_tmp) ; MatrixFree(&m_tmp2) ; MatrixFree(&m_evectors) ;
  MatrixFree(&m_trans) ; 
  return(m_xform) ;
}
