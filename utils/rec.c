...
REC *RecRead(char *fname, int iop_neeg, int iop_nmeg) {
  ...
  fp = fopen(fname, "r");
  if (fp == NULL) ErrorReturn(NULL, (ERROR_BADFILE, "RecRead: could not open file %s", fname));

  rec = (REC *)calloc(1, sizeof(REC));
  if (rec == NULL) ErrorExit(ERROR_NOMEMORY, "RecRead: couldn't allocate rec struct");
  ...
  if (!rec->latencies) ErrorExit(ERROR_NOMEMORY, "RecRead(%s): could not allocate latency vector", fname);
  ...
  for (j = 0; j < rec->ntimepts; j++) {
    ...
    for (i = 0; i < rec->neeg_channels; i++) {
      fscanf(fp, "%f", &f);
      if (iop_neeg > 0) *MATRIX_RELT(rec->m_data, i + 1, j + 1) = f;
    }
    for (i = 0; i < rec->nmeg_channels; i++) {
      fscanf(fp, "%f", &f);
      if (iop_nmeg > 0) *MATRIX_RELT(rec->m_data, i + iop_neeg + 1, j + 1) = f;
    }
  }
  fclose(fp);
