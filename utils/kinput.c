...
KINPUT *KinputAlloc(
  ...
  if (nscales > MAX_SCALES) nscales = MAX_SCALES;
...
int KinputFree(KINPUT **pkinput) {
  ...
  *pkinput = NULL;
  ...
  if (kinput->xImage) ImageFree(&kinput->xImage);
  if (kinput->yImage) ImageFree(&kinput->yImage);
#else
  ImageFree(&kinput->xInputs);
  ImageFree(&kinput->yInputs);
  for (scale = 1; scale < kinput->nscales; scale++) ImageFree(&kinput->gImages[scale]);
#endif
  ...
  */
  for (index = scale = 0; scale < kinput->nscales; scale++) {
    for (y = y0-(neg_half ; y <= y0+half_in ; y++)
        ...
        if (kinput->parms.abs_gradient) xval = fabs(xval);
        kinput->inputs[index++] = xval;
        yval = kinputGetPoint(kinput->ypyr, x, y, scale);
        if (kinput->parms.abs_gradient) yval = fabs(yval);
...
static float kinputGetPoint(IMAGEPyramid *pyr, int x, int y, int level) {
  ...
  if (x < 0) x = 0;
  if (y < 0) y = 0;
  if (x >= cols0) x = cols0 - 1;
  if (y >= rows0) y = rows0 - 1;
...
#endif
#if TEST
  if (x == TCOL && y == TROW) fprintf(stderr, "inputs(%2.1f, %2.1f) = %2.4f\n", xc, yc, val);
#endif
  return (val);
}
#endif
