         ...
         char *typestring, int nitems)
{
  ...
  else                        ImaDictionary[nthEntry].nitems = nitems;
...
IMAFILEINFO *imaLoadFileInfo(char *imafile)
{
  ...
  if(ifi->VolDim[0] != ifi->NImageCols) ifi->IsMosaic = 1;
  else                                  ifi->IsMosaic = 0;
...
{
  switch(e->type){
  case IMA_TYPE_SHORT:  fprintf(fp,"%d",e->sval); break;
  case IMA_TYPE_INT:    fprintf(fp,"%d",e->ival); break;
  case IMA_TYPE_LONG:   fprintf(fp,"%ld",e->lval); break;
  case IMA_TYPE_FLOAT:  fprintf(fp,"%f",e->fval); break;
  case IMA_TYPE_DOUBLE: fprintf(fp,"%f",(float)e->dval); break;
  case IMA_TYPE_STRING: fprintf(fp,"%s",e->string); break;
  }
  return(0);
}
...
IMAELEMENT *imaMakeElement(char *descr, int offset, int type, int nitems)
{
  ...
  if(val != NULL){
    if(type != IMA_TYPE_STRING){
      switch(type){
      case IMA_TYPE_SHORT:  e->sval = *( (short *)  val); break;
      case IMA_TYPE_INT:    e->ival = *( (int *)    val); break;
      case IMA_TYPE_LONG:   e->lval = *( (long *)   val); break;
      case IMA_TYPE_FLOAT:  e->fval = *( (float *)  val); break;
      case IMA_TYPE_DOUBLE: e->dval = *( (double *) val); break;
      }
    }
    else { 
