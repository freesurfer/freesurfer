...
void MRISCallocVertices(MRI_CUDA_SURFACE *mrics, MRI_SURFACE *mris)
{
	...
	MRISCcheckCUDAError("MRISallocCudaVertices: cannot allocate device memory !");
...
void MRISCuploadVertices(MRI_CUDA_SURFACE *mrics,MRI_SURFACE *mris)
{
	...
	cudaMemcpy(mrics->d_V,mrics->h_V,mris->nvertices*sizeof(float4),cudaMemcpyHostToDevice);
...
void MRISCdownloadDistances(MRI_CUDA_SURFACE *mrics, MRI_SURFACE *mris)
{
	...
	if(mrics->h_Distances == NULL || mrics->d_Distances == NULL) {
		fprintf(stderr,"MRISCdownloadDistances: Cannot operate on unallocated memory !\n");
	}
	gettimeofday(&tv1,NULL);
	// download the data from the device
	cudaMemcpy(mrics->h_Distances,mrics->d_Distances,mrics->ntotalneighbors*sizeof(float),cudaMemcpyDeviceToHost);
...
void MRISCallocTotalNeighborArray(MRI_CUDA_SURFACE *mrics, MRI_SURFACE *mris)
{
	...
	if(mrics->h_TotalNeighborArray != NULL)
		free(mrics->h_TotalNeighborArray);
	mrics->h_TotalNeighborArray = (unsigned int *)malloc(ntotal*sizeof(unsigned int));
	if(mrics->h_TotalNeighborArray == NULL) {
		fprintf(stderr,"MRISCallocTotalNeighborArray: Cannot allocate host memory for neighbor array !\n");
		exit(-1);
	}
	...
	cudaMalloc((void **)&(mrics->d_TotalNeighborArray),ntotal*sizeof(unsigned int));
	if(mrics->d_TotalNeighborArray == NULL) {
		fprintf(stderr,"MRISCallocTotalNeighborArray: Cannot allocate device memory for neighbor array !\n");
		exit(-1);
	}
	...
	mrics->h_NTotalNeighbors = (unsigned int *)malloc(mrics->nvertices*sizeof(unsigned int));
	if(mrics->h_NTotalNeighbors == NULL) {
		fprintf(stderr,"MRISCallocTotalNeighborArray: Cannot allocate host memory for n-neighbor array !\n");
		exit(-1);
	}
	
	if(mrics->d_NTotalNeighbors != NULL)
		cudaFree(mrics->d_NTotalNeighbors);
	cudaMalloc((void **)&(mrics->d_NTotalNeighbors),mrics->nvertices*sizeof(unsigned int));
	if(mrics->d_NTotalNeighbors == NULL) {
		fprintf(stderr,"MRISCallocTotalNeighborArray: Cannot allocate device memory for n-neighbor array !\n");
		exit(-1);
	}
	...
	mrics->h_NNeighbors = (unsigned int *)malloc(mrics->nvertices*sizeof(unsigned int));
	if(mrics->h_NNeighbors == NULL) {
		fprintf(stderr,"MRISCallocTotalNeighborArray: Cannot allocate host memory for n-neighbor array !\n");
		exit(-1);
	}
	...
	if(mrics->d_NNeighbors != NULL)
		cudaFree(mrics->d_NNeighbors);
	cudaMalloc((void **)&(mrics->d_NNeighbors),mrics->nvertices*sizeof(unsigned int));
	if(mrics->d_NNeighbors == NULL) {
		fprintf(stderr,"MRISCallocTotalNeighborArray: Cannot allocate device memory for n-neighbor array !\n");
		exit(-1);
	}
	...
	if(mrics->h_TotalNeighborOffsets != NULL)
		free(mrics->h_TotalNeighborOffsets);
	mrics->h_TotalNeighborOffsets = (unsigned int *)malloc(mrics->nvertices*sizeof(unsigned int));
	if(mrics->h_TotalNeighborOffsets == NULL) {
		fprintf(stderr,"MRISCallocTotalNeighborArray: Cannot allocate host memory for neighbor-offsets array !\n");
		exit(-1);
	}
	...
	cudaMalloc((void **)&(mrics->d_TotalNeighborOffsets),mrics->nvertices*sizeof(unsigned int));
	if(mrics->d_TotalNeighborOffsets == NULL) {
		fprintf(stderr,"MRISCallocTotalNeighborArray: Cannot allocate device memory for neighbor-offsets array !\n");
		exit(-1);
	}
}
...
void MRISCuploadTotalNeighborArray(MRI_CUDA_SURFACE *mrics, MRI_SURFACE *mris)
{
	...
	for(vno = 0; vno < mris->nvertices; vno++) {
		...
		if(vno == 0)
			...
			mrics->h_TotalNeighborOffsets[vno] = mrics->h_TotalNeighborOffsets[vno-1] + mrics->h_NTotalNeighbors[vno-1];
...
void MRISCallocGradients(MRI_CUDA_SURFACE *mrics, MRI_SURFACE *mris)
{
	...
	if(mrics->h_D == NULL) {
		fprintf(stderr,"MRISCallocGradients: Can't allocate host memory for gradients !\n");
		exit(-1);
	}
	...
	if(mrics->d_D == NULL) {
		fprintf(stderr,"MRISCallocGradients: Can't allocate device memory for gradients !\n");
		exit(-1);
	}
...
void MRISCuploadGradients(MRI_CUDA_SURFACE *mrics, MRI_SURFACE *mris)
{
	...
	// finally transfer the data
	cudaMemcpy(mrics->d_D,mrics->h_D,mris->nvertices*sizeof(float4),cudaMemcpyHostToDevice);
...
void MRISCdownloadGradients(MRI_CUDA_SURFACE *mrics, MRI_SURFACE *mris)
{
	...
	cudaMemcpy(h_D,mrics->d_D,mris->nvertices*sizeof(float4),cudaMemcpyDeviceToHost);
...
__global__ void GradientAverageKernel(float4 *D, float4 *TD,unsigned int *NEIGHBOR, unsigned int *NBOFFSETS, unsigned int *nNeighbors,
																			unsigned int nVertices)
{
  int n,N;
  int offset,soffset;
...
void MRISCaverageGradients(MRI_CUDA_SURFACE *mrisc, MRI_SURFACE *mris, unsigned int niter)
{
    ...
    // run the kernel
    GradientAverageKernel<<<dimGrid, dimBlock>>>(mrisc->d_D, mrisc->d_TD, mrisc->d_TotalNeighborArray, 
																								 mrisc->d_TotalNeighborOffsets, 
																								 mrisc->d_NNeighbors, 
																								 mrisc->nvertices);
		cudaThreadSynchronize();
    // run the update kernel
    updateGradientsKernel<<<dimGrid, dimBlock>>>(mrisc->d_D, mrisc->d_TD, mrisc->nvertices);
		cudaThreadSynchronize();
  }
