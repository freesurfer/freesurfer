...
static int CSAEcomputeGradient(CSAE *csae, AE *ae, VOXEL_LIST *vl, MRI *mri, int start_index, int end_index, int *indices, SAE_INTEGRATION_PARMS *parms) ;
...
// NOTE: input MRI must be type float and scaled to be 0->1 (that is, scale down MRI_UCHAR vols by 255)
double
SAEtrainFromVoxlist(SAE *sae, VOXEL_LIST *vl, MRI **mri_pyramid, SAE_INTEGRATION_PARMS *parms)
{
  double error = 0.0, rms, last_rms, total_rms, last_total_rms, pct_decrease, running_last_rms, G_rms, G_last_rms = 1e10 ;
  ...
  do
  {
	...
	running_pct_dec = 100 * (running_last_rms/visited - total_rms/visited) / (running_last_rms/visited + total_rms/visited) ;
	pct_decrease = 100 * (last_total_rms - total_rms/visited) / (last_total_rms + total_rms/visited) ;
	printf("%d of %d (%2.2f%%): rms = %2.4f (%2.3f%%) (%2.3f%%)\n", ind, vl->nvox, 100*ind/(float)vl->nvox, total_rms/visited, pct_decrease,running_pct_dec) ;
      }
      i = indices[ind]  ; x = vl->xi[i] ;  y = vl->yi[i] ; z = vl->zi[i] ; 
      if (x < sae->whalf || y < sae->whalf || (z < sae->whalf && mri->depth > 1) ||
	  x >= mri->width-sae->whalf || y >= mri->height-sae->whalf || (z >= mri->depth-sae->whalf && mri->depth > 1))
	continue ;
...
static int
CSAEcomputeGradient(CSAE *csae, AE *ae, VOXEL_LIST *vl, MRI *mri, int start_index, int end_index, int *indices, SAE_INTEGRATION_PARMS *parms)
{
  ...
  { 
    i = indices[ind]  ; x = vl->xi[i] ;  y = vl->yi[i] ; z = vl->zi[i] ; 
    if (x < whalf || y < whalf || (z < whalf && mri->depth > 1) || x >= mri->width-whalf || y >= mri->height-whalf || (z >= mri->depth-whalf && mri->depth > 1))
      continue ;
    
    if (x == Gx && y == Gy && z == Gz)
      DiagBreak() ;
    parms->class_label = vl->vsrc[i] ;
  ...
  for (j = 1 ; j <= nhidden ; j++)
  {
    ...
    for (i = 1 ; i <= ninputs ; i++)
      *MATRIX_RELT(ae->m_grad_input_to_hidden, j, i) += (parms->sparsity_trate/ninputs) * (ae->average_act[j] - parms->sparsity_target) ;
  }
  
  // compute weight decay
  for (i = 1 ; i <= ninputs ; i++)
    for (j = 1 ; j <= nhidden ; j++)
      *MATRIX_RELT(ae->m_grad_input_to_hidden, j, i) += parms->weight_decays[parms->layer] * *MATRIX_RELT(ae->m_input_to_hidden, j, i) ;
  for (j = 1 ; j <= nhidden ; j++)
    for (k = 1 ; k <= noutputs ; k++)
      *MATRIX_RELT(ae->m_grad_hidden_to_output, k, j) += parms->weight_decays[parms->layer] * *MATRIX_RELT(ae->m_hidden_to_output, k, j) ;

  return(NO_ERROR) ;
}

static int
AEclearGradients(AE *ae) 
{
  int j ;
...
static double
AEaccumulateGradient(AE *ae, SAE_INTEGRATION_PARMS *parms) 
{
...
static double
aeApplyAccumulatedGradient(AE *ae, SAE_INTEGRATION_PARMS *parms) 
{
...
AEtrain(AE *ae, SAE_INTEGRATION_PARMS *parms) 
{
  ...
  dt = parms->orig_dt ; momentum = parms->momentum ; wt = (1 - parms->class_weight) ;
  ninputs = ae->v_input->rows ; noutputs = ae->v_output->rows ; nhidden = ae->v_hidden->rows ;
  if (ae->v_error == NULL)
  {
    ae->v_error = VectorClone(ae->v_output) ;
...
#pragma omp parallel for firstprivate(noutputs, nhidden, dt) shared(ae) schedule(static,1) 
  ...
  if (parms->integration_type != INTEGRATE_CONJUGATE_GRADIENT)  // don't scale grads by -dt for conjugate gradient
  {
    ...
    {
      ae->average_act[j] = .999 * ae->average_act[j] + (1-.999)*VECTOR_ELT(ae->v_hidden, j+1) ;
      VECTOR_ELT(ae->v_grad_hidden_bias, j+1) += parms->sparsity_trate * (ae->average_act[j] - parms->sparsity_target) ;
    }
...
int
CSAEwrite(CSAE *csae, char *fname) 
{
  return(SAEwrite(csae->sae, fname)) ;
}

CSAE *
CSAEalloc(int type, int nlayers, int *ksizes, int *ngroups, MRI *mri_inputs)
{
  ...
  {
    csae->mri_outputs[layer] = MRIallocSequence(mri_inputs->width, mri_inputs->height, mri_inputs->depth, MRI_FLOAT, ngroups[layer]) ;
    MRIcopyHeader(mri_inputs, csae->mri_outputs[layer]) ;
  }
  return(csae) ;
}
int
CSAEfillInputs(CSAE *csae, MRI *mri_inputs, VECTOR *v_visible, int x0, int y0, int z0, int ksize)
{
...
MRI *
CSAEcreateOutputs(CSAE *csae, MRI *mri_inputs, int first_layer, int last_layer) 
{
  ...
  for (layer = first_layer, mri_layer_inputs = mri_inputs ; layer <= last_layer ; layer++)
  {
	...
	for (z = 0 ; z < mri_inputs->depth ; z++)
	{
	  ...
	  next = ae->next ; ae->next = NULL ; AEactivateLayer(ae, ae->v_input) ; ae->next = next ;
...
AE *
CSAEaddLayer(CSAE *csae, int ksize, int nhidden)
{
  AE  *ae, *last ;
  int  ninputs ;
  float scale ;
...
// NOTE: input MRI must be type float and scaled to be 0->1 (that is, scale down MRI_UCHAR vols by 255)
double
CSAEtrainLayerFromVoxlist(CSAE *csae, int layer, VOXEL_LIST *vl, MRI **mri_pyramid, SAE_INTEGRATION_PARMS *parms)
{
  double total_rms, last_total_rms, pct_decrease, running_last_rms, running_rms ;
...
//  last_total_rms = CSAEcomputeTotalRMS(csae, layer, mri_pyramid) ;
  ...
  do
  {
    ...
    if (iter == 0)
    {
//      min_rms = last_total_rms = CSAEcomputeVoxlistRMS(csae, parms, layer, mri_pyramid, vl, indices, end_index+1, vl->nvox, &always, &never) ;
      min_rms = last_total_rms = CSAEcomputeVoxlistRMS(csae, parms, layer, mri_pyramid, vl, indices, 0, end_index, &always, &never) ;
      printf("%3.3d: rms = %2.4f\n", iter, last_total_rms) ;
    }
    for (i = ind = 0 ; ind <= end_index ; ind += parms->mini_batch_size)
    {
      end_mini_batch = MIN(ind + parms->mini_batch_size-1, vl->nvox-parms->held_out) ;
      running_last_rms = CSAEcomputeVoxlistRMS(csae, parms, layer, mri_pyramid, vl, indices, ind, end_mini_batch, &always, &never) ;
      ...
      running_rms = CSAEcomputeVoxlistRMS(csae, parms, layer, mri_pyramid, vl, indices, ind, end_mini_batch, &always, &never) ;

      if (1 || (ind && !(ind % MAX((vl->nvox/50),2))))
      {
	double running_pct_dec ;
	...
	printf("%d of %d (%2.2f%%): rms = %2.4f (%2.3f%%) (%2.3f%%)\n", end_mini_batch+1, end_index, 100*ind/(float)end_index, total_rms, pct_decrease,running_pct_dec) ;
      }
...
//    total_rms /= visited ; last_total_rms = running_last_rms / visited ;
#endif
//    total_rms = CSAEcomputeVoxlistRMS(csae, parms, layer, mri_pyramid, vl, indices, end_index+1, vl->nvox, &always, &never) ;
    total_rms = CSAEcomputeVoxlistRMS(csae, parms, layer, mri_pyramid, vl, indices, 0, end_index, &always, &never) ;
    if (always || never)
...
MRI *
CSAElayerWeightsToMRI(CSAE *csae, int layer)
{
  ...
  else if (layer >= 1)
  {
	...
	for (x = 0 ; x < ae->ksize ; x++)
	{
	  for (y = 0 ; y < ae->ksize ; y++, v++)
	  {
		...
		val_prev *= *MATRIX_RELT(ae->m_input_to_hidden, h, v) ;
...
MRI *
SAElayerWeightsToMRI(SAE *sae, int layer)
{
  MRI  *mri = NULL, *mri_prev, *mri_counts ;
...
double
CSAEcomputeTotalRMS(CSAE *csae, int layer, MRI **mri)
{
  int     x, y, z, nvox ;
  double  rms, total_rms ;
  AE      *ae ;
...
static double
AEcomputeHiddenRMS(AE *ae, SAE_INTEGRATION_PARMS *parms) 
{
  int    j ;
...
double
CSAEcomputeVoxlistRMS(CSAE *csae, SAE_INTEGRATION_PARMS *parms, int layer, MRI **mri, VOXEL_LIST *vl, int *indices, int start_index, int num_indices, int *always, int *never) 
{
  int     x, y, z, nvox, i, ind, *histo, h, nhidden ;
  double  rms, total_rms, total_class_rms ;
  AE      *ae ;
  ...
  total_class_rms = total_rms = 0.0 ; nvox = 0 ;
  
  for (i = start_index ; i < num_indices ; i++)
  {
    ind = indices[i] ;
    x = vl->xi[ind] ; y = vl->yi[ind] ; z = vl->zi[ind] ;
    parms->class_label = vl->vsrc[i] ;
#if 0
	...
	histo[h]++ ;
    rms = AEcomputeRMS(ae) ;              total_rms += rms ;
    rms = AEcomputeHiddenRMS(ae,parms) ;  total_class_rms += rms ;
    if (!devFinite(rms))
      DiagBreak() ;
    if (rms > 1000)
      DiagBreak() ;
...
CSAEread(char *fname) 
{
  ...
  {
    ae = csae->aes[layer] ;
    csae->mri_outputs[layer] = MRIallocSequence(csae->sae->vg.width, csae->sae->vg.height, csae->sae->vg.depth, MRI_FLOAT, ae->v_hidden->rows) ;
