...
int LPAFreadImageAnswer(LPAF *lpaf, int current) {
  ...
  fp = fopen(fname, "rb");
  if (!fp) ErrorReturn(-1, (ERROR_NO_FILE, "LPAFreadImageAnswer(%d): could not open %s", current, fname));

  ecode = fread_header(fp, &Iheader, fname);
  fclose(fp);
  if (ecode) ErrorReturn(-2, (ERROR_BADFILE, "LPAFreadImageAnswer(%s): could not read header", fname));

  if (Iheader.numparam < Iheader.num_frame) return (0);
...
*/
  ...
  if (lpb->xc < 0 || lpb->xc >= Iheader.cols || lpb->yc < 0 || lpb->xc >= Iheader.rows) return (0);
...
int LPAFresetImageAnswer(LPAF *lpaf, int current) {
  ...
  if (type != HIPS_IMAGE) return (0);

  infp = fopen(fname, "rb");
  if (!infp) ErrorReturn(-1, (ERROR_NO_FILE, "LPAFwriteImageAnswer(%d): could not open %s", current, fname));
  ...
  if (Iheader.numparam == 0) /* must make room for header in image file */
  {
    ...
    for (frame = 0; frame < nframes; frame++) {
      ...
      if (!I) ErrorExit(ERROR_BADFILE, "LPwriteImageAnswer: could not read frame");
...
LP_ANSWER_FILE *LPAFcreate(char *out_fname, int argc, char *argv[]) {
  ...
  if (nfiles <= 0) ErrorReturn(NULL, (ERROR_NO_FILE, "LPAFcreate: no valid files specified"));

  lpaf = (LP_ANSWER_FILE *)calloc(1, sizeof(*lpaf));
  if (!lpaf) ErrorExit(ERROR_NO_MEMORY, "LPAFcreate: allocation failed");
  ...
  if (!lpaf->filelist) ErrorExit(ERROR_NO_MEMORY, "LPAFcreate: allocation failed");
  lpaf->coords = (LP_BOX *)calloc(nfiles, sizeof(LP_BOX));
  if (!lpaf->coords) ErrorExit(ERROR_NO_MEMORY, "LPAFcreate: allocation failed");
  lpaf->last_written = 0;
  ...
  for (i = 0; i < lpaf->nfiles; i++) lpaf->coords[i].fpos = -1L;
  ...
  for (i = 0; i < nentries; i++) {
    switch (type) {
      case LIST_FILE:
        ...
        if (!fp) ErrorReturn(0, (ERROR_NO_FILE, "lpafFillEntries: could not open %s\n", base_name));
...
static void lpafDump(FILE *fp, LP_ANSWER_FILE *lpaf) {
  ...
  for (i = 0; i < lpaf->nfiles; i++) fprintf(fp, "%s\n", lpaf->filelist[i]);
}
...
int LPAFread(LPAF *lpaf, int current) {
  ...
  {
    if (!lpaf->fp || (lpb->fpos < 0)) return (0); /* hasn't been written yet */

    if (fseek(lpaf->fp, lpb->fpos, SEEK_SET) < 0)
      ErrorReturn(-1, (ERROR_BADFILE, "LPAFread could not seek to %ld", lpb->fpos));

    cp = fgetl(line, 299, lpaf->fp);
    if (!cp) ErrorReturn(-1, (ERROR_BADFILE, "LPAFread: could not read line"));
...
static void lpafAllocParms(IMAGE *I) {
  ...
  if (!params) ErrorExit(ERROR_NO_MEMORY, "lpafAllocParms: could not allocate (%d, %d)", nframes, sizeof(*params));
  I->paramdealloc = TRUE;
  ...
  for (frame = 0; frame < nframes; frame++) {
    ...
    for (i = 0; i < NPARAMS; i++) params[frame].val.v_pi[i] = INIT_VAL;
  }
}
