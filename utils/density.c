...
DENSITY *
DensityHistogramEstimate(MRI *mri1,
                         MRI *mri2, 
                         int nbins, 
                         float sigma, 
                         int *valid1, 
                         int *valid2)
{
	...
	if (!pdf)
		ErrorExit
      (ERROR_NOMEMORY, 
       "DensityHistogramEstimate(%d): could not allocate pdf", 
       nbins) ;
	pdf->sigma = sigma ;
	...
	if (sigma > 0)
    {
      ...
      ImageFree(&pdf->Ipdf) ; ImageFree(&Ikernel) ;
      pdf->Ipdf = Ismooth ;
      if ((Gdiag & DIAG_WRITE) && DIAG_VERBOSE_ON)
...
int
DensityWrite(DENSITY *pdf, char *fname)
{
	FILE *fp ;
	int  i, j, n ;
#if 0
	...
	if (fp == NULL)
      ...
      (ERROR_BADPARM, 
       (ERROR_BADPARM, 
        "DensityWrite(%s): could not open file", fname)) ;
...
DensityRead(char *fname)
{
	...
	if (fp == NULL)
      ...
      (NULL, 
       (ERROR_BADPARM, 
        "DensityRead(%s): could not open file", fname)) ;
	...
	if (pdf->Ipdf == NULL)
		ErrorExit
      (ERROR_NOMEMORY, 
       "DensityRead(%d): could not allocate density image", 
       nbins) ;
	fscanf(fp, ":vol1 %f %f\n", &pdf->min_val1, &pdf->max_val1) ;
	fscanf(fp, ":vol2 %f %f\n", &pdf->min_val2, &pdf->max_val2) ;
	fscanf(fp, ":sigma %f\n", &pdf->sigma) ;
...
double
DensityLogLikelihood(DENSITY *pdf, float val1, float val2)
{
	int    bin1, bin2, nbins ;
	double p ;
...
MRI *
DensityLikelihoodImage(MRI *mri1, 
                       MRI *mri2, 
                       MRI *mri_ll, 
                       MATRIX *m, 
                       DENSITY *pdf, 
      ...
      for (y = 0 ; y < mri2->height ; y++)
        {
          ...
          if (inverse)
            ll = 
              -DensityLogLikelihood(pdf, val_src, val_dst) ; /* nissl,block */
          else
            ll = 
              -DensityLogLikelihood(pdf, val_dst, val_src) ; /* nissl,block */
