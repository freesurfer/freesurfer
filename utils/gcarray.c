...
GCARRAY *GCarrayAlloc(MRI *mri_template, int scale, int nvars) {
  ...
  if (!gcarray) ErrorReturn(NULL, (ERROR_NO_MEMORY, "GCarrayAlloc: could not alloc struct"));
  ...
  if (!gcarray->gcs) ErrorExit(ERROR_NO_MEMORY, "GCarrayAlloc: could not allocate gcs");

  for (z = 0; z < depth; z++) {
    gcarray->gcs[z] = (GCLASSIFY ***)calloc(height, sizeof(GCLASSIFY **));
    if (!gcarray->gcs[z]) ErrorExit(ERROR_NO_MEMORY, "GCarrayAlloc: could not allocate gcs[%d]", z);
      ...
      for (y = 0; y < gcarray->height; y++) {
        if (gcarray->gcs[z][y]) {
          for (x = 0; x < gcarray->width; x++) {
            if (gcarray->gcs[z][y][x]) GCfree(&gcarray->gcs[z][y][x]);
          }
...
GCARRAY *GCarrayTrainAll(GCARRAY *gcarray, char *training_file_name, int scale, int ninputs) {
  ...
  if (!fp) ErrorReturn(NULL, (ERROR_NO_FILE, "GCarrayTrainAll(%s): could not open file", training_file_name));
...
int GCarrayTrain(GCARRAY *gcarray, MRI *mri_src, MRI *mri_zscore, MRI *mri_target) {
  ...
  */
        ...
        for (zm = z0; zm <= z1; zm++) {
          for (ym = y0; ym <= y1; ym++) {
            for (xm = x0; xm <= x1; xm++) {
              ...
              yv = nint(yrv);
              zv = nint(zrv);
              if (xv < 0 || xv >= swidth || yv < 0 || yv >= sheight || zv < 0 || zv >= sdepth) continue;
  ...
  for (classno = 0; classno < NCLASSES; classno++) MatrixFree(&m_inputs[classno]);
  ...
  for (z = 0; z < sdepth; z++) {
    for (y = 0; y < sheight; y++) {
      ...
      for (x = 0; x < swidth; x++) {
        ...
        if (pclasses) *pclasses++ = (BUFTYPE)classno;
...
GCARRAY *GCarrayRead(char *fname) {
  ...
  fp = fopen(fname, "r");
  if (!fp) ErrorReturn(NULL, (ERROR_NO_FILE, "GCarrayRead(%s): could not open file", fname));
...
int GCarrayWrite(GCARRAY *gcarray, char *fname) {
  ...
  fp = fopen(fname, "wb");
  if (!fp) ErrorReturn(ERROR_NO_FILE, (ERROR_NO_FILE, "GCarrayWrite(%s): could not open file", fname));
  ...
  for (z = 0; z < depth; z++) {
    for (y = 0; y < height; y++) {
      ...
      for (x = 0; x < width; x++) {
        prob = *pprobs++;
        class = (int)*pclasses++;
        if (prob >= threshold && class == WHITE_MATTER) *pdst++ = 255;
#if 0
