...
// Revision Date  : $Date: 2006/08/04 04:45:33 $
...
//static char vcid[] = "$Id: MRISwaveletsTransform.cpp,v 1.1 2006/08/04 04:45:33 pengyu Exp $";
...
{ 
	...
	if (atoi(argv[3])  == 0) {ANALYSIS=1;}
	else if (atoi(argv[3]) == 1) {ANALYSIS = 1; CURV=1;}
	else if (atoi(argv[3]) == 2) {SYNTHESIS = 1; CURV = 0;}
	else if (atoi(argv[3]) == 3) {SYNTHESIS = 1; CURV = 1;}
	else {	
			...
			for (i=0; i<IcoNVtxsFromOrder(order); i++)
			  {
				...
				array[0][i*3]=area; array[1][i*3]=i*3;
				//if ( area>threshold )
				//{mris_out->vertices[i].origx = mris_in->vertices[i].x ;
				//fprintf(stdout, "%d %f\n", i, area); count++;}
				...
				array[0][i*3+1]=area; array[1][i*3+1]=i*3+1;
				//if ( area>threshold )
				//{mris_out->vertices[i].origy = mris_in->vertices[i].y ;
				//fprintf(stdout, "%d %f\n", i, area);  count++;}
				...
				array[0][i*3+2]=area; array[1][i*3+2]=i*3+2;
				//if ( area>threshold )			 
				//{mris_out->vertices[i].origz = mris_in->vertices[i].z ;
				//fprintf(stdout, "%d %f\n", i, area);  count++;}
			  }
...
center_brain(MRI_SURFACE *mris_src, MRI_SURFACE *mris_dst)
{
  ...
  for (fno = 0 ; fno < mris_src->nfaces ; fno++)
  {
    ...
    z += mris_dst->vertices[face->v[2]].z;
		x = x*face->area/3; y = y*face->area/3; z = z*face->area/3;
		x0 += x/mris_dst->total_area; y0 += y/mris_dst->total_area; z0 += z/mris_dst->total_area;
  ...
  // T is a 4X4 matrix, which takes care of everything: translation, rotation, and scaling
{
  ...
  for (k = 0; k < N; k++) {
    ...
    V1[k].x = v.x; V1[k].y = v.y; V1[k].z = v.z; 
  }
