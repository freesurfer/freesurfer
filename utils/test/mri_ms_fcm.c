...
main(int argc, char *argv[])
{
  ...
  nargs = handle_version_option (argc, argv, "$Id: mri_ms_fcm.c,v 1.3 2006/10/02 17:32:15 nicks Exp $", "$Name:  $");
  ...
  iter = 0; max_change = 10000.0; GKflag = 0;
  while(max_change > tolerance && iter < max_iters){
    ...
    if(GKflag && setsize == 0 && max_change <= tolerance){
      ...
      setsize = 1; max_change = 100*tolerance;
      for(c=0; c < num_classes; c++){
