...
mrisIntegrationEpoch(MRI_SURFACE *mris, INTEGRATION_PARMS *parms,int base_averages)
{
       ...
       n_averages /= 4)
  {
        ...
        (parms->integration_type == INTEGRATE_LM_SEARCH)))
    {
      ...
      fprintf(stderr, "taking momentum steps...\n") ;
...
MRIScorrectTopology(MRI_SURFACE *mris, MRI_SURFACE *mris_corrected)
{
  ...
  */
  ...
  /*  MRISprojectOntoSphere(mris, mris, MRISaverageRadius(mris)) ;*/
  mrisOrientRetessellatedSurface(mris_corrected, dl, vertex_trans) ;
  fprintf(stderr, "final surface representation: %d vertices, %d triangles\n",
          mris->nvertices, mris->nfaces) ;
...
mrisTessellateDefect(MRI_SURFACE *mris, MRI_SURFACE *mris_corrected, 
                     DEFECT *defect, int *vertex_trans)
{
  int    i, j, vlist[10000], n, nvertices, nedges, max_i, nadded, max_added ;
  VERTEX *v, *v2 ;
  EDGE   *et ;
  double  dx, dy, dz, cx, cy, cz, max_len, inflated_len, orig_len ;
  static int dno = 0 ;
  ...
  */
  ...
  for (n = i = 0 ; i < nvertices ; i++)
  {
    ...
    for (j = i+1 ; j < nvertices ; j++, n++)
    {
      ...
      dy = v->origy - v2->origy ;
      dz = v->origz - v2->origz ;
      orig_len = sqrt(dx*dx+dy*dy+dz*dz) ;
  ...
  fprintf(stderr, "removing intersecting border edges...\n") ;
  for (n = 0 ; n < nedges ; n++)
  {
    if (et[n].used == 0)
      continue ;
  ...
  fprintf(stderr, "retessellating planar representation...\n") ;
#endif
  ...
  for (i = 0 ; i < nedges ; i++)
  {
    ...
    if ((et[i].vno1 == 52022 || et[i].vno2 == 52022) &&
        (et[i].vno1 == 53151 || et[i].vno2 == 53151))
      DiagBreak() ;
    if ((et[i].vno1 == 52022 || et[i].vno2 == 52022) &&
  ...
  if (max_len > 10)
  {
    ...
    v1 = &mris_corrected->vertices[et[n].vno1] ;
    v2 = &mris_corrected->vertices[et[n].vno2] ;
    fprintf(stderr, "%d: v %d --> %d, len = %2.3f, nadded = %d of %d\n",
            max_i, et[n].vno1, et[n].vno2, et[n].len, max_added, nadded-1) ;
    fprintf(stderr, "INFLATED:  (%2.1f, %2.1f, %2.1f) --> "
            "(%2.1f, %2.1f, %2.1f), len = %2.2f\n",
            v1->tx, v1->ty, v1->tz,
            v2->tx, v2->ty, v2->tz,
                     ...
                     int *vertex_trans)
{
  ...
  for (i = 0 ; i < defect->nvertices+defect->nchull ; i++)
  {
    ...
    for (n = 0 ; n < v1->vnum ; n++)
    {
      if (vno == Gdiag_no)
        DiagBreak() ;
      if ((vno == 53153 && v1->v[n] == 53158) ||
          (v1->v[n] == 53153 && vno == 53158))
        DiagBreak() ;
      if ((vno == 52024 && v1->v[n] == 53158) ||
          (v1->v[n] == 52024 && vno == 53158))
        DiagBreak() ;
...
mrisOrientRetessellatedSurface(MRI_SURFACE *mris, DEFECT_LIST *dl,int *vtrans)
{
  ...
  for (vno = 0 ; vno < mris->nvertices ; vno++)
  {
    ...
    if (dot < 0)
    {
      fprintf(stderr, "vertex %d has inverted normal!\n", vno) ;
      DiagBreak() ;
    }
                          ...
                          double e0[3], double e1[3])
{
  ...
  len = DOT(e0, e1) ;
  if (FZERO(VLEN(e0)) || FZERO(VLEN(e1)))
  {
  ...
  for (nbad = vno = 0 ; vno < mris->nvertices ; vno++)
  {
    ...
    for (n = 0 ; n < v->vnum ; n++)
    {
      ...
      if (nfaces != 2)
      {
        ...
        nbad++ ;
        fprintf(stderr, "edge %d <--> %d has %d face(s)! ",
                vno, vno2, nfaces) ;
        fprintf(stderr, "(") ;
        for (i = 0 ; i < nfaces ; i++)
          fprintf(stderr, "%d%s", flist[i], i < nfaces-1 ? "," : "") ;
        fprintf(stderr, ")\n") ;
