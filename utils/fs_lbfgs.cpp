...
fs_lbfgs::fs_lbfgs() : fs_lbfgs_subject(), f_(0) { init_parameters(); }

//: Constructor. f is the cost function to be minimized.
// Calls init_parameters
fs_lbfgs::fs_lbfgs(vnl_cost_function &f) : fs_lbfgs_subject(), f_(&f) { init_parameters(); }
...
bool fs_lbfgs::minimize(vnl_vector< double > &x)
{
  ...
  if (we_trace) vcl_cerr << "vnl_lbfgs: ";
  ...
  vcl_cerr << vcl_setw(6) << i << ' ' << vcl_setw(20) << a << ' ' << vcl_setw(20) << b << ' ' << vcl_setw(20) << c \
    ...
    if (check_derivatives_) {
      ...
      if (verbose_) {
        ...
        print_("i", "x", "g", "fdg", "dg");
        print_("-", "-", "-", "---", "--");
        for (int i = 0; i < l; ++i) print_(i, x[i], g[i], fdg[i], g[i] - fdg[i]);
        if (n > limit) {
          vcl_cerr << "   ...\n";
          for (int i = n - limit_tail; i < n; ++i) print_(i, x[i], g[i], fdg[i], g[i] - fdg[i]);
        }
      }
    ...
    if (we_trace) vcl_cerr << iflag << ":" << f_->reported_error(f) << " ";
