...
RFA  *
RFAalloc(RFA_PARMS *parms, int alloc_trees) 
{
  int      x, y, z ;
  RF_NODE  *node ;
  RFA      *rfa ;
  ...
  rfa->width = rfa->vg.width ;  rfa->height = rfa->vg.height ;  rfa->depth = rfa->vg.depth ; 
...
int
RFAtrain(RFA *rfa, MRI *mri_inputs, MRI *mri_labels, TRANSFORM *transform) 
{
  return(NO_ERROR) ;
}

int
RFAcompleteTraining(RFA *rfa, RFA_PARMS *parms)
{
...
int
RFAaddInput(RFA *rfa, MRI *mri_seg, MRI *mri_inputs, TRANSFORM *transform, RFA_PARMS *parms)
{
  int  index ;
...
int
RFAvoxelToNode(const RFA *rfa, double xt, double yt, double zt, double *px, double *py, double *pz)
{
  *px = xt / rfa->spacing ;
  *py = yt / rfa->spacing ;
  *pz = zt / rfa->spacing ;
...
int
RFAnodeToVoxel(const RFA *rfa, double xt, double yt, double zt, double *px, double *py, double *pz)
{
  *px = xt * rfa->spacing ;
  *py = yt * rfa->spacing ;
  *pz = zt * rfa->spacing ;
...
int
RFAsourceVoxelToNode( const RFA *rfa, MRI *mri, TRANSFORM *transform,
...
int
RFAsourceVoxelToAtlas( const RFA *rfa, MRI *mri, TRANSFORM *transform,
		       int xv, int yv, int zv,
...
int
RFAnodeToSourceVoxel(RFA *rfa, MRI *mri, TRANSFORM *transform,
		     ...
		     double *pxv, double *pyv, double *pzv)
{
  ...
  else  // template to source
  {
    ...
    xv = (double)xf ; yv = (double)yf ; zv = (double)zf ;
  }
  *pxv = xv ;
  *pyv = yv ;
  *pzv = zv ;
...
int
RFAupdateTraining(RFA *rfa, RFA_PARMS *parms) 
{
  int       ntraining_sets, ntraining, start, x, y, z, xn, yn, zn, i, *training_classes, *wmsa_permutation ;
  int       xk, yk, zk, xi, yi, zi, tno, label, index, total_correct, total_training, n, nwmsa, 
    max_count, wmsa_count, wmsa_label = WM_hypointensities ;
  ...
  for (i = 0 ; i < ntraining ; i++)  // allow for augmenting with other wmsa examples
  {
    ...
    if (training_data[i] == NULL)
      ErrorExit(ERROR_NOMEMORY, "RFAupdateTraining: could not allocate %d-len feature vector #%d",
		rfa->nfeatures, i) ;
  }
  ...
  {
    mri_in = parms->mri_inputs[i] ; mri_seg = parms->mri_segs[i] ; transform = parms->transforms[i];
  ...
  for (xn = 0 ; xn < rfa->width ; xn++)
  {
	...
	for (zn = 0 ; zn <  rfa->depth; zn++)
	{
		...
		for (zk = 0 ; zk < rfa->spacing ; zk++)
		{
		  ...
		  if (index >= node->nlabels)
		  {
		    if (index >= node->max_labels) 
		    {
		      ...
		      free(node->label_priors) ; free(node->labels) ;
		      node->max_labels += 5 ;
		      node->labels = 
	  ...
	  if (node->nlabels > 1)
	  {
	    ...
	    {
	      extra = (max_count/2 - wmsa_count) ; extra = 0 ; // disabled!!
	    ...
	    node->rf->min_step_size = .5 ; node->rf->max_steps = 256 ;
	    RFtrain(node->rf, parms->feature_fraction, parms->training_fraction, training_classes, training_data,ntraining+extra) ;
  ...
  free(training_data) ; free(training_classes) ; free(wmsa_permutation) ; free(wmsa_data) ;
  return(NO_ERROR) ;
}
int
RFAwrite(RFA *rfa, char *fname)
{
  FILE    *fp ;
  int     xn, yn, zn, n ;
  RF_NODE *node ;
...
RFAread(char *fname)
{
  ...
  for (xn = 0 ; xn < rfa->width ; xn++)
	...
	free(node->labels) ; free(node->label_priors) ;
	node->labels = (unsigned short *)calloc(node->max_labels, sizeof(node->labels[0])) ;
	if (node->labels == NULL)
	  ErrorExit(ERROR_NOMEMORY, "could not reallocate %d node labels @(%d %d %d)",
		    node->max_labels, xn, yn, zn) ;
	node->label_priors = (float *)calloc(node->max_labels,sizeof(node->label_priors[0]));
	if (node->label_priors == NULL)
	  ErrorExit(ERROR_NOMEMORY, "could not reallocate %d node priors @(%d %d %d)",
		    node->max_labels, xn, yn, zn) ;
	for (n = 0 ; n < node->nlabels ; n++)
	{
	  fscanf(fp, "%d ", &label) ;
...
MRI *
RFAlabel(MRI *mri_in, RFA *rfa, MRI *mri_labeled, TRANSFORM *transform)
{
  int       x, y, z, xn, yn, zn ;
  int       label ;
  RF_NODE   *node ;
  ...
  for (x = 0 ; x < mri_in->width ; x++)
  {
      ...
      for (z = 0 ; z <  mri_in->depth; z++)
	{
	  ...
	  {
	    int j ;
	    printf("voxel (%d, %d, %d) -> node (%d, %d, %d), labels = ",
		   x, y, zn, xn, yn, zn) ;
...
int
extract_feature(MRI *mri_in, int wsize, int x, int y, int z, double *feature, int xatlas, int yatlas, int zatlas)
{
