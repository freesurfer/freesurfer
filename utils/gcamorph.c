...
// Revision Date  : $Date: 2007/01/02 15:51:22 $
...
int i, int j, int k, double *pdx, double *pdy, double *pdz)
{
  ...
  *pdz = l_area*V3_Z(v_grad) ;
  ...
  if (i == Gx && j == Gy && k == Gz)
	{
		gcamn = &gcam->nodes[i][j][k] ;
		printf("l_volume: node(%d,%d,%d): uk=%2.4f, total_delta = %2.3f, grad=(%2.3f,%2.3f,%2.3f)\n", 
...
GCAMsmoothConditionalDensities(GCA_MORPH *gcam, float sigma)
{
  ...
  MRIfree(&mri) ; MRIfree(&mri_kernel) ; MRIfree(&mri_smooth) ;
  return(NO_ERROR) ;
}
