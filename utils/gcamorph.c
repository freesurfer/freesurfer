...
#define MAX_ERROR 1000
        ...
        if (x == Gx && y == Gy && z == Gz)
        {
          ...
          ("ll_like: node(%d,%d,%d)-->vox(%2.0f,%2.0f,%2.0F): dI=(%2.1f,%2.1f,%2.1f), "
           ...
           x, y, z, dx, dy, dz, gcamn->x, gcamn->y, gcamn->z, gcamn->dx, gcamn->dy, gcamn->dz,
...
log_integration_parms(FILE *fp, GCA_MORPH_PARMS *parms)
{
  ...
  (fp,
   ...
   parms->relabel, parms->noneg > 0 ? "no" : parms->noneg == 0 ? "yes" : "yes+") ;
  return(NO_ERROR) ;
}
    ...
    for (index = x = 0 ; x < gcam->width ; x++)
      for (y = 0 ; y < gcam->height ; y++)
        for (z = 0 ; z < gcam->depth ; z++, index++)
        {
          ...
          dx[index] = gcamn->dx ; dy[index] = gcamn->dy ; dz[index] = gcamn->dz ;
#if 0
          if (x == Gx && y == Gy && z == Gz)
  ...
  do
  {
    ...
    if (run > 10 && new_neg == 1 && old_neg == 1)
    {
      ...
      printf("neg @ (%d, %d, %d): invalid = %d, status = %d, orig_area = %f, area = %f\n",
	     Gxneg, Gyneg, Gzneg, gcamn->invalid, gcamn->status, gcamn->orig_area, gcamn->area) ;
      DiagBreak() ;
    }
  }
...
int
GCAMreadPositionsFromMRI( GCA_MORPH *gcam, const MRI *mri_pos )
{
  int             xp, yp, zp, xv, yv, zv ;
  double          x, y, z ;
  GCA_MORPH_NODE  *gcamn ;

  if (gcam->width  == mri_pos->width &&
      gcam->height == mri_pos->height &&
...
MRI *
GCAMwritePositionsToMRI( const GCA_MORPH *gcam, MRI *mri_pos)
{
  int    x, y, z;
  double xp, yp, zp ;
  float  xw, yw, zw ;
...
static MRI *
GCAMreplaceWarpAtInvalidNodes(GCAM *gcam, MRI *mri_warp, MRI *mri_warp_dst, MRI *mri_neg_atlas) 
{
  ...
  for (x = 0 ; x < mri_warp->width ; x++)
  {
	    ...
	    (gcam->nodes[x][y][z].invalid > 0))              // and it's invalid area or position
	{
	  if (x == Gx && y == Gy && z == Gz)
	    DiagBreak() ;
	  num = 0 ; memset(means, 0, sizeof(means)) ; ;
	  for (xk = -1 ; xk <= 1 ; xk++)
	  {
	    for (yk = -1 ; yk <= 1 ; yk++)
	    {
	      for (zk = -1 ; zk <= 1 ; zk++)
	      {
		xi = x+xk ;  yi = y+yk ;  zi = z+zk ; 
		if (xi < 0 || yi < 0 || zi < 0 || xi >= mri_warp->width || yi >= mri_warp->height || zi >= mri_warp->depth)
		  continue ;
...
//  MRIcopyVolGeomToMRI(mri_warp, &gcam->image);
  ...
  MRIsetResolution(mri_neg_atlas, gcam->atlas.xsize*gcam->spacing, gcam->atlas.ysize*gcam->spacing, gcam->atlas.zsize*gcam->spacing) ;
...
static int
grow_ventricles(MRI *mri_vent, MRI *mri_inputs, int whalf, int thresh) 
{
  VOXLIST        *vl = NULL ;
...
int
GCAMcomputeVentricleExpansionGradient(GCA_MORPH *gcam, MRI *mri, MRI *mri_vent, int navgs)
{
  int             x, y, z ;
  GCA_MORPH_NODE *gcamn ;
...
static int
most_likely_label(GCA_MORPH *gcam, TRANSFORM *transform, int xp, int yp, int zp, MRI *mri_inputs)
{
  int         n, best_label ;
  float       p, max_p, x, y, z ;
  GCA_PRIOR  *gcap ;

  max_p = -1 ; best_label = -1 ;
