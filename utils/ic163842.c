...
MRI_SURFACE *ic163842_make_surface(int max_vertices, int max_faces)
{
  ...
  for (vno = 0; vno < ICO_NVERTICES; vno++) {
    ...
    if (!v->v) ErrorExit(ERROR_NOMEMORY, "ic163842: could not allocate %dth vertex list.", vno);
    v->vnum = 0;
  }
  ...
  for (vno = 0; vno < ICO_NVERTICES; vno++) {
    ...
    if (!v->f) ErrorExit(ERROR_NO_MEMORY, "ic163842: could not allocate %d faces", v->num);
    v->n = (uchar *)calloc(v->num, sizeof(uchar));
    if (!v->n) ErrorExit(ERROR_NO_MEMORY, "ic163842: could not allocate %d nbrs", v->n);
