...
/*!
  ...
  $Id: fmriutils.c,v 1.47.2.4 2009/01/15 23:35:57 greve Exp $
...
{
  return("$Id: fmriutils.c,v 1.47.2.4 2009/01/15 23:35:57 greve Exp $");
}
  ...
  for (c=0; c < nc; c++)  {
    for (r=0; r < nr; r++)    {
      for (s=0; s< ns; s++)      {
        ...
        GLMfit(mriglm->glm);
	if(mriglm->yffxvar == NULL) GLMtest(mriglm->glm);
	else                        GLMtestFFx(mriglm->glm);
