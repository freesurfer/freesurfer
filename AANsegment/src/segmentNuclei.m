...
%
% This function is heavily based on the prototypes I wrote for the FS
% brainstem parcellation into pons, medulla, midbrain and SCP
%
...
% - atlasDumpFileName: this is actually empty, I only use it as reference to rasterize the atlas
% - atlasDumpFileNameWhole: mask with whole brainstem, VDC, and thalami 
...
% - additionaID: this is just a name for the analysis with the additional file, e.g., T2, or FLAIR


function segmentNuclei(subjectName,subjectDir,resolution,atlasMeshFileName,atlasDumpFileName,atlasDumpFileNameWhole,compressionLUTfileName,K,optimizerType,suffix,FSdir,additionalFile)


% Eugenio November 2017: added option for smoother resampling
MODEL_PAG_INDEPENDENTLY = 1;
DEBUG=0;
...
elseif exist(atlasDumpFileName,'file')==0
    error('Provided targetWorkingRes.mgz does not exist');
elseif exist(atlasDumpFileNameWhole,'file')==0
    error('Provided targetRes.mgz does not exist');
...
% Fix to make sure libraries are not mixed
a = system('export');
if a==0
    FSdir = ['export LD_LIBRARY_PATH && ' FSdir];
else
    a = system('setenv');
    if a==0
        FSdir = ['setenv LD_LIBRARY_PATH && ' FSdir];
    else
       error('neither export nor setenv seem to work'); 
    end
end
...
system(['cp ' atlasDumpFileNameWhole ' ./imageDump.mgz']);

% Target is masked aseg
targetRegFileName=[tempdir '/BS-TH-DE-binaryMask.mgz'];
targetRegFileNameCropped=[tempdir '/BS-TH-DE-binaryMask_autoCropped.mgz'];
system(['cp ' subjectDir '/' subjectName '/mri/aseg.mgz ./']);
ASEG=myMRIread('./aseg.mgz',0,tempdir);
TARGETREG=ASEG;
TARGETREG.vol=double(ASEG.vol==BRAINSTEM | ASEG.vol==DE_label_left | ASEG.vol==DE_label_right | ASEG.vol==TH_label_left | ASEG.vol==TH_label_right);
...
system(['cp imageDump.mgz imageDump.orig.mgz >/dev/null']);
...
if 1==1 % new addition to ensure registration is left-right symmetric.
    % suffix = 'test_PAG_5caseAtlas_symmtest';
    a=myMRIread('imageDump.orig.mgz',0,tempdir);
    a.vol = flipdim(a.vol,2);
    myMRIwrite(a, 'imageDump.orig.flipped.mgz','float',tempdir);
    cmd=[FSdir '/mri_robust_register --mov imageDump.orig.flipped.mgz  --dst ' targetRegFileNameCropped ...
    ' -lta trash.lta --mapmovhdr imageDump_flipped_coregistered.mgz  --sat 50'];
    system(cmd);
    cmd=[FSdir '/mri_robust_register --mov imageDump_flipped_coregistered.mgz  --dst ' targetRegFileNameCropped ...
        ' -lta trash.lta --mapmovhdr imageDump_flipped_coregistered_affine.mgz --affine --sat 50'];
    system(cmd);
    a=myMRIread('imageDump.mgz',0,tempdir);
    b=myMRIread('imageDump_flipped_coregistered_affine.mgz',0,tempdir);
    c=[]; c.vol=a.vol; c.vox2ras0 = a.vox2ras0/2 + b.vox2ras0/2; c.volres = sqrt(sum(c.vox2ras0(1:3,1:3).^2,1));
    myMRIwrite(c,'imageDump.mgz','float',tempdir);
end
    


% Apply to cropped version!
aux=myMRIread('imageDump.mgz',0,tempdir);
aux2=myMRIread('imageDump.orig.mgz',0,tempdir);
T=aux.vox2ras0*inv(aux2.vox2ras0);
aux=myMRIread(atlasDumpFileName,0,tempdir);
aux2=[];
aux2.vol=aux.vol;
aux2.vox2ras0=T*aux.vox2ras0;
aux2.volres=sqrt(sum(aux2.vox2ras0(1:3,1:3).^2));
myMRIwrite(aux2,'imageDump.mgz','float',tempdir);
system(['rm -rf imageDump.orig.mgz']);
...
aux.vol(ASEG.vol==BRAINSTEM | ASEG.vol==7 | ASEG.vol==8 | ASEG.vol==15 | ASEG.vol==28 ...
    | ASEG.vol==46 | ASEG.vol==47 | ASEG.vol==60  | ASEG.vol==10  | ASEG.vol==49) = 255;
...
for FreeSurferLabel = {'Left-Cerebellum-Cortex','Left-Cerebellum-White-Matter','4th-Ventricle','Brain-Stem','DR','LC_R','LC_L','PBC_R','MnR','LDTg_R','LDTg_L','PnO_R','PnO_L','PAG','mRt_R','mRt_L','Left-VentralDC','PTg_R','PTg_L','PBC_L','VTA'}
    sameGaussianParameters{end} = [ sameGaussianParameters{end} FreeSurferLabels( find( strcmp( FreeSurferLabel, cellstr( names ) ) ) ) ];
end
sameGaussianParameters{end+1} = [];
for FreeSurferLabel = {'Left-Cerebral-White-Matter','Left-Lateral-Ventricle','Left-Cerebral-Cortex','Background','Left-choroid-plexus','Left-Hippocampus','Left-Inf-Lat-Vent','Left-Thalamus-Proper','3rd-Ventricle','Left-Putamen','Left-Pallidum','Left-Amygdala','Left-vessel','Left-Caudate','Left-Accumbens-area'}
    sameGaussianParameters{end} = [ sameGaussianParameters{end} FreeSurferLabels( find( strcmp( FreeSurferLabel, cellstr( names ) ) ) ) ];
end
cheatingMeans=[255;1];
...
[~,cropping]=cropLabelVol(L.vol==BRAINSTEM | L.vol==DE_label_left | L.vol==DE_label_right | L.vol==TH_label_left | L.vol==TH_label_right,round(margin/mean(A.volres)));
...
if MODEL_PAG_INDEPENDENTLY > 0
    % Brainstem structures except for PAG
    sameGaussianParameters{end+1} = [];
    for FreeSurferLabel = {'Brain-Stem','DR','LC_R','LC_L','PBC_R','MnR','LDTg_R','LDTg_L','Left-VentralDC','PnO_R','PnO_L','mRt_R','mRt_L','PTg_R','PTg_L','PBC_L','VTA'}
        sameGaussianParameters{end} = [ sameGaussianParameters{end} FreeSurferLabels( find( strcmp( FreeSurferLabel, cellstr( names ) ) ) ) ];
    end
    % PAG
    sameGaussianParameters{end+1} = [];
    for FreeSurferLabel = {'PAG'}
        sameGaussianParameters{end} = [ sameGaussianParameters{end} FreeSurferLabels( find( strcmp( FreeSurferLabel, cellstr( names ) ) ) ) ];
    end
else
    % Brainstem structures except for PAG
    sameGaussianParameters{end+1} = [];
    for FreeSurferLabel = {'Brain-Stem','DR','LC_R','LC_L','PBC_R','MnR','LDTg_R','LDTg_L','Left-VentralDC','PnO_R','PnO_L','mRt_R','PAG','mRt_L','PTg_R','PTg_L','PBC_L','VTA'}
        sameGaussianParameters{end} = [ sameGaussianParameters{end} FreeSurferLabels( find( strcmp( FreeSurferLabel, cellstr( names ) ) ) ) ];
    end
end
% CSF structures
sameGaussianParameters{end+1} = [];
for FreeSurferLabel = {'4th-Ventricle','Left-Lateral-Ventricle','Left-Inf-Lat-Vent','3rd-Ventricle' }
    sameGaussianParameters{end} = [ sameGaussianParameters{end} FreeSurferLabels( find( strcmp( FreeSurferLabel, cellstr( names ) ) ) ) ];
end
% Gray matter structures
sameGaussianParameters{end+1} = [];
for FreeSurferLabel = {'Left-Cerebral-Cortex','Left-Hippocampus','Left-Amygdala'}
    sameGaussianParameters{end} = [ sameGaussianParameters{end} FreeSurferLabels( find( strcmp( FreeSurferLabel, cellstr( names ) ) ) ) ];
end
% Caudate
sameGaussianParameters{end+1} = [];
...
sameGaussianParameters{end+1} = [];
for FreeSurferLabel = {'Left-Cerebral-White-Matter','Left-vessel'}
    sameGaussianParameters{end} = [ sameGaussianParameters{end} FreeSurferLabels( find( strcmp( FreeSurferLabel, cellstr( names ) ) ) ) ];
end
% Background: misc tissue
sameGaussianParameters{end+1} = [];
for FreeSurferLabel = {'Background'}
    sameGaussianParameters{end} = [ sameGaussianParameters{end} FreeSurferLabels( find( strcmp( FreeSurferLabel, cellstr( names ) ) ) ) ];
end
...
for g=1:length(sameGaussianParameters)
    ...
    elseif any(labels==16)  % brainstem + diencephalon
        listMask=[16 28 60];
...
% PAG is special case
if MODEL_PAG_INDEPENDENTLY > 0
    meanHyper(PAGind) = (2/3)*meanHyper(WMind) + (1/3) * meanHyper(GMind);
    nHyper(PAGind) = 20; % low number
end
...
mm1 = tmp1.vol==DE_label_left | tmp1.vol==DE_label_right;
mm2 = tmp1.vol==BRAINSTEM;
[I,J,K]=ind2sub(size(tmp1.vol),find(imdilate(mm1,createSphericalStrel(1)) & imdilate(mm2,createSphericalStrel(1))));
Ic1=mean(I); Jc1=mean(J); Kc1=mean(K);
mm1 = tmp2.vol==DE_label_left | tmp2.vol==DE_label_right;
mm2 = tmp2.vol==BRAINSTEM;
[I,J,K]=ind2sub(size(tmp2.vol),find(imdilate(mm1,createSphericalStrel(1)) & imdilate(mm2,createSphericalStrel(1))));
...
fid=fopen([tempdir '/volumesArousalNuclei.txt'],'w');
strOfInterest={'DR','LC_R','LC_L','PBC_R','PBC_L','VTA','MnR','LDTg_R','LDTg_L','PnO_R','PnO_L','PAG','mRt_R','mRt_L','PTg_R','PTg_L'};
totVol=0;
...
fprintf(fid,'Whole_arousal_network %f\n',totVol);
fclose(fid);
...
if exist('additionalFile','var')>0
    system(['mv discreteLabels.mgz ' subjectDir '/' subjectName '/mri/arousalNetworkLabels.' suffix '.' additionalID '.mgz']);
    system(['mv discreteLabelsResampledT1.mgz ' subjectDir '/' subjectName '/mri/arousalNetworkLabels.' suffix '.' additionalID '.FSvoxelSpace.mgz']);
    system(['mv volumesArousalNuclei.txt ' subjectDir '/' subjectName '/mri/arousalNetworkVolumes.' suffix '.' additionalID '.txt']);
else
    system(['mv discreteLabels.mgz ' subjectDir '/' subjectName '/mri/arousalNetworkLabels.' suffix '.mgz']);
    system(['mv discreteLabelsResampledT1.mgz ' subjectDir '/' subjectName '/mri/arousalNetworkLabels.' suffix '.FSvoxelSpace.mgz']);
    system(['mv volumesArousalNuclei.txt ' subjectDir '/' subjectName '/mri/arousalNetworkVolumes.' suffix '.txt']);
end
