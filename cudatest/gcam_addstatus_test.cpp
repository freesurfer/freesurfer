...
const string inFileDefault = "gcamAddStatusinput";
#ifdef FS_CUDA
const string outFileDefault = "gcamAddStatusoutputGPU";
#else
const string outFileDefault = "gcamAddStatusoutputCPU";
#endif
...
const char* Progname = "gcam_addstatus_test";
...
void ReadCommandLine( int ac, char* av[] ) {
  ...
  try {
    ...
    desc.add_options()
      ...
      ("flag", bpo::value<int>(&statusFlag)->default_value(statusFlagDefault), "Status flag to add" );
      ;
...
int main( int argc, char *argv[] ) {
  ...
  cout << "GCAM Add Status Tester" << endl;
  cout << "======================" << endl << endl;
  ...
  tTotal.Start();
#ifdef FS_CUDA
  gcamAddStatusGPU( gcam, statusFlag );
#else
  GCAMaddStatus( gcam, statusFlag );
#endif
  tTotal.Stop();
