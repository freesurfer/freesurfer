...
main(int argc, char *argv[])
{
  ...
  GCAmaxLikelihoodBorders
    (gca, mri_inputs, mri_labeled, mri_labeled,transform,mle_niter, 5.0);
  if (expand_flag){
...
usage_exit(int code)
{
  ...
  printf("\t-pthresh <float n>           use p threshold n for "
         "adaptive renormalization (default=.7)\n");
  printf("\t-niter <int n>               apply max likelihood for "
         "n iterations (default=2)\n");
  printf("\t-novar                       do not use variance in "
         "classification\n");
  printf("\t-regularize <float n>        regularize variance to be "
         "sigma+nC(noise)\n");
  printf("\t-nohippo                     do not auto-edit hippocampus\n");
  printf("\t-fwm <mri_vol>               use fixed white matter "
         "segmentation from wm\n");
  printf("\t-mri <mri_vol>               write most likely MR volume "
         "to mri_vol\n");
  printf("\t-heq <mri_vol>               use histogram equalization "
         "from mri_vol\n");
  printf("\t-renorm <mri_vol>            renormalize using predicted "
         "intensity values in mri_vol\n");
  printf("\t-flash                       use FLASH forward model "
         "to predict intensity values\n");
  printf("\t-flash_params <filename>     use FLASH forward model "
         "and tissue params in filename to predict\n");
  printf("\t-renormalize <wsize> <iter>  renorm class means <iter> "
         "times after initial label with window of <wsize>\n");
  printf("\t-r <mri_vol>                 set input volume\n");
  printf("\t-h                           use GCA to histogram "
         "normalize input image\n");
  printf("\t-a <int n>                   mean filter n time to "
         "conditional densities\n");
  printf("\t-w <int n> <filename>        write snapshots of "
         "gibbs process every n times to filename\n");
  printf("\t-m <mri_vol>                 use mri_vol to mask "
         "final labeling\n");
  printf("\t-e <int n>                   expand\n");
  printf("\t-n <int n>                   set max iterations to "
         "n (default=200)\n");
  printf("\t-f <int f> <float t>         filter labeled volume "
                                  ...
                                  GCA *gca, TRANSFORM *transform, GCA *gca_all)
{
  ...
  */
  ...
  MRIfree(&mri_tmp) ;
  MRIfree(&mri_probs) ; MRIfree(&mri_tmp_labels) ;
  for (x = 0 ; x < width ; x++) free(added[x]) ;
  free(added) ;
