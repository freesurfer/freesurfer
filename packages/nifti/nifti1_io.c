...
void nifti_disp_lib_hist(void)
{
  ...
  for (c = 0; c < len; c++) fputs(gni_history[c], stdout);
}
...
static void update_nifti_image_for_brick_list(nifti_image *nim, int nbricks)
{
  ...
  for (nim->nvox = 1, ndim = 1; ndim <= nim->dim[0]; ndim++) nim->nvox *= nim->dim[ndim];
...
int nifti_image_load_bricks(nifti_image *nim, int nbricks, const int *blist, nifti_brick_list *NBL)
{
  ...
  if (blist && !valid_nifti_brick_list(nim, nbricks, blist, g_opts.debug > 0)) return -1;

  /* for efficiency, let's read the file in order */
  if (blist && nifti_copynsort(nbricks, blist, &slist, &sindex) != 0) return -1;

  /* open the file and position the FILE pointer */
  fp = nifti_image_load_prep(nim);
  if (!fp) {
    if (g_opts.debug > 0) fprintf(stderr, "** nifti_image_load_bricks, failed load_prep\n");
    if (blist) {
      free(slist);
      free(sindex);
    }
...
int valid_nifti_brick_list(nifti_image *nim, int nbricks, const int *blist, int disp_error)
{
  ...
  if (!nim) {
    if (disp_error || g_opts.debug > 0) fprintf(stderr, "** valid_nifti_brick_list: missing nifti image\n");
    return 0;
  }

  if (nbricks <= 0 || !blist) {
    if (disp_error || g_opts.debug > 1) fprintf(stderr, "** valid_nifti_brick_list: no brick list to check\n");
    return 0;
  }
...
mat33 nifti_mat33_mul(mat33 A, mat33 B) /* multiply 2 3x3 matrices */
{
  ...
  int i, j;
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++) C.m[i][j] = A.m[i][0] * B.m[0][j] + A.m[i][1] * B.m[1][j] + A.m[i][2] * B.m[2][j];
  return C;
}
...
int nifti_is_complete_filename(const char *fname)
{
  ...
  if (fname == NULL || *fname == '\0') {
    if (g_opts.debug > 1) fprintf(stderr, "-- empty filename in nifti_validfilename()\n");
    return 0;
  }

  ext = nifti_find_file_extension(fname);
  if (ext == NULL) { /*Invalid extension given */
    if (g_opts.debug > 0) fprintf(stderr, "-- no nifti valid extension for filename '%s'\n", fname);
    return 0;
  }

  if (ext && ext == fname) { /* then no filename prefix */
    if (g_opts.debug > 0) fprintf(stderr, "-- no prefix for filename '%s'\n", fname);
    return 0;
  }
  return 1;
}
...
char *nifti_find_file_extension(const char *name)
{
  ...
  if (g_opts.debug > 1) fprintf(stderr, "** find_file_ext: failed for name '%s'\n", name);
...
char *nifti_makehdrname(const char *prefix, int nifti_type, int check, int comp)
{
  ...
  if ((ext = nifti_find_file_extension(iname)) != NULL) {
    if (strncmp(ext, ".img", 4) == 0) memcpy(ext, ".hdr", 4); /* then convert img name to hdr */
  }
...
char *nifti_makeimgname(const char *prefix, int nifti_type, int check, int comp)
{
  ...
  /* use any valid extension */
  if ((ext = nifti_find_file_extension(iname)) != NULL) {
    if (strncmp(ext, ".hdr", 4) == 0) memcpy(ext, ".img", 4); /* then convert hdr name to img */
  }
...
int nifti_set_filenames(nifti_image *nim, const char *prefix, int check, int set_byte_order)
{
  ...
  if (g_opts.debug > 1) fprintf(stderr, "+d modifying output filenames using prefix %s\n", prefix);
  ...
  if (g_opts.debug > 2) fprintf(stderr, "+d have new filenames %s and %s\n", nim->fname, nim->iname);
...
int nifti_type_and_names_match(nifti_image *nim, int show_warn)
{
  ...
  if (!is_valid_nifti_type(nim->nifti_type)) {
    if (show_warn) fprintf(stderr, "** %s: bad nifti_type %d\n", func, nim->nifti_type);
    errs++;
  }
  ...
  if (!ext_h) {
    if (show_warn) fprintf(stderr, "-d missing NIFTI extension in header filename, %s\n", nim->fname);
    errs++;
  }
  if (!ext_i) {
    if (show_warn) fprintf(stderr, "-d missing NIFTI extension in image filename, %s\n", nim->iname);
    errs++;
  }
  ...
  if (nim->nifti_type == NIFTI_FTYPE_NIFTI1_1) { /* .nii */
    if (strncmp(ext_h, ".nii", 4)) {
      if (show_warn) fprintf(stderr, "-d NIFTI_FTYPE 1, but no .nii extension in header filename, %s\n", nim->fname);
      errs++;
    }
    if (strncmp(ext_i, ".nii", 4)) {
      if (show_warn) fprintf(stderr, "-d NIFTI_FTYPE 1, but no .nii extension in image filename, %s\n", nim->iname);
      errs++;
    }
    ...
    if (strncmp(ext_h, ".hdr", 4) != 0) {
      if (show_warn) fprintf(stderr, "-d no '.hdr' extension, but NIFTI type is %d, %s\n", nim->nifti_type, nim->fname);
      errs++;
    }
    if (strncmp(ext_i, ".img", 4) != 0) {
      if (show_warn) fprintf(stderr, "-d no '.img' extension, but NIFTI type is %d, %s\n", nim->nifti_type, nim->iname);
      errs++;
    }
  }
...
int nifti_set_type_from_names(nifti_image *nim)
{
  ...
  if (g_opts.debug > 2) fprintf(stderr, "-d verify nifti_type from filenames: %d", nim->nifti_type);
...
int is_nifti_file(const char *hname)
{
  ...
  if (tmpname == NULL) {
    if (g_opts.debug > 0) fprintf(stderr, "** no header file found for '%s'\n", hname);
    return -1;
  }
...
nifti_image *nifti_convert_nhdr2nim(struct nifti_1_header nhdr, const char *fname)
{
  ...
  if (nhdr.datatype == DT_BINARY || nhdr.datatype == DT_UNKNOWN) ERREX("bad datatype");
  ...
  for (ii = 1, nim->nvox = 1; ii <= nhdr.dim[0]; ii++) nim->nvox *= nhdr.dim[ii];
  ...
  if (!is_nifti || nhdr.qform_code <= 0) {
    ...
    if (g_opts.debug > 1) nifti_disp_matrix_orient("-d qform orientations:\n", nim->qto_xyz);
  }
  ...
  if (!is_nifti || nhdr.sform_code <= 0) {
    ...
    if (g_opts.debug > 1) nifti_disp_matrix_orient("-d sform orientations:\n", nim->sto_xyz);
  }
...
nifti_1_header *nifti_read_header(const char *hname, int *swapped, int check)
{
  ...
  if (hfile == NULL) {
    if (g_opts.debug > 0) LNI_FERR(fname, "failed to find header file for", hname);
    return NULL;
  }
  ...
  if (has_ascii_header(fp) == 1) {
    znzclose(fp);
    if (g_opts.debug > 0) LNI_FERR(fname, "ASCII header type not supported", hname);
    return NULL;
  }
...
int nifti_hdr_looks_good(const nifti_1_header *hdr)
{
  ...
  for (c = 1; c <= hdr->dim[0] && c <= 7; c++)
    if (hdr->dim[c] <= 0) {
      if (g_opts.debug > 0) fprintf(stderr, "** bad nhdr field: dim[%d] = %d\n", c, hdr->dim[c]);
      errs++;
    }
  ...
  if (hdr->datatype == DT_BINARY || hdr->datatype == DT_UNKNOWN) {
    if (g_opts.debug > 0) fprintf(stderr, "** bad nhdr field: datatype = %d\n", hdr->datatype);
    errs++;
  }
...
nifti_image *nifti_image_read(const char *hname, int read_data)
{
  ...
  if (nim == NULL) {
    ...
    if (g_opts.debug > 0) LNI_FERR(fname, "cannot create nifti image from header", hfile);
...
nifti_image *nifti_read_ascii_image(znzFile fp, char *fname, int flen, int read_data)
{
  ...
  if (g_opts.debug > 1) fprintf(stderr, "-d %s: have ASCII NIFTI file of size %d\n", fname, slen);
  ...
  if (read_data && rv != 0) {
    if (g_opts.debug > 1) fprintf(stderr, "-d failed image_load, free nifti image struct\n");
    free(nim);
    return NULL;
  }
...
static int nifti_read_extensions(nifti_image *nim, znzFile fp, int remain)
{
  ...
  if (!nim || znz_isnull(fp)) {
    if (g_opts.debug > 0) fprintf(stderr, "** nifti_read_extensions: bad inputs (%p,%p)\n", (void *)nim, (void *)fp);
    return -1;
  }
  ...
  if (count < 4) {
    if (g_opts.debug > 1) fprintf(stderr, "-d file '%s' is too short for an extender\n", nim->fname);
    return 0;
  }
  ...
  remain -= 4;
  if (g_opts.debug > 2) fprintf(stderr, "-d found valid 4-byte extender, remain = %d\n", remain);
  ...
  while (nifti_read_next_extension(&extn, nim, remain, fp) > 0) {
    if (nifti_add_exten_to_list(&extn, &Elist, count + 1) < 0) {
      if (g_opts.debug > 0) fprintf(stderr, "** failed adding ext %d to list\n", count);
      return -1;
    }
...
static int nifti_add_exten_to_list(nifti1_extension *new_ext, nifti1_extension **list, int new_length)
{
  ...
  *list = (nifti1_extension *)malloc(new_length * sizeof(nifti1_extension));
  ...
  if (g_opts.debug > 2) fprintf(stderr, "+d allocated and appended extension #%d to list\n", new_length);
...
static int nifti_read_next_extension(nifti1_extension *nex, nifti_image *nim, int remain, znzFile fp)
{
  ...
  if (remain < 16) {
    if (g_opts.debug > 2) fprintf(stderr, "-d only %d bytes remain, so no extension\n", remain);
    return 0;
  }
  ...
  if (count != 2) {
    if (g_opts.debug > 2) fprintf(stderr, "-d current extension read failed\n");
    znzseek(fp, -4 * count, SEEK_CUR); /* back up past any read */
    return 0;                          /* no extension, no error condition */
  }

  if (swap) {
    if (g_opts.debug > 2) fprintf(stderr, "-d pre-swap exts: code %d, size %d\n", code, size);

    nifti_swap_4bytes(1, &size);
    nifti_swap_4bytes(1, &code);
  }

  if (g_opts.debug > 2) fprintf(stderr, "-d potential extension: code %d, size %d\n", code, size);
  ...
  if (count < size) {
    if (g_opts.debug > 0) fprintf(stderr, "-d read only %d (of %d) bytes for extension\n", count, size);
    free(nex->edata);
    nex->edata = NULL;
    return -1;
  }
  ...
  /* success! */
  if (g_opts.debug > 2) fprintf(stderr, "+d successfully read extension, code %d, size %d\n", nex->ecode, nex->esize);
...
int valid_nifti_extensions(const nifti_image *nim)
{
  ...
  for (c = 0; c < nim->num_ext; c++) {
    if (!nifti_is_valid_ecode(ext->ecode)) {
      if (g_opts.debug > 1) fprintf(stderr, "-d ext %d, invalid code %d\n", c, ext->ecode);
      errs++;
    }

    if (ext->esize <= 0) {
      if (g_opts.debug > 1) fprintf(stderr, "-d ext %d, bad size = %d\n", c, ext->esize);
      errs++;
    }
    else if (ext->esize & 0xf) {
      if (g_opts.debug > 1) fprintf(stderr, "-d ext %d, size %d not multiple of 16\n", c, ext->esize);
      errs++;
    }
  ...
  if (errs > 0) {
    if (g_opts.debug > 0) fprintf(stderr, "-d had %d extension errors, none will be written\n", errs);
    return 0;
  }
...
static int nifti_check_extension(nifti_image *nim, int size, int code, int rem)
{
  ...
  if (!nifti_is_valid_ecode(code)) {
    if (g_opts.debug > 2) fprintf(stderr, "-d invalid extension code %d\n", code);
    return 0;
  }

  if (size < 16) {
    if (g_opts.debug > 2) fprintf(stderr, "-d ext size %d, no extension\n", size);
    return 0;
  }

  if (size > rem) {
    if (g_opts.debug > 2) fprintf(stderr, "-d ext size %d, space %d, no extension\n", size, rem);
    return 0;
  }

  if (size & 0xf) {
    if (g_opts.debug > 2) fprintf(stderr, "-d nifti extension size %d not multiple of 16\n", size);
    return 0;
  }

  if (nim->nifti_type == NIFTI_FTYPE_ASCII && size > LNI_MAX_NIA_EXT_LEN) {
    if (g_opts.debug > 2) fprintf(stderr, "-d NVE, bad nifti_type 3 size %d\n", size);
    return 0;
  }
...
static znzFile nifti_image_load_prep(nifti_image *nim)
{
  ...
  if (nim->iname_offset < 0) {
    if (nifti_is_gzfile(nim->iname)) {
      if (g_opts.debug > 0) LNI_FERR(fname, "negative offset for compressed file", nim->iname);
      znzclose(fp);
      return NULL;
    }
...
int nifti_image_load(nifti_image *nim)
{
  ...
  if (fp == NULL) {
    if (g_opts.debug > 0) fprintf(stderr, "** nifti_image_load, failed load_prep\n");
    return -1;
  }
  ...
  if (nim->data == NULL) {
    ...
    if (nim->data == NULL) {
      if (g_opts.debug > 0) fprintf(stderr, "** failed to alloc %d bytes for image data\n", (int)ntot);
      znzclose(fp);
      return -1;
    }
  }
...
size_t nifti_read_buffer(znzFile fp, void *dataptr, size_t ntot, nifti_image *nim)
{
  ...
  if (dataptr == NULL) {
    if (g_opts.debug > 0) fprintf(stderr, "** ERROR: nifti_read_buffer: NULL dataptr\n");
    return -1;
  }
  ...
  if (g_opts.debug > 2) fprintf(stderr, "+d nifti_read_buffer: read %u bytes\n", (unsigned)ii);
...
#ifdef isfinite
  {
    ...
    if (g_opts.debug > 1) fprintf(stderr, "+d in image, %d bad floats were set to 0\n", fix_count);
  }
#endif
...
int nifti_free_extensions(nifti_image *nim)
{
  ...
  if (g_opts.debug > 2) fprintf(stderr, "+d free'd %d extension(s)\n", nim->num_ext);
...
int nifti_write_all_data(znzFile fp, nifti_image *nim, const nifti_brick_list *NBL)
{
  ...
  if (!NBL) { /* just write one buffer and get out of here */
    ...
    if (g_opts.debug > 1) fprintf(stderr, "+d wrote single image of %u bytes\n", (unsigned)ss);
  }
  else {
...
static int nifti_write_extensions(znzFile fp, nifti_image *nim)
{
  ...
  if (znz_isnull(fp) || !nim || nim->num_ext < 0) {
    if (g_opts.debug > 0) fprintf(stderr, "** nifti_write_extensions, bad params\n");
    return -1;
  }
  ...
  if (g_opts.debug > 1) fprintf(stderr, "+d wrote out %d extension(s)\n", nim->num_ext);
...
nifti_1_header *nifti_make_new_header(const int arg_dims[], int arg_dtype)
{
  ...
  if (g_opts.debug > 1) fprintf(stderr, "+d nifti_make_new_header, dim[0] = %d, datatype = %d\n", dim[0], dtype);
...
nifti_image *nifti_make_new_nim(const int dims[], int datatype, int data_fill)
{
  ...
  if (g_opts.debug > 1) fprintf(stderr, "+d nifti_make_new_nim, data_fill = %d\n", data_fill);
...
int nifti_copy_extensions(nifti_image *nim_dest, const nifti_image *nim_src)
{
  ...
  if (g_opts.debug > 1) fprintf(stderr, "+d duplicating %d extension(s)\n", nim_src->num_ext);
  ...
  for (c = 0; c < nim_src->num_ext; c++) {
    ...
    if (g_opts.debug > 2) fprintf(stderr, "+d dup'ing ext #%d of size %d (from size %d)\n", c, size, old_size);
  ...
  switch (nim->nifti_type) {
    ...
    case NIFTI_FTYPE_NIFTI1_1:
      ...
      if (nim->iname_offset != offset) {
        if (g_opts.debug > 1) fprintf(stderr, "+d changing offset from %d to %d\n", nim->iname_offset, offset);
        nim->iname_offset = offset;
      }
      break;
    ...
    nifti_image *nim, int write_opts, const char *opts, znzFile imgfile, const nifti_brick_list *NBL)
{
  ...
  if (g_opts.debug > 1) {
    fprintf(stderr, "-d writing nifti file '%s'...\n", nim->fname);
    if (g_opts.debug > 2) fprintf(stderr, "-d nifti type %d, offset %d\n", nim->nifti_type, nim->iname_offset);
  }
  ...
  if (!znz_isnull(imgfile) && nim->nifti_type == NIFTI_FTYPE_NIFTI1_1) {
    ...
    if (g_opts.debug > 2) fprintf(stderr, "+d opening output file %s [%s]\n", nim->fname, opts);
    fp = znzopen(nim->fname, opts, nifti_is_gzfile(nim->fname));
    if (znz_isnull(fp)) {
      LNI_FERR(func, "cannot open output file", nim->fname);
      return fp;
  ...
  if (nim->nifti_type != NIFTI_FTYPE_ANALYZE) (void)nifti_write_extensions(fp, nim);
  ...
  if (nim->nifti_type != NIFTI_FTYPE_NIFTI1_1) { /* get a new file pointer */
    ...
    if (!znz_isnull(imgfile)) {
      ...
      if (g_opts.debug > 2) fprintf(stderr, "+d opening img file '%s'\n", nim->iname);
      fp = znzopen(nim->iname, opts, nifti_is_gzfile(nim->iname));
      if (znz_isnull(fp)) ERREX("cannot open image file");
    }
  }
...
znzFile nifti_write_ascii_image(
    nifti_image *nim, const nifti_brick_list *NBL, const char *opts, int write_data, int leave_open)
{
  znzFile fp;
  char *hstr;
...
static int unescape_string(char *str)
{
  ...
  for (ii = jj = nn = 0; ii < ll; ii++, jj++) { /* scan at ii; results go in at jj */
    ...
    if (str[ii] == '&') { /* start of escape? */
      ...
      else if (ii + 3 < ll && str[ii + 1] == '#' && isdigit(str[ii + 2])) { /* &#dec; */
      ...
      else if (ii + 4 < ll && str[ii + 1] == '#' && str[ii + 2] == 'x' && isxdigit(str[ii + 3])) { /* &#hex; */
...
char *nifti_image_to_ascii(const nifti_image *nim)
{
  ...
  if (nim->toffset != 0.0) sprintf(buf + strlen(buf), "  toffset = '%g'\n", nim->toffset);
  ...
  if (nim->freq_dim > 0) sprintf(buf + strlen(buf), "  freq_dim = '%d'\n", nim->freq_dim);
  if (nim->phase_dim > 0) sprintf(buf + strlen(buf), "  phase_dim = '%d'\n", nim->phase_dim);
  if (nim->slice_dim > 0) sprintf(buf + strlen(buf), "  slice_dim = '%d'\n", nim->slice_dim);
  if (nim->slice_code > 0)
    sprintf(buf + strlen(buf),
            "  slice_code = '%d'\n"
            "  slice_code_name = '%s'\n",
  ...
  if (nim->slice_start >= 0 && nim->slice_end > nim->slice_start)
    sprintf(buf + strlen(buf),
            ...
            nim->slice_start,
            nim->slice_end);
  if (nim->slice_duration != 0.0) sprintf(buf + strlen(buf), "  slice_duration = '%g'\n", nim->slice_duration);
...
#define QQNUM(n1, n2) \
  if (strcmp(lhs, #n1) == 0) nim->n2 = strtod(rhs, NULL)
...
int nifti_nim_has_valid_dims(nifti_image *nim, int complain)
{
  ...
  if (nim->dim[0] <= 0 || nim->dim[0] > 7) {
    errs++;
    if (complain) fprintf(stderr, "** NVd: dim[0] (%d) out of range [1,7]\n", nim->dim[0]);
    return 0;
  }
  ...
  if (g_opts.debug > 2) fprintf(stderr, "-d nim_has_valid_dims check, errs = %d\n", errs);
...
int nifti_read_collapsed_image(nifti_image *nim, const int dims[8], void **data)
{
  ...
  if (g_opts.debug > 1) fprintf(stderr, "+d read %d bytes of collapsed image from %s\n", bytes, nim->fname);
...
static int rci_alloc_mem(void **data, int prods[8], int nprods, int nbyper)
{
  ...
  for (index = 0, size = 1; index < nprods; index++) size *= prods[index];
...
int *nifti_get_intlist(int nvals, const char *str)
{
  ...
  if (g_opts.debug > 1) fprintf(stderr, "-d making int_list (vals = %d) from '%s'\n", nvals, str);
