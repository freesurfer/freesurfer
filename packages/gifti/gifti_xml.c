...
static int apply_da_list_order(gxml_data *xd, const int *orig, int len)
{
  ...
  for (newc = 0; newc < len; newc++) {
    ...
    for (oldc = 0; oldc < nDA; oldc++)
      if (xd->da_list[oldc] == orig[newc]) break;
...
int gxml_write_image(gifti_image *gim, const char *fname, int write_data)
{
  ...
  if (GXD.verb > 1) {
    ...
    if (write_data) fprintf(stderr, " (%d DA elements = %lld MB)", gim->numDA, gifti_gim_DA_size(gim, 1));
    fputc('\n', stderr);
  }
  ...
  if (xd->update_ok) {
    if (xd->verb > 2) fprintf(stderr, "++ setting GIFTI MD: gifticlib-version to %s\n", gifticlib_version());
...
static int init_gxml_data(gxml_data *dp, int doall, const int *dalist, int len)
{
  ...
  /* maybe show the user */
  if (dp->verb > 2) disp_gxml_data("-- user opts: ", dp, dp->verb > 3);
...
static int short_sorted_da_list(gxml_data *dp, const int *dalist, int len)
{
  ...
  if (dp->verb > 1) {
    ...
    for (c = 0; c < len; c++) fprintf(stderr, " %d", dalist[c]);
    fputc('\n', stderr);
    fprintf(stderr, "++ unique, sorted da_list:");
    for (c = 0; c < dp->da_len; c++) fprintf(stderr, " %d", dp->da_list[c]);
    fputc('\n', stderr);
  }
...
static int ename2type(const char *name)
{
  ...
  for (etype = GXML_ETYPE_LAST; etype > GXML_ETYPE_INVALID; etype--)
    if (!strcmp(name, enames[etype])) break;
  return etype;
}
...
static int epush(gxml_data *xd, int etype, const char *ename, const char **attr)
{
  ...
  if (xd->skip) {
    if (xd->verb > 2) fprintf(stderr, "-- skip=%d, depth=%d, skipping element '%s'\n", xd->skip, xd->depth, ename);
    return 0;
  }

  /* determine whether we should enter a skip block */
  if (etype == GXML_ETYPE_INVALID) {
    if (xd->verb > 0) fprintf(stderr, "** pushed invalid element, '%s', skip depth %d\n", ename, xd->depth);
    xd->skip = xd->depth;
    return 1;
  }
...
static int push_darray(gxml_data *xd, const char **attr)
{
  ...
  if (da->nvals > 0 && da->nbyper > 0) update_xml_buf_size(xd, da->nvals * da->nbyper);
...
static int pop_darray(gxml_data *xd)
{
  ...
  if (da->ext_fname && *da->ext_fname) (void)gifti_read_extern_DA_data(da); /* nothing to do on failure */
  ...
  if (da->data && da->encoding != GIFTI_ENCODING_ASCII) {
    ...
    if (gifti_check_swap(da->data, da->endian, nvals, swapsize)) xd->gim->swapped = 1; /* flag that it happened */
  }
...
static int push_data(gxml_data *xd)
{
  ...
  if (!xd->dstore) {
    if (xd->verb > 3) fprintf(stderr, "-- skipping data[%d]\n", xd->gim->numDA - 1);
    xd->skip = xd->depth;
    return 1;
  }

  if (update_partial_buffer(&xd->ddata, &xd->dlen, da->nbyper * da->nvals, 0)) return 1;
  ...
  if (da->encoding == GIFTI_ENCODING_B64GZ) {
    ...
    if (xd->verb > 2) fprintf(stderr, "++ creating extra zdata for zlib extraction\n");
    if (update_partial_buffer(&xd->zdata, &xd->zlen, zsize, 1)) return 1;
  }
...
static int epop(gxml_data *xd, int etype, const char *ename)
{
  ...
  if (xd->skip == xd->depth) { /* just completed skip element */
    if (xd->verb > 2) fprintf(stderr, "-- popping skip element '%s' at depth %d\n", ename, xd->depth);
    ...
    switch (etype) {
      ...
      case GXML_ETYPE_GIFTI:
        ...
        if (xd->verb > 2) gifti_disp_gifti_image("pop:", xd->gim, xd->verb > 4);

        if (xd->verb > 1) { /* check flags */
          if (xd->gim->swapped) fprintf(stderr, "++ data was byte-swapped\n");
...
static void XMLCALL cb_char(void *udata, const char *cdata, int length)
{
  ...
  switch (parent) {
    ...
    case GXML_ETYPE_DATASPACE:
    case GXML_ETYPE_XFORMSPACE:
      if (xd->verb > 4) fprintf(stderr, "++ append cdata, parent %s\n", enames[parent]);
...
static int append_to_cdata(gxml_data *xd, const char *cdata, int len)
{
  ...
  if (xd->verb > 4) fprintf(stderr, "++ a2cdata, len %d, clen %d, data '%.*s'\n", len, xd->clen, len, cdata);
...
static int append_to_data_b64(gxml_data *xd, char *dest, long long tot_bytes, const char *cdata, int cdlen)
{
  ...
  if (xd->verb > 4) fprintf(stderr, "++ appending %d base64 binary bytes to data\n", cdlen);
  ...
  while (rem_bytes_in > 0) {
    ...
    if (xd->verb > 5) fprintf(stderr, "-- %lld bytes left at offset %lld\n", rem_bytes_out, xd->dind);
    ...
    if (xd->doff >= xd->dlen - 1) {
      if (xd->verb) fprintf(stderr, "** A2Db64: failed to process buffer\n");
    ...
    if (xd->doff > 0) {
      ...
      if (xd->verb > 6) fprintf(stderr, "   bytes are '%.*s'\n", xd->doff, (char *)xd->ddata);
    }
...
static int copy_b64_data(gxml_data *xd, const char *src, char *dest, int src_len, int *dest_len)
{
  ...
  switch (xd->b64_check) {
    ...
    case GIFTI_B64_CHECK_COUNT: /* count bad characters */
      for (c = 0; c < src_len; c++)
        if (b64_decode_table[usrc[c]] == (unsigned char)0x80) errs++;
      memcpy(dest, src, src_len);
      apply_len = src_len;
      break;
    ...
    case GIFTI_B64_CHECK_SKIP: /* skip bad characters, but don't count */
      apply_len = 0;
      for (c = 0; c < src_len; c++) {
        if (b64_decode_table[usrc[c]] != (unsigned char)0x80) dest[apply_len++] = src[c];
      }
      break;
...
static int count_bad_b64_chars(const char *src, int len)
{
  ...
  for (c = 0; c < len; c++)
    if (b64_decode_table[usrc[c]] == (unsigned char)0x80) bad++;
...
static int append_to_data_ascii(gxml_data *xd, const char *cdata, int len)
{
  ...
  while (rem_len > 0) {
    ...
    if (xd->verb > 5) fprintf(stderr, "-- %lld vals left at offset %lld, nbyper %d\n", rem_vals, xd->dind, da->nbyper);
...
static int decode_b64(gxml_data *xd, char *cdata, int cdlen, char *dptr, long long *needed)
{
  ...
  if (xd->verb > 4) fprintf(stderr, "-- DB64: decode len %d, remain %lld\n", cdlen, *needed);
  if (*needed <= 0) {
    if (cdlen > 0) fprintf(stderr, "** DB64: %d bytes left without a home\n", cdlen);
    return 0;
  }
  ...
  if (ind < blocks - 1 || (ind < blocks && *needed == 0)) {
    if (xd->verb > 6) gifti_disp_hex_data("decoded b64: 0x ", dptr, assigned, stderr);
...
static int decode_ascii(gxml_data *xd, char *cdata, int cdlen, int type, void *dptr, long long *nvals, int *mod_prev)
{
  ...
  switch (type) {
    ...
    case NIFTI_TYPE_RGB24: {
      ...
      while ((vals < 0 || vals < *nvals) && p1) {
        ...
        if (xd->verb > 6) fprintf(stderr, "  v %u (%ld)", ptr[3 * vals], lval);
  ...
  if (whitespace_len(cdata + (cdlen - remain), remain) == remain) remain = 0;
...
static int show_stack(char *mesg, gxml_data *xd)
{
  ...
  for (c = 0; c < xd->depth; c++) fprintf(stderr, " : %s", enames[xd->stack[c]]);
  fputc('\n', stderr);
  return 0;
}
...
static int stack_is_valid(gxml_data *xd)
{
  ...
  switch (etype) {
    ...
    case GXML_ETYPE_GIFTI: /* should only be at depth 1 */
      ...
      break;
    case GXML_ETYPE_META:
      if (parent != GXML_ETYPE_GIFTI && parent != GXML_ETYPE_DATAARRAY) bad_parent = 1;
      break;
    case GXML_ETYPE_MD:
  ...
  if (bad_parent && GXD.verb) fprintf(stderr, "** %s: bad parent '%s'\n", enames[etype], enames[parent]);
...
static int reset_xml_buf(gxml_data *xd, char **buf, int *bsize)
{
  if (*bsize == xd->buf_size) {
    if (xd->verb > 3) fprintf(stderr, "-- buffer kept at %d bytes\n", *bsize);
    return 0;
  }

  if (xd->verb > 2) fprintf(stderr, "++ update buf, %d to %d bytes\n", *bsize, xd->buf_size);
...
static int update_xml_buf_size(gxml_data *xd, long long bytes)
{
  ...
  if (!xd || bytes < 0) {
    if (xd->verb > 1) fprintf(stderr, "** bad update_xml_buf_size with %p and %lld\n", (void *)xd, bytes);
    return 0;
  }

  new_size = partial_buf_size(bytes);
  if (new_size != xd->buf_size) {
    if (xd->verb > 2) fprintf(stderr, "++ update XML buf size, %d to %d (for %lld)\n", xd->buf_size, new_size, bytes);
...
static int update_partial_buffer(char **buf, int *blen, long long bytes, int full)
{
  ...
  /* just make sure we have a text buffer to work with */
  if (*buf || *blen != bsize) {
    if (GXD.verb > 2) fprintf(stderr, "++ UPB, alloc %d bytes (from %lld, %d) for buff\n", bsize, bytes, full);
    *buf = (char *)realloc(*buf, bsize * sizeof(char));
    if (!*buf) {
...
static int gxml_write_gifti(gxml_data *xd, FILE *fp)
{
  ...
  if (!gim->darray) {
    ...
    for (c = 0; c < gim->numDA; c++) ewrite_darray(xd, gim->darray[c], fp);
  }
...
static int ewrite_darray(gxml_data *xd, giiDataArray *da, FILE *fp)
{
  ...
  for (c = 0; c < da->numCS; c++) ewrite_coordsys(xd, da->coordsys[c], fp);
...
static int ewrite_data(gxml_data *xd, giiDataArray *da, FILE *fp)
{
  ...
  if (xd->verb > 3) fprintf(stderr, "++ write %s Data\n", gifti_list_index2string(gifti_encoding_list, da->encoding));
  ...
  if (xd->dstore) {
    if (da->encoding == GIFTI_ENCODING_ASCII) {
      ...
      for (c = 0; c < rows; c++) ewrite_data_line(da->data, da->datatype, c, cols, spaces + xd->indent, fp);
      fprintf(fp, "%*s", spaces, "");
    }
  ...
  if (da->encoding != GIFTI_ENCODING_EXTBIN) fprintf(fp, "</%s>\n", enames[GXML_ETYPE_DATA]);
...
static int ewrite_coordsys(gxml_data *xd, giiCoordSystem *cs, FILE *fp)
{
  ...
  for (c = 0; c < 4; c++) ewrite_double_line(cs->xform[c], 4, spaces + xd->indent, fp);
  fprintf(fp, "%*s</MatrixData>\n", spaces, "");
