...
main(int argc, char *argv[])
{
  ...
  mri_labels = MRIallocSequence(area->n_points, 2, 1, 1, MRI_FLOAT) ;
...
MRISextractVolumeWindow(MRI_SURFACE *mris, MRI *mri, int wsize, int vno)
{
  MRI    *mri_vol ;
  VERTEX *v ;
  double x0, y0, z0, whalf, xs, ys, zs, xv, yv, zv, val, xc, yc, zc ;
  int   xi, yi, zi ;
  ...
  for (zi = 0 ; zi < wsize ; zi++)
	...
	yc = (yi-whalf) ;
	zc = (zi-whalf) ;
	xs = x0 + xc*v->e1x + yc*v->e2x + zc*v->nx ;
	ys = y0 + yc*v->e1y + yc*v->e2y + zc*v->ny;
	zs = z0 + zc*v->e1z + zc*v->e2z + zc*v->nz ;
