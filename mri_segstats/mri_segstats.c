...
int main(int argc, char **argv)
{
  ...
  */
  ...
  for (n=0; n < nsegid; n++)
  {
    ...
    if (!dontrun)
    {
      if (!mris)
      {
        ...
        {
          vol = MRIvoxelsInLabelWithPartialVolumeEffects(seg,
                                                         pvvol,
                                                         StatSumTable[n].id,
                                                         NULL, NULL);
  ...
  if (StatTableFile != NULL)
  {
    ...
    {
      fprintf(fp,
              "# Measure TotalGray, TotalGrayVol, Total gray matter volume, %f, mm^3\n",
              BrainVolStats[8]);
    }
    if(DoSupraTent)
    {
  ...
  if (DoFrameAvg)
  {
    ...
    for (n=0; n < nsegid; n++)
    {
      ...
      if(RmFrameAvgMn) for(f=0; f < invol->nframes; f++)
        {
          favg[n][f] -= favgmn[n];
        }
    }
...
static int parse_commandline(int argc, char **argv)
{
  ...
  while (nargc > 0)
  {
    ...
    else if(!strcmp(option, "--replace-file"))
    {
      ...
      int err=GTMloadReplacmentList(pargv[0],
                                    &nReplace,
                                    &(SrcReplace[0]),
                                    &(TrgReplace[0]));
      if(err)
