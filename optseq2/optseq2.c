...
static char vcid[] = "$Id: optseq2.c,v 2.12.2.1 2008/01/10 23:25:40 greve Exp $";
char *Progname = NULL;
...
int main(int argc, char **argv) {
  ...
  nargs = handle_version_option (argc, argv, "$Id: optseq2.c,v 2.12.2.1 2008/01/10 23:25:40 greve Exp $", "$Name:  $");
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))  print_help() ;
      ...
      while (fp != NULL) {
        ...
        memcpy(infilelist[nInFiles],fname,strlen(fname));
        nInFiles++;
        sprintf(fname,"%s-%03d.par",instem,nInFiles+1);
        fp = fopen(fname,"r");
      }
