...
MRIdetrendVolume(MRI *mri_in, int *conditions, MRI *mri_out)
{
		...
		for (y = 0 ; y < mri_out->height ; y++)
		{
			for (z = 0 ; z < mri_out->depth ; z++)
			{
				...
				for (mean = 0.0, i = t = 0 ; t < ntime ; t++)
				{
					...
					switch (mri_in->type)
					{
					default: ErrorExit(ERROR_UNSUPPORTED, "MRIdetrendVolume: input type %d unsupported",
														 mri_in->type) ;
					case MRI_FLOAT: val = MRIFseq_vox(mri_in, x, y, z, t) ; break ;
					case MRI_SHORT: val = MRISseq_vox(mri_in, x, y, z, t) ; break ;
					case MRI_INT:   val = MRIIseq_vox(mri_in, x, y, z, t) ; break ;
					case MRI_UCHAR: val = MRIseq_vox(mri_in, x, y, z, t) ; break ;
					}
					*MATRIX_RELT(v_out, i+1, 1) = val ;
					*MATRIX_RELT(m_X, i+1, 1) = t ; *MATRIX_RELT(m_X, i+1, 2) = 1 ;
					mean += val ;
					i++ ;
				}
				mean /= (float)i ;
				...
				a = VECTOR_ELT(v_p, 1) ; b = VECTOR_ELT(v_p, 2) ;
				...
				for (t = 0 ; t < ntime ; t++)
				{
					...
					default: ErrorExit(ERROR_UNSUPPORTED, "MRIdetrendVolume: input type %d unsupported",
														 mri_in->type) ;
					case MRI_FLOAT: MRIFseq_vox(mri_in, x, y, z, t) = val ; break ;
					case MRI_SHORT: MRISseq_vox(mri_in, x, y, z, t) = (short)nint(val) ; break ;
					case MRI_INT:   MRIIseq_vox(mri_in, x, y, z, t) = nint(val) ; break ;
					case MRI_UCHAR: MRIseq_vox(mri_in, x, y, z, t) = (BUFTYPE)nint(val) ; break ;
					}
				}
			}
		}
  ...
  MatrixFree(&m_X) ;  MatrixFree(&m_Xt) ; MatrixFree(&m_XtXinv) ; MatrixFree(&m) ;
	MatrixFree(&m_XtX) ;
	VectorFree(&v_out) ; VectorFree(&v_p) ;
	return(mri_out) ;
}
		...
		for (y = 0 ; y < mri_out->height ; y++)
		{
			for (z = 0 ; z < mri_out->depth ; z++)
			{
				...
				for (t = 0 ; t < ntime ; t++)
				{
					switch (mri_in->type)
					{
					default: ErrorExit(ERROR_UNSUPPORTED, "MRIextractConditions: input type %d unsupported",
														 mri_in->type) ;
					case MRI_FLOAT: val = MRIFseq_vox(mri_in, x, y, z, t) ; break ;
					case MRI_SHORT: val = MRISseq_vox(mri_in, x, y, z, t) ; break ;
					case MRI_INT:   val = MRIIseq_vox(mri_in, x, y, z, t) ; break ;
