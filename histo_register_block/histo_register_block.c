...
static char vcid[] = "$Id: histo_register_block.c,v 1.4 2006/08/10 21:39:25 nicks Exp $";
...
main(int argc, char *argv[])
{
  ...
  nargs = handle_version_option (argc, argv, "$Id: histo_register_block.c,v 1.4 2006/08/10 21:39:25 nicks Exp $", "$Name:  $");
  ...
  if (argc < 4)
	...
	if (rotate_radians > 0)
	{
		...
		MRIfree(&mri_block) ; mri_block = mri_tmp ;
	}
  ...
  out_name = argv[argc-1] ;
...
#if 0
	MRIdilate(mri_seg, mri_seg) ; MRIdilate(mri_seg, mri_seg) ;
	...
	MRIfree(&mri_histo) ; mri_histo = mri_tmp ;
...
{	
	...
	mri_src_pyramid[0] = mri_src ; mri_dst_pyramid[0] = mri_dst ;
  ...
  *MATRIX_RELT(m_origin, 2, 3) = mri_fullres_src->height/2 ;
	...
	*MATRIX_RELT(m_scale, 3, 3) = 1.0 ;
	...
	for (scale = 1 ; scale >= 0.005 ; scale /= 2)
	{
		...
		min_error = compute_alignment_error(mri_src, mri_dst, mri_seg, pdf, m_best, skip, level, cost_type) ;
		mint = -maxt ;
		maxs += 0.5*sstep ; maxa += 0.5*astep ; maxt += 0.5*tstep;
		if (sstep < 0.00000001)
			sstep = 1 ;
		do
		{
	...
	MatrixFree(&m_rot) ; MatrixFree(&m_trans) ; MatrixFree(&m) ; MatrixFree(&m_scale) ;
	MatrixCopy(m_best, xform) ; MatrixFree(&m_best) ; MatrixFree(&m_origin) ; MatrixFree(&m_tmp);
	MatrixFree(&m_origin_inv) ; MatrixFree(&m_delta) ; MatrixFree(&m_start) ;
	return(NO_ERROR) ;
}
...
compute_ml_alignment_error(MRI *mri_src, MRI *mri_dst, MRI *mri_seg, DENSITY *pdf, MATRIX *m_total, int skip, int level)
{
	...
	VectorFree(&v_src) ; VectorFree(&v_dst) ;
	if (FZERO(num))
		num = 1 ;
	MatrixFree(&m_inv) ;
	return(sse / num) ;
...
mri_apply_slice_xform(MRI *mri_src, MRI *mri_dst, MATRIX *m, int slice)
{
	...
	for (x = 0 ; x < mri_dst->width ; x++)
	{
		...
		for (y = 0 ; y < mri_dst->height ; y++)
		{
			...
			xs = VECTOR_ELT(v_src, 1) ; ys = VECTOR_ELT(v_src, 2) ;
														...
														int maxt, int ntrans, int level, int skip, MRI *mri_fullres_src, int cost_type)
{
	...
	MatrixFree(&m_trans) ; MatrixFree(&m_total) ; MatrixFree(&m_start) ;
	return(found) ;
}
	...
	for (nseg = oob = 0,  num = x = 0 ; x < mri_dst->width ; x += skip)
	{
		...
		for (y = 0 ; y < mri_dst->height ; y += skip)
		{
			...
			if (MRIindexNotInVolume(mri_src, xs, ys, 0) == 0)  /* a valid point */
			{
				...
				if (i == Gdiag_no)
					DiagBreak() ;
				means[i] += val_src ; vars[i] += val_src*val_src ; nvox[i]++ ;
				mean += val_src ; var += val_src*val_src ; num++ ;
			}
			else
				oob++ ;
		}
...
rotate_image(MRI *mri_src, MRI *mri_dst, double rotate_radians)
{
  ...
  *MATRIX_RELT(m_origin, 2, 3) = mri_src->height/2 ;
	...
	*MATRIX_RELT(m_rot, 3, 3) = 1.0 ;
	...
	VectorFree(&v_src) ; VectorFree(&v_dst) ;
	MatrixFree(&m_rot) ; MatrixFree(&m_origin) ; MatrixFree(&m_origin_inv) ;
	MatrixFree(&m_total) ;
	return(mri_dst) ;
}
...
probe_cost_function(MRI *mri_src, MRI *mri_dst, MRI *mri_seg, DENSITY *pdf, MATRIX *mat, int cost_type, char *out_name)
{
  ...
  *MATRIX_RELT(m_origin, 2, 3) = mri_src->height/2 ;
	...
	*MATRIX_RELT(m_total, 3,3) = 1.0 ;
	...
	MatrixFree(&m_rot) ; MatrixFree(&m) ; MatrixFree(&m_scale) ; MatrixFree(&m_origin) ;  MatrixFree(&m_origin_inv) ; 
	return(NO_ERROR) ;
}
#define NPARMS (3*3)
#ifdef TOL
...
powell_minimize(MRI *mri_block, MRI *mri_histo, MRI *mri_seg, DENSITY *pdf, MATRIX *mat, int cost_type)
{
	...
	Gmri_block = mri_block ; Gmri_histo = mri_histo ; Gmri_seg = mri_seg ;
	Gpdf = pdf ; Gcost_type = cost_type ;
	for (r = 1 ; r <= NPARMS ; r++)
	{
		for (c = 1 ; c <= NPARMS ; c++)
		{
