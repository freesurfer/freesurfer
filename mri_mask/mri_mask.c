...
int main(int argc, char *argv[])
{
  ...
  if(xform_fname != NULL){
    ...
    //    LTAchangeType(lta, LINEAR_VOX_TO_VOX);
    mri_tmp = MRIalloc(mri_src->width, mri_src->height, mri_src->depth, mri_mask->type) ;
    MRIcopyHeader(mri_src, mri_tmp) ; 
 
    mri_tmp = LTAtransformInterp(mri_mask, mri_tmp, lta, InterpMethod);

  // mri_tmp = MRIlinearTransformInterp(mri_mask, mri_tmp, lta->xforms[0].m_L, InterpMethod);
...
void usage(int exit_val)
{
  ...
  fprintf(fout, "usage: %s [options] <in vol> <mask vol> <out vol>\n", Progname);
  fprintf(fout, "this program applies a mask volume (typically skull stripped)\n") ;

  fprintf(fout, "Options:\n") ;
  fprintf(fout, "   -xform %%s apply LTA transform to align mask to input volume\n");
