...
main(int argc, char *argv[])
{
  ...
  mri_sc = MRIread(argv[3]) ;
	...
	mri_tmp = MRIchangeType(mri_ctrl, MRI_UCHAR, 0, 1, 1); MRIfree(&mri_ctrl); mri_ctrl = mri_tmp;
	MRImask(mri_bc, mri_mask, mri_bc, 0, 0) ;
	MRImask(mri_sc, mri_mask, mri_sc, 0, 0) ;
	mri_tmp = MRIchangeType(mri_bc, MRI_FLOAT, 0, 1, 1) ; MRIfree(&mri_bc) ; mri_bc = mri_tmp ;
	mri_tmp = MRIchangeType(mri_sc, MRI_FLOAT, 0, 1, 1) ; MRIfree(&mri_sc) ; mri_sc = mri_tmp ;
	...
	MRIfree(&mri_ctrl) ; MRIfree(&mri_mask) ;
	MRIfree(&mri_bc) ; MRIfree(&mri_sc) ;
	...
	MRIfree(&mri_kernel) ; MRIfree(&mri_bias) ; MRIfree(&mri_smooth_bias) ;
