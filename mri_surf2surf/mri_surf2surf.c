...
static char vcid[] = "$Id: mri_surf2surf.c,v 1.59.2.1 2007/09/21 22:38:59 greve Exp $";
char *Progname = NULL;
...
int main(int argc, char **argv) {
  ...
  nargs = handle_version_option (argc, argv, "$Id: mri_surf2surf.c,v 1.59.2.1 2007/09/21 22:38:59 greve Exp $", "$Name:  $");
  ...
  if (!strcmp(srctypestring,"curv")) { /* curvature file */
	...
	if(ProjType == 2){
	  // Load thickness
	  sprintf(tmpstr,"%s/%s/surf/%s.thickness",SUBJECTS_DIR,srcsubject,srchemi);
	...
	for(n=0; n < SurfSrc->nvertices; n++){
	  if(ProjType == 1) ProjNormDist(&x, &y, &z,SurfSrc, n, ProjDepth);
	  if(ProjType == 2) ProjNormFracThick(&x, &y, &z,SurfSrc, n, ProjDepth);
