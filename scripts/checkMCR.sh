...
if ( ! -e ${FREESURFER_HOME}/MCRv84/bin/glnxa64/libmwlaunchermain.so \
  ...
  echo "curl "\""https://surfer.nmr.mgh.harvard.edu/fswiki/MatlabRuntime?action=AttachFile&do=get&target=runtime2014bLinux.tar.gz"\"" -o "\""runtime2012b.tar.gz"\"""
  echo "tar xvf runtime2014b.tar.gz"
  echo " "
  echo "MAC:"
  echo " "
  echo "cd ${FREESURFER_HOME}"
  echo "curl "\""https://surfer.nmr.mgh.harvard.edu/fswiki/MatlabRuntime?action=AttachFile&do=get&target=runtime2014bMAC.tar.gz"\"" -o "\""runtime2012b.tar.gz"\"""
  echo "tar xvf runtime2014b.tar.gz"
  echo " "
  echo " "
