...
// Revision Date  : $Date: 2003/04/03 21:43:42 $
// Revision       : $Revision: 1.8 $
//
////////////////////////////////////////////////////////////////////
char *MRI_WATERSHED_VERSION = "$Revision: 1.8 $";
...
void calcForce1(double &force0, double &force1, double &force, 
		...
		const double &sx, const double &sy, const double &sz, const double &sd,
...
void calcForce2(double &force0, double &force1, double &force, 
		...
		const double &nx, const double &ny, const double &nz,
		MRI_variables *mri_var);
static void FitShape(MRI_variables *MRI_var, const int convLimit, const int maxIter,
		     void (*calcForce)
...
static void lisse(unsigned long *tab);
...
static int mrisLocalizeErrors(MRIS* mris_curv,MRIS *mris_dCOG,MRI_variables *MRI_var,MRIS *mrisphere);
...
static void MRISscaleFields(MRIS *mris_src,MRIS *mris_dst,MRIS *mris_vdst,int whichfield);
...
int main(int argc, char *argv[])
{
  ...
  nargs = handle_version_option (argc, argv, "$Id: mri_watershed.cpp,v 1.8 2003/04/03 21:43:42 tosa Exp $");
  argc -= nargs ;
  if (1 == argc)
    exit (0);
...
get_option(int argc, char *argv[],STRIP_PARMS *parms)
{
  ...
  else if (!strcmp(option, "mask"))
  {
    if (nmask_volumes >= MAX_MASK_VOLUMES)
      ErrorExit(ERROR_UNSUPPORTED, "%s: too many mask volumes specified (max=%d)\n",
                MAX_MASK_VOLUMES) ;
    strcpy(mask_in_fnames[nmask_volumes], argv[2]) ;
    strcpy(mask_out_fnames[nmask_volumes++], argv[3]) ;
    printf("Mode:          masking volume %s with brain mask and writing it to %s...\n", argv[2], argv[3]) ;
    nargs = 2 ;
  }
  else if(!strcmp(option, "LABEL"))
  {
    ...
    if(parms->surfname==NULL)
      parms->surfname="./";
    fprintf(stderr,"Mode:          Writing out tissue label into output volume\n") ;
    fprintf(stderr,"                       Assumption : no biais field and FGM\n");
      ...
      for(j=1;j<r-1;j++)
        for(i=1;i<r-1;i++)
          if(mean_val[k][j][i]==MRI_var->WM_HALF_MAX)
            ...
            {
              n++;
              MRI_var->tabdim[MRIvox(MRI_var->mri_src,xmin+1+i,ymin+1+j,zmin+1+k)]--;
              MRIvox(MRI_var->mri_src,xmin+1+i,ymin+1+j,zmin+1+k)=MRI_var->Imax;
              MRI_var->tabdim[MRI_var->Imax]++;
              MRI_var->Basin[zmin+1+k][ymin+1+j][xmin+1+i].type=3;
              MRI_var->Basin[zmin+1+k][ymin+1+j][xmin+1+i].next=CreateBasinCell(MRI_var->Imax,1,0);
            }
...
static void Template_Deformation(STRIP_PARMS *parms,MRI_variables *MRI_var)
{
  ...
  if (parms->template_deformation!=2)
  {
    ...
    fprintf(stderr,"\n\n*************************VALIDATION*************************");
    if(MRI_var->mri_src->slice_direction!=MRI_CORONAL)
      fprintf(stderr,"\n      The   Validation   Step  will  Not  Provide"
    ...
    {
      fprintf(stderr,"\nCorrection of the shape of the surface...");
...
// analyze CSF, GM, values etc using the surface
static void local_params(STRIP_PARMS *parms,MRI_variables *MRI_var)
{
  ...
  {
    find_normal(mris->vertices[kv].nx,mris->vertices[kv].ny,mris->vertices[kv].nz,n1,n2,MRI_var->direction);
    //
    ...
    for(h=-2;h<2;h++)
    {
      ...
      if (!(k<0||k>=MRI_var->depth||i<0||i>=MRI_var->width||j<0||j>=MRI_var->height))
      {
	// get the voxel value
    ...
    if (val<3*MRI_var->CSF_intensity)
    {
      ...
      for(a=-1;a<2;a++)
        for(b=-1;b<2;b++)
        {
          px=mris->vertices[kv].x-rp*mris->vertices[kv].nx + a*n1[0]+b*n2[0];
  ...
  for (kv=0;kv<mris->nvertices;kv++)
  {
	...
	k=(int)(tz+0.5);
        ...
        if (!(k<0||k>=MRI_var->depth||i<0||i>=MRI_var->width||j<0||j>=MRI_var->height))
          if (MRIvox(MRI_var->mri_src,i,j,k)<MRI_var->WM_MIN-MRI_var->WM_VARIANCE/2)
            break;
      }
      for (c=h;c>=0;c--)
      {
      ...
      for(;c<h;c++)
	    ...
	    k=(int)(tz+0.5);
            if (!(k<0||k>=MRI_var->depth||i<0||i>=MRI_var->width||j<0||j>=MRI_var->height))
            {
              ...
              if(val_buff<MRI_var->WM_MIN-MRI_var->WM_VARIANCE/2 && val_buff>MRI_var->WM_MIN/4)
              {
                int_percent[val_buff]++;
                n++;
              }
            }
          }
  ...
  MRI_var->TRANSITION_INTENSITY=
    ...
    * (MRI_var->CSF_MAX-MRI_var->CSF_intensity)/(MRI_var->CSF_MAX+MRI_var->GM_INTENSITY-MRI_var->GM_MIN-MRI_var->CSF_intensity);
  ...
  MRI_var->GM_MIN=MAX(MAX(MRI_var->TRANSITION_INTENSITY,MRI_var->GM_MIN),MRI_var->WM_MIN/4);
  MRI_var->GM_INTENSITY=MAX(MRI_var->GM_INTENSITY,MRI_var->GM_MIN);

  if (!parms->skull_type)
  {
    MRI_var->TRANSITION_INTENSITY=(2*MRI_var->GM_MIN+MRI_var->TRANSITION_INTENSITY)/3;
    MRI_var->CSF_HALF_MAX=MIN(MRI_var->CSF_intensity/2+MIN(MRI_var->CSF_MAX,MRI_var->TRANSITION_INTENSITY)/2
    ...
    MRI_var->GM_INTENSITY=(MRI_var->GM_INTENSITY+3*MRI_var->TRANSITION_INTENSITY)/4;
  }
  else if (parms->skull_type==-1)
  {
    if (MRI_var->GM_MIN==MRI_var->TRANSITION_INTENSITY)
    {
      MRI_var->TRANSITION_INTENSITY=(3*MRI_var->GM_MIN+MRI_var->GM_INTENSITY)/4;
      MRI_var->GM_INTENSITY=(MRI_var->GM_INTENSITY+MRI_var->GM_MIN)/2;
    }
    else
    {
      MRI_var->TRANSITION_INTENSITY=MRI_var->GM_MIN;
      MRI_var->GM_INTENSITY=(MRI_var->GM_INTENSITY+2*MRI_var->GM_MIN)/3;
    };
    MRI_var->CSF_HALF_MAX=MIN(MRI_var->CSF_intensity/2+MIN(MRI_var->CSF_MAX,MRI_var->TRANSITION_INTENSITY)/2
                     ,MRI_var->CSF_HALF_MAX);
    MRI_var->CSF_MAX=(MRI_var->CSF_HALF_MAX+MRI_var->TRANSITION_INTENSITY)/2;
  ...
  else if (parms->skull_type==1) // more option
  {
    if (MRI_var->GM_MIN==MRI_var->TRANSITION_INTENSITY)
    {
      MRI_var->TRANSITION_INTENSITY=(MRI_var->CSF_intensity+3*MRI_var->TRANSITION_INTENSITY)/4;
      MRI_var->GM_INTENSITY=MIN(MRI_var->GM_MIN,(MRI_var->GM_INTENSITY+3*MRI_var->TRANSITION_INTENSITY)/4);
    }
    else
    {
      MRI_var->TRANSITION_INTENSITY=(MRI_var->GM_MIN+MRI_var->TRANSITION_INTENSITY)/2;
      MRI_var->GM_INTENSITY=(MRI_var->GM_INTENSITY+3*MRI_var->TRANSITION_INTENSITY)/4;
    }
    MRI_var->CSF_HALF_MAX=MIN((MRI_var->CSF_intensity+MIN(MRI_var->CSF_MAX,MRI_var->TRANSITION_INTENSITY))/2
                     ,MRI_var->CSF_HALF_MAX);
    if (MRI_var->CSF_MAX>=MRI_var->TRANSITION_INTENSITY)
      MRI_var->CSF_MAX=(MRI_var->CSF_HALF_MAX+MRI_var->TRANSITION_INTENSITY)/2;
  };

   fprintf(stderr,"\n      after  analyzing : CSF_MAX=%d, TRANSITION_INTENSITY=%d, GM=%d",
          MRI_var->CSF_MAX, MRI_var->TRANSITION_INTENSITY,MRI_var->GM_INTENSITY);
...
static void analyseCSF(unsigned long *CSF_percent,MRI_variables *MRI_var)
{
...
#if OUTPUT_CURVES
  for(k=0;k<256;k++)
    fprintf(MRI_var->fout, " %ld",CSF_percent[k]);
    fprintf(MRI_var->fout, "\n");
#endif
  ...
  for(k=0;k<3*MRI_var->CSF_intensity;k++)
    if (CSF_percent[k]>tmp)
      tmp=CSF_percent[k];
  
  
  for(k=0;k<3*MRI_var->CSF_intensity;k++)
  {
    fprintf(stderr,"\n%d %ld",k,CSF_percent[k]);
    for(n=0;n<CSF_percent[k]*50/tmp;n++)
      fprintf(stderr,".");
  }
#endif
...
static void analyseGM(unsigned long *CSF_percent,unsigned long *int_percent,MRI_variables *MRI_var)
{
...
#if OUTPUT_CURVES
    for(k=0;k<256;k++)
    fprintf(MRI_var->fout, " %ld",int_percent[k]); 
    fprintf(MRI_var->fout, "\n");    
#endif
  ...
  for(k=0;k<MRI_var->CSF_intensity;k++)
    int_percent[k]=0;
  for(;k<256;k++)
    int_percent[k]=(long unsigned int) (MAX(0,int_percent[k]-tmp*CSF_percent[k]/buff));
...
#if DEBUG_CURVE
  fprintf(stderr, " gm curve\n");
  tmp=0;
  for(k=0;k<MRI_var->WM_INTENSITY;k++)
    if (int_percent[k]>tmp)
      tmp=int_percent[k];
  
  for(k=0;k<MRI_var->WM_INTENSITY;k++)
  {
    fprintf(stderr,"\n%d %ld",k,int_percent[k]);
    for(n=0;n<int_percent[k]*50/tmp;n++)
      fprintf(stderr,".");
      }
...
#if OUTPUT_CURVES
  for(k=0;k<256;k++)
    fprintf(MRI_var->fout, " %ld",MRI_var->gmnumber[k]); 
    fprintf(MRI_var->fout, "\n");
#endif

  lisse(MRI_var->gmnumber);
  ...
  for(k=0;k<MRI_var->CSF_intensity;k++)
    MRI_var->gmnumber[k]=0;
  for(;k<256;k++)
    MRI_var->gmnumber[k]=(long unsigned int) (MAX(0,MRI_var->gmnumber[k]-tmp*CSF_percent[k]/buff));

#if DEBUG_CURVE
  fprintf(stderr, " other gm curve\n");
  tmp=0;
  for(k=0;k<MRI_var->WM_INTENSITY;k++)
    if (MRI_var->gmnumber[k]>tmp)
      tmp=MRI_var->gmnumber[k];
  
  for(k=0;k<MRI_var->WM_INTENSITY;k++)
  {
    fprintf(stderr,"\n%d %ld",k,MRI_var->gmnumber[k]);
    for(n=0;n<MRI_var->gmnumber[k]*50/tmp;n++)
      fprintf(stderr,".");
      }
#endif
...
static void shrinkstep(MRI_variables *MRI_var)
{
  ...
  double tx,ty,tz;
  float xmin=MRI_var->width,xmax=0,ymin=MRI_var->height,ymax=0,zmin=MRI_var->depth,zmax=0;
  MRIS *mris;
  ...
  // set the center of gravity as voxel value.
  MRI_var->xCOG=tx;
  MRI_var->yCOG=ty;
  MRI_var->zCOG=tz;
}
...
static int ValidationSurfaceShape(MRI_variables *MRI_var)
{
  ...
  MRI_var->mris_curv     =MRISfromParameterization(mrisp_template, MRI_var->mris_curv, 0);  
  MRI_var->mris_var_curv =MRISfromParameterization(mrisp_template, MRI_var->mris_var_curv, 1);  
  MRI_var->mris_dCOG     =MRISfromParameterization(mrisp_template, MRI_var->mris_dCOG, 3);  
  MRI_var->mris_var_dCOG =MRISfromParameterization(mrisp_template, MRI_var->mris_var_dCOG, 4);  
...
static void MRISinitSurfaces(MRIS *mris_curv,MRIS *mris_dCOG, const MRIS *mrisphere)
{
  ...
  MRISchangeCoordinates(mris_dCOG, mris_curv); // use the mris_curv vertex positions (smoothed values)
  //
  ...
  //
  MRISdistanceToCOG(mris_dCOG);               // store dCOG as vetex.curv member
  MRISaverageCurvatures(mris_dCOG, navgs) ;   // so average curvature is actually the average dCOG
  MRISnormalizeCurvature(mris_dCOG) ;         // move the value to mean and divide by std
  // change coordinates to sphere coord
  // even though the curvatures, the dCOG are from smoothed surface, the vertex positions
                            ...
                            int use_stds)
{
  ...
  for (sse = 0.0f, var_sse=0.0f, count=0 , vno = 0 ; vno < mris->nvertices ; vno++)
  {
    v = &mris->vertices[vno] ;
...
static int
mrisRigidBodyAlignGlobal(MRIS *mris_curv, MRIS *mris_dist,INTEGRATION_PARMS *parms,int mode,
...
static void MRISscaleFields(MRIS *mris_src,MRIS *mris_fdst,MRIS *mris_vdst,int whichfield)
{
  int k;
  double tp;
  ...
  if(VERBOSE_MODE)
    fprintf(stderr,"\n         source values: mean = %2.5f sigma = %2.5f  ",field,sigma);
  ...
  if(VERBOSE_MODE)
    fprintf(stderr,"\n         destination values: mean = %2.5f sigma = %2.5f  ",oldfield,oldsigma);

  /*scale dst fields to sources values*/
  for(k=0;k<mris_src->nvertices;k++)
  {
    mris1->vertices[k].curv= (mris1->vertices[k].curv-oldfield)*sigma/oldsigma+field;
...
static void MRISCorrectSurface(MRI_variables *MRI_var)
{
  ...
  for (iter=0;niter;iter++)
  {
    ...
    for (k=0;k<mris->nvertices;k++)
    {
      ...
      if((iter<30) && (!MRI_var->mrisphere->vertices[k].val))
	continue;
    ...
    /*    if ((niter==int_smooth) && !(iter % 5))
      {
      fprintf(stderr,
      "%d: lm=%5.3f,f1m=%5.3f,f2m=%5.3f,f3m=%5.3f,dm=%5.3f,d10m=%5.3f,c=%5.3f\n"
...
static void MRISComputeLocalValues(MRI_variables *MRI_var)
{
  ...
  if(MRI_var->atlas)
  {
    ...
    for(k=0;k<nvertices;k++)
    {
      ...
      vsphere->tx=(MRI_var->mris_dCOG->vertices[k].curv-mris->vertices[k].curv);
  ...
  while(niter)
  {
	...
	w1+=mrisphere->vertices[v->v[m]].ty;
	gm+=mrisphere->vertices[v->v[m]].tz*mrisphere->vertices[v->v[m]].ty;
	var_csf+=mrisphere->vertices[v->v[m]].tdx*mrisphere->vertices[v->v[m]].ty;
	var_gray+=mrisphere->vertices[v->v[m]].tdz*mrisphere->vertices[v->v[m]].ty;
  ...
  for(k=0;k<nvertices;k++)
  {
    ...
    mean_gray+=gm;
    var_gray+=SQR(gm);
    vsphere->y=(csf*vsphere->odz+gm*vsphere->odx)/(vsphere->odx+vsphere->odz);
...
static void MRISFineSegmentation(MRI_variables *MRI_var)
{
  ...
  for (iter=0;niter;iter++)
  {
    ...
    {   
      ...
      force4 = -coeff4*0.0002*(MRI_var->mris_curv->vertices[k].curv-mris->vertices[k].curv)
	/MRI_var->mris_var_curv->vertices[k].curv;
    ...
    {      
      fprintf(stderr,
              "\n%d: lm=%5.3f,f1m=%5.3f,f2m=%5.3f,f3m=%5.3f,f4m=%5.3f,dm=%5.3f,d10m=%5.3f,c=%5.3f"
	...
	fprintf(stderr,"\nScaling of atlas fields onto current surface fields");
      ...
      MRISscaleFields(mris,MRI_var->mris_dCOG,MRI_var->mris_var_dCOG,DIST_MODE);
      MRISscaleFields(mris,MRI_var->mris_curv,MRI_var->mris_var_curv,CURV_MODE);
      if(VERBOSE_MODE)
	fprintf(stderr,"\nCompute Local Values csf/gray");  
      MRISComputeLocalValues(MRI_var);
    }
...
static void MRISgoToClosestDarkestPoint(MRI_variables *MRI_var)
{
  ...
  for (iter=0;niter;iter++) // niter = 10
  {
    MRIScomputeNormals(mris);
    for (k=0;k<mris->nvertices;k++)
    {
		...
		MRI_variables *mri_var)
{
  ...
  ///////////////////////////////////////////////////////////////
  // force2 = -1 or 0 (outside region : push-in), force3 = 1 or 0 (inside-region: push-out)
