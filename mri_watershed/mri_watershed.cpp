...
template <typename T> void DebugCurve(const T *int_percent, const int max, const char *msg);
...
static void local_params(STRIP_PARMS *parms,MRI_variables *MRI_var)
{
  ...
  for (kv=0;kv<mris->nvertices;kv++)
  {
    ...
    for(h=0;h<ninside-4;h++) // ninside = 30  goes deeper
    {
      ...
      // if find White Matter (mean and variation are of white matter), then stop
	...
	j=(int)(ty+0.5);
	k=(int)(tz+0.5);
	// if inside and grey value is less than (white matter min - variance/2)
        if (!(k<0||k>=MRI_var->depth||i<0||i>=MRI_var->width||j<0||j>=MRI_var->height))
  ...
  if (!parms->skull_type) // default
  {
    ...
    // change CSF_HALF_MAX
    MRI_var->CSF_HALF_MAX=MIN(MRI_var->CSF_intensity/2 
  ...
  else if (parms->skull_type==-1) // less
  {
    ...
    MRI_var->CSF_HALF_MAX
      =MIN(MRI_var->CSF_intensity/2 + MIN(MRI_var->CSF_MAX, MRI_var->TRANSITION_intensity)/2,
           MRI_var->CSF_HALF_MAX);
    MRI_var->CSF_MAX=(MRI_var->CSF_HALF_MAX+MRI_var->TRANSITION_intensity)/2;
  ...
  else if (parms->skull_type==1) // more 
  {
    if (MRI_var->GM_MIN==MRI_var->TRANSITION_intensity)
    {
      ...
      MRI_var->GM_intensity        = MIN(MRI_var->GM_MIN,
...
static int ValidationSurfaceShape(MRI_variables *MRI_var)
{
  ...
  // modify the values using template (MRI_var contains the ones from the template)
...
static double estimateNRG(MRI_variables *MRI_var, double cx, double cy ,double cz);
static void computeGradient(MRI_variables *MRI_var,double cx, double cy ,double cz,double *gx,double *gy, double *gz);
static int computeCOG(MRI_variables *MRI_var,double *xCOG, double *yCOG, double *zCOG,int mode);
...
static void computeGradient(MRI_variables *MRI_var,double cx, double cy ,double cz,double *gx,double *gy, double *gz){
  ...
  for(tx = 0 , ty = 0 , tz = 0 , n = 0 ; n < mris->nvertices ; n++){
		dCOG=sqrt(SQR(mris->vertices[n].x-cx)+SQR(mris->vertices[n].y-cy)
							+SQR(mris->vertices[n].z-cz));
		tx += (1.0-MRI_var->mris_dCOG->vertices[n].curv/dCOG)/MRI_var->mris_var_dCOG->vertices[n].curv
			*(mris->vertices[n].x-cx);
		
		ty += (1.0-MRI_var->mris_dCOG->vertices[n].curv/dCOG)/MRI_var->mris_var_dCOG->vertices[n].curv
			*(mris->vertices[n].y-cy);

		tz += (1.0-MRI_var->mris_dCOG->vertices[n].curv/dCOG)/MRI_var->mris_var_dCOG->vertices[n].curv
    ...
    for (v=0;v<=numv;v++)
    {
      ...
      for (u=0;u<=numu;u++)
      {
        ...
        pz = pz0 + (pz1-pz0)*u/numu;
				...
				j=(int)(ty+0.5);
				i=(int)(tx+0.5);
				if (i>=0 && i<mri->width && j>=0 && j<mri->height && imnr>=0 && imnr<mri->depth)
		...
		for(k = 1 ; k< mri->depth-1 ; k++)
			for(j = 1 ; j < mri->height-1 ; j++)
				for(i = 1 ; i < mri->width-1 ; i++){
					...
					if(new_val>0){
						if(MRIvox(mri,i-1,j,k)<new_val){changed=1;MRIvox(mri,i-1,j,k)=new_val;}
						if(MRIvox(mri,i+1,j,k)<new_val){changed=1;MRIvox(mri,i+1,j,k)=new_val;}
						if(MRIvox(mri,i,j-1,k)<new_val){changed=1;MRIvox(mri,i,j-1,k)=new_val;}
						if(MRIvox(mri,i,j+1,k)<new_val){changed=1;MRIvox(mri,i,j+1,k)=new_val;}
						if(MRIvox(mri,i,j,k-1)<new_val){changed=1;MRIvox(mri,i,j,k-1)=new_val;}
						if(MRIvox(mri,i,j,k+1)<new_val){changed=1;MRIvox(mri,i,j,k+1)=new_val;}
					}
				}
	}
...
#if WRITE_SURFACES
			mris_tmp->vertices[k].curv=(MRI_var->mris_dCOG->vertices[k].curv-dCOG)/
				MRI_var->mris_var_dCOG->vertices[k].curv;
#endif
...
static void MRISgoToClosestDarkestPoint(MRI_variables *MRI_var)
{
  ...
  for (iter=0;niter;iter++)
  {
	...
	if(val<min_val) /* don't move if not sure */ 
	{
      ...
      if(min_val>=1000.0)
	distance=0; 
      v->val=distance; // save the shortest distance (only vlaues -1 -.5, 0, .5, 1.)
      mdist += distance;
    }
    // printf("mean distance to the darkest point = %.4f\n", mdist/mris->nvertices);
    // if (iter%10==0)
    //   printf("brainsize = %d\n", calcBrainSize(mri, mris)); 
