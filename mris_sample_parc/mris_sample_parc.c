...
static char vcid[] = 
"$Id: mris_sample_parc.c,v 1.21.2.1 2006/07/26 02:59:29 nicks Exp $";
...
main(int argc, char *argv[])
{
  ...
  nargs = handle_version_option (argc, argv, 
																 "$Id: mris_sample_parc.c,v 1.21.2.1 2006/07/26 02:59:29 nicks Exp $", "$Name:  $");  if (nargs && argc - nargs == 1)
																	 exit (0);
  argc -= nargs;
  ...
  if (!mri_parc)
	...
	if (mask_fname)
	{
		...
		MRIdilate(mri_mask, mri_mask) ; MRIdilate(mri_mask, mri_mask) ; 
		MRIfree(&mri_tmp) ; mri_tmp = MRIclone(mri_parc, NULL) ;
		MRIcopyLabeledVoxels(mri_parc, mri_mask, mri_tmp, mask_val) ;
		MRIfree(&mri_parc) ; mri_parc = mri_tmp ;
