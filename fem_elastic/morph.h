...
class Transform
{
 public:
  ...
  Transform() : m_strName(), m_pInitial()
    ...
    void setInitial(boost::shared_ptr<Transform> transform) { m_pInitial = transform; }
    boost::shared_ptr<Transform> initial() const { return m_pInitial; }
 ...
 protected:
  ...
  { if (m_pInitial) return m_pInitial->img(pt); return pt; }
 ...
 class AffineTransform3d : public gmp::Transform<3>
{
 public:
  ...
  void set_pars(float* pdata) { if (m_pdata) delete[] m_pdata; m_pdata = pdata; }
...
class DeltaTransform3d : public gmp::Transform<3>
{
 public:
  ...
  // assumes ownership
  void set_field(MRI* field) { if (m_field) MRIfree(&m_field); m_field = field; }
  void set_mask(MRI* mask) { if (m_mask) MRIfree(&m_mask); m_mask = mask; }
 ...
 class FemTransform3d : public gmp::Transform<3>
{
 public:
  ...
  void set_mesh(const boost::shared_ptr<TMesh3d> cpmesh) { m_sharedMesh = cpmesh; }
