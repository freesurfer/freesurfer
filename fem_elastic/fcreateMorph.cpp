     ...
     char* argv[])
{
  ...
  if ( !params.strTemplate.empty() )
  {
    ...
    if (!mri)
      std::cerr << " Failed to open template mri " << params.strTemplate << std::endl;
    else
  ...
  if ( !params.strSubject.empty() )
  {
    ...
    if (!mri)
      std::cerr << " Failed to open subject mri " << params.strSubject << std::endl;
    else
        ...
        ++it )
  {
    ...
    else if ( it->type == "gcam" )
    {
      ...
      try
      {
        //MRI* mask = CopyGcamToDeltaField(gcam, mri);
                ...
                char* av[])
{
  ...
  ("out", po::value(&strOut), " output transform file (.nm3d)")
