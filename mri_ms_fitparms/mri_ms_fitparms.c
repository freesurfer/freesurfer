...
#endif
static double estimate_ms_params(MRI **mri_flash, MRI **mri_flash_synth, int nvolumes, MRI *mri_T1, MRI *mri_PD, MRI *mri_sse, MATRIX **M_reg,LTA *lta);
...
static MRI *estimate_T2star(MRI **mri_all_flash, int nvolumes, MRI *mri_PD, MATRIX **Mreg, LTA *lta) ;
static MRI *compute_T2star_map(MRI **mri_flash, int nvolumes, int *scan_types, MATRIX **Mreg, LTA *lta) ;
...
main(int argc, char *argv[])
{
  ...
  mri_T2star = estimate_T2star(mri_all_flash, nvolumes_total, mri_PD, M_reg, Glta) ;
  if (mri_T2star)
  {
  ...
  case 'T':
		{
			...
			mri = MRIread(argv[3]) ;
			if (mri == NULL)
				ErrorExit(ERROR_NOFILE, "%s: could not read volume %s for output volume settings", Progname, argv[3]) ;
...
static double
estimate_ms_params(MRI **mri_flash, MRI **mri_flash_synth, int nvolumes, MRI *mri_T1, MRI *mri_PD, MRI *mri_sse, MATRIX **M_reg, LTA *lta)
{
  ...
  MRI      *mri ;
  MATRIX   *vox2ras[MAX_NVOLS], *ras2vox[MAX_NVOLS], *voxvec1, *voxvec2, *rasvec1, *rasvec2, *m_xform = NULL ;
  ...
  if (m_xform)
    MatrixFree(&m_xform) ;
  MatrixFree(&voxvec1) ; MatrixFree(&voxvec2) ; MatrixFree(&rasvec1) ; MatrixFree(&rasvec2) ;
  for (j = 0 ; j < nvolumes ; j++)
  {
...
static MRI *
estimate_T2star(MRI **mri_flash, int nvolumes, MRI *mri_PD, MATRIX **Mreg, LTA *lta)
{
...
static MRI *
compute_T2star_map(MRI **mri_flash, int nvolumes, int *scan_types, MATRIX **Mreg, LTA *lta)
{
  ...
  for (x = 0 ; x < width ; x++)
  {
	...
	for (e = 0 ; e < nvolumes ; e++)
	{
	  ...
	  if (m_xform)
	  {
	    /* map from output voxel-space to voxel-space of synth (T1, PD) */ 
