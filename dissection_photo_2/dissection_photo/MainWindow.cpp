...
bool MainWindow::eventFilter(QObject *watched, QEvent *event)
{
  if (watched == ui->widgetDropImages || watched == ui->widgetDropCalibration)
  {
    if (event->type() == QEvent::DragEnter)
    {
      ...
      if (!urls.isEmpty())
      {
        if (urls.last().scheme().toLower() == "file")
        {
    ...
    else if (event->type() == QEvent::Drop)
    {
      ...
      if (!urls.isEmpty())
      {
        if (urls.last().scheme().toLower() == "file")
        {
          ...
          QMessageBox::warning(this, "Error", "Please drop a folder or files");
      }
    }
  }
...
void MainWindow::SetInputFolder(const QString &path, const QString& root_output)
{
  m_strInputFolder = path;
  if (path.isEmpty())
  {
  ...
  {
    if (root_output.isEmpty())
      m_strRootOutputFolder = path;
    else
      m_strRootOutputFolder = root_output;

    ui->labelImageFolder->setText(path + "<br /><a href='clear'>Clear</a><br /><br />" + "Output path: " +
                                  m_strRootOutputFolder + "<br /><a href='change'>Change</a>");
  }
}

void MainWindow::SetCalibrationFile(const QString &path)
{
  m_strCalibrationFile = path;
  if (path.isEmpty())
  {
...
{
  QString path = m_strRootOutputFolder + "/connected_components";
  QDir().mkpath(path);
  if (!path.isEmpty())
  {
    QStringList fns;
    foreach (QFileInfo fi, m_listInputFiles)
      fns << fi.absoluteFilePath();
    ui->listWidgetThumbnail->LoadImageFiles(fns);
...
{
  if (sender() == ui->labelCalibrationFile)
  {
    SetCalibrationFile("");
  }
  else if (sender() == ui->labelImageFolder)
  {
    if (link == "clear")
      SetInputFolder("");
    else if (link == "change")
    {
      QString path = QFileDialog::getExistingDirectory(this, "Select Folder", m_strRootOutputFolder);
      if (!path.isEmpty())
      {
        SetInputFolder(m_strInputFolder, path);
      }
    }
  }
}
...
{
  QString path = QFileDialog::getOpenFileName(this, "Select Calibration File", "");
  if (!path.isEmpty())
  {
...
{
  QString path = m_strRootOutputFolder + "/mask";
  QDir().mkpath(path); // QFileDialog::getExistingDirectory(this, "Select Folder", ui->lineEditPathOutput->text());
  if (!path.isEmpty())
  {
    ...
    ui->listWidgetThumbnail->Clear();
    QStringList fns, mask_fns;
    foreach (QFileInfo fi, m_listInputFiles)
      fns << fi.absoluteFilePath();
    if (m_listInputFiles.size() == m_listMaskFiles.size())
    {
      foreach (QFileInfo fi, m_listMaskFiles)
        mask_fns << fi.absoluteFilePath();
    }
    ui->listWidgetThumbnail->LoadImageFiles(fns, mask_fns);
...
{
  m_strOutputFolder = m_strRootOutputFolder + "/deformed";
  if (!QFileInfo::exists(m_strOutputFolder))
    QDir().mkpath(m_strOutputFolder);
  ...
  if (m_listInputFiles.isEmpty())
  {
    QMessageBox::warning(this, "Error", "No image files were found in " + m_strInputFolder);
    return;
  }
  ...
  m_bCalibratedMode = !m_strCalibrationFile.isEmpty();
  ui->labelTitle->setText(m_bCalibratedMode?"Calibrated":"Retrospective Correction");
  ui->widgetPointMode->setVisible(!m_bCalibratedMode);

  ui->frameBottomBar->show();
  ui->frameTopBar->show();
  ui->widgetIndex->show();
  ui->widgetIndexSeg->hide();
  ui->widgetIndexCC->hide();
...
void MainWindow::OnTogglePointMode(bool b)
{
  ...
  ui->labelRectWidth->setText(b2Points?"Ruler width (mm)":"Box width (mm)");
...
void MainWindow::UpdateIndex()
{
  ...
  else if (ui->stackedWidget->currentWidget() == ui->pageCC)
  {
    ...
    ui->pushButtonNextCC->setEnabled(m_nIndex < m_listInputFiles.size()-1); // m_listRegionData.size());
  }
  ui->listWidgetThumbnail->blockSignals(true);
  ui->listWidgetThumbnail->SetCurrentIndex(m_nIndex);
  ui->listWidgetThumbnail->blockSignals(false);
  label->setText(tr("%1 / %2").arg(m_nIndex+1).arg(m_listInputFiles.size()));
}

void MainWindow::LoadImage(int n, bool bNoProcessing, bool bPreview)
{
  QList<QPoint> pts;
  ...
  if (ui->stackedWidget->currentWidget() == ui->pageCorrection)
  {
    if (m_listPointData.contains(n))
      pts = m_listPointData[n];
    if (bPreview)
    {
      QString before = "."+QFileInfo(fn).completeSuffix();
      QString fn_p = QFileInfo(m_strOutputFolder, QFileInfo(fn).fileName().replace(before, "_deformed"+before)).absoluteFilePath();
      if (QFile::exists(fn_p))
      {
        fn = fn_p;
        pts.clear();
      }
  ...
  {
    mask_fn = m_listMaskFiles[n].absoluteFilePath();
    if (m_listRegionData.contains(n))
  ...
  if (ui->stackedWidget->currentWidget() == ui->pageCorrection)
  {
    if (pts.isEmpty() && !m_strCalibrationFile.isEmpty())
    {
      QStringList cmd;
  ...
  {
    QString fn = m_listMaskFiles[n].fileName();
    fn.replace(QString(".")+QFileInfo(mask_fn).suffix(), ".npz");
    fn = QFileInfo(m_sTempDir, fn).absoluteFilePath();
    if (QFile::exists(fn) && !m_maskProcessor.Load(fn))
    {
      QMessageBox::warning(this, "Error", "Failed to load processed npz files");
      return;
    }
...
void MainWindow::RunRetrospectiveCorrection(const QString &infile, const QList<QPoint> &pts, double w, double h)
{
  QStringList strList;
  foreach (QPoint pt, pts)
...
void MainWindow::OnProcessFinished()
{
  ...
  else if (task == "fiducials_detection")
  {
    ...
    if (list.size() >= 6)
    {
      ...
      m_listPointData.clear();
      for (int i = 0; i < m_listInputFiles.size(); i++)
        m_listPointData[i] = pts;

      if (m_listInputFiles.size() > 0)
      {
        if (m_listInputFiles.size() > 1)
        {
          info["next_index"] = 1;
          info["prev_index"] = 0;
          m_proc->setProperty("next_task_info", info);
        }
        RunRetrospectiveCorrection(m_listInputFiles.first().absoluteFilePath(), pts, info["width"].toDouble(), info["height"].toDouble());
      }
    }
  ...
  else if (task == "retrospective_correction")
  {
    QVariantMap task_info = m_proc->property("next_task_info").toMap();
    if (m_bCalibratedMode && task_info.contains("next_index"))
    {
      int n = task_info["next_index"].toInt();
      if (n < m_listInputFiles.size())
      {
        task_info["next_index"] = n+1;
        m_proc->setProperty("next_task_info", task_info);
        RunRetrospectiveCorrection(m_listInputFiles[n].absoluteFilePath(), m_listPointData[n], task_info["width"].toDouble(), task_info["height"].toDouble());
      }
      else
      {
        task_info.clear();
...
void MainWindow::OnButtonProceedToCC()
{
  ...
  m_listInputFiles = QDir(m_strOutputFolder).entryInfoList(QDir::Files, QDir::Name);
  ui->widgetIndex->hide();
  ui->widgetIndexCC->show();
  ui->widgetIndexSeg->hide();
  OnButtonOutputFolderCC();
}

void MainWindow::OnButtonProceedToRecon()
{
  ui->stackedWidget->setCurrentWidget(ui->pagePhotoRecon);
  m_nIndex = 0;
  ui->frameTopBar->hide();
  ui->frameBottomBar->hide();
}
...
void MainWindow::OnThumbnailIndexChanged(int n)
{
  if (n >= 0)
  {
    if (ui->stackedWidget->currentWidget() == ui->pageSegEdit)
    {
      if (ui->widgetImageView->SaveMaskIfEdited())
      {
        QImage img = ui->widgetImageView->GetImageInView();
        if (!img.isNull() && m_nIndex < ui->listWidgetThumbnail->count())
...
void MainWindow::OnButtonTogglePreview(bool b)
{
  if (ui->stackedWidget->currentWidget() == ui->pageCorrection && m_nIndex >= 0)
  {
    LoadImage(m_nIndex, true, b);
  }
}
