/* $Header: /space/repo/1/dev/dev/tiff/tif_packbits.c,v 1.1.2.2 2009/08/03 00:56:00 nicks Exp $ */
...
PackBitsPreEncode(TIFF* tif, tsample_t s)
{
	 ...
	 */
	if (isTiled(tif))
		tif->tif_data = (tidata_t)(unsigned long) TIFFTileRowSize(tif);
	else
		tif->tif_data = (tidata_t)(unsigned long) TIFFScanlineSize(tif);
	return (1);
}
