 ...
 for ( ; argc > 1 && ISOPTION(*argv[1]) ; argc--, argv++)
  ...
  if (!mris)
	...
	{ 
		...
		t = 0; ratio = 1; energy_old = energy_new = 0;
		...
		{ 
			...
			energy_quadcur = 0;	energy_label = 0;	 energy_Tspring = 0; 
			energy_Nspring = 0;  energy_repulse = 0; energy_Gspring = 0;
		...
		MRIfree(&mri_smooth[0]); MRIfree(&mri_smooth[1]); 
		MRIfree(&mri_smooth[2]); MRIfree(&mri_smooth[3]); MRIfree(&mri_smooth[4]);
	...
	MRISfree(&mris);
	MRIfree(&mri_label[0]); MRIfree(&mri_label[1]); MRIfree(&mri_orig);
	MRIfree(&mri_label[2]); MRIfree(&mri_label[3]); MRIfree(&mri_label[4]);
	exit(0) ;
	return(0) ;
}
...
mrisExaminemovelength(MRI_SURFACE *mris)
{
  ...
  VERTEX        *v;
	...
	for (k=0;k<mris->nvertices;k++)
	{
		...
		x = v->x - v->origx;	y = v->y - v->origy; 	z = v->z - v->origz;   //the movement it has made in past 
		dx = step_size*v->dx; dy = step_size*v->dy; dz = step_size*v->dz; // the movement it's going to make
		mag = MAX_mag*MAX_mag;
		...
		v->x += dx ; v->y += dy ; v->z += dz ;                  //then make it to 1mm
#else
  ...
  VERTEX   *vertex, *vn ;
	...
	for (i=0; i<niter; i++)
	{
		for (vno = 0 ; vno < mris->nvertices ; vno++)
		{
			...
			if (vertex->marked ==1)
			{
				num=0; x = vertex->x ; y = vertex->y ; z = vertex->z ;
		...
		for (vno = 0 ; vno < mris->nvertices ; vno++)
		{
			...
			if (vertex->marked == 1)
			{
				vertex->x = vertex->tdx ; vertex->y = vertex->tdy ; vertex->z = vertex->tdz ;
			}
		}
#if 1	
		for (j=0; j<2; j++)
