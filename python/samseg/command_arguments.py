...
def parse_sbtiv_args(argv=None, parser=argparse.ArgumentParser()):
    ...
    parser.add_argument('-m', '--map',  metavar="FILE", help="file containing a list over intracranial structure labels. Label names must be identical to those defined in the atlas used by samseg. A default is used, if omitted. ")
    parser.add_argument('-v', '--verbose', action='store_true', default=False, help="verbose debug output")    
