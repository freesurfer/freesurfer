...
fsa_surf = fs.Surface.read(
    os.path.join(fsdir, "fsaverage", "surf", hemi + ".sphere.reg")
)
...
func_dir = os.path.join(
    "bold", "bold.self.sm0.%s.lang" % hemi, contrast_name, "t.nii.gz"
)
...
pad = 8
smooth_steps = (
    200  # apply Gaussian smoothing on the surface (sigma~sqrt(smoothing))
)
if "spheres" not in locals() and "spheres" not in globals():
    (
        ...
        spheres,
        snames,
    ) = fsd.surf_utils.load_func_and_spheres(
        spaths,
        base_dir,
        func_dir,
        sdir1,
...
warp_downsize = 2
fgen = fsd.surf_utils.mrisp_semi_gen(
    mrisps_func,
    mrisps_geom,
...
ggen = fsd.surf_utils.mrisp_semi_gen(
    mrisps_geom,
    mrisps_func,
    batch_size=batch_size,
    use_rand=False,
...
warp_sphere_loss = fsd.losses.spherical_loss(
    tuple(np.array(mrisp_shape_nopad) // warp_downsize), pad=pad // 2
)
sphere_loss = fsd.losses.spherical_loss(
    mrisp_shape_nopad, pad=pad, threshold=threshold, win=[41, 41]
)
if which_semi == "NCC":
...
for func_wt in func_wt_list:
    ...
    print(
        "training with %s loss weight %2.2f and which_semi = %s, geom_wt=%2.2f, warp_smooth_wt=%2.2f"
        % (
            which_training,
            func_wt_this_round,
            which_semi,
            geom_match_wt_this_round,
            warp_smooth_wt,
        )
    )
    ...
    with strategy.scope():
        ...
        losses = [
            semi_loss,
            warp_sphere_loss.gradientLoss(penalty="l2"),
            semi_loss,
        ]
        if which_training == "sup":
            loss_weights = [
        ...
        fhist = model_semi.fit(
            gen, epochs=2000, steps_per_epoch=100, callbacks=callbacks
        )
    ...
    if 0:
        model_fsemi = vxm.networks.VxmDenseSemiSupervisedSeg(
            mrisp_shape,
            1,
            nb_unet_features=unet_nfeats,
            int_downsize=1,
        ...
        model_fsemi.compile(
            optimizer=keras.optimizers.Adam(lr=1.0 * lr),
            loss=losses,
            loss_weights=loss_weights,
        )
        fhist2 = model_fsemi.fit(
            fgen, epochs=200, steps_per_epoch=100, callbacks=callbacks
        )
        plt.figure()
    ...
    if 0:
        ...
        fv.vol(
            moving_func_warped,
            name="moving_func_warped_%2.2f" % func_wt,
            colormap="heat",
            visible="0",
        )
        fv.vol(fixed_func, name="func_fixed", colormap="heat", visible="0")
        fv.vol(
            (moving_func + fixed_func) / 2,
            name="avg_b4",
            colormap="heat",
            visible="1",
        )
        fv.vol(
            (moving_func_warped + fixed_func) / 2,
            name="avg_after",
            colormap="heat",
            visible="1",
        )
    ...
    for sno, sname in enumerate(tqdm(snames)):
        ...
        warped_geom = trans_model.predict(
            [mrisps_geom[sno][np.newaxis], avg_warp[np.newaxis]]
        )
        warped_func = trans_model.predict(
            [mrisps_func[sno][np.newaxis], avg_warp[np.newaxis]]
        )
    ...
    fv = fs.Freeview()
    fv.vol(
        np.transpose(np.array(avg_geoms[0:20]), (2, 3, 4, 0, 1)), name="geom"
    )
    fv.vol(
        np.transpose(np.array(avg_funcs[0:20]), (2, 3, 4, 0, 1)),
        name="func_%2.2f" % func_wt,
        colormap="heat",
        opts=":heatscale=2,3",
    )
    fv.show()
    ...
    fs.Image(mean_geom).write(
        "%s.d%d.func_wt%2.2f.warp_wt%2.2f.smooth%d.mean_geom.mrisp.mgz"
        % (hemi, warp_downsize, func_wt, warp_smooth_wt, smooth_steps)
    )
    fs.Image(std_geom).write(
        "%s.d%d.func_wt%2.2f.warp_wt%2.2f.smooth%d.std_geom.mrisp.mgz"
        % (hemi, warp_downsize, func_wt, warp_smooth_wt, smooth_steps)
    )
    fs.Image(mean_func).write(
        "%s.d%d.func_wt%2.2f.warp_wt%2.2f.smooth%d.mean_func.mrisp.mgz"
        % (hemi, warp_downsize, func_wt, warp_smooth_wt, smooth_steps)
    )
    fs.Image(std_func).write(
        "%s.d%d.func_wt%2.2f.warp_wt%2.2f.smooth%d.std_func.mrisp.mgz"
        % (hemi, warp_downsize, func_wt, warp_smooth_wt, smooth_steps)
    )
...
for ono, wt in enumerate(func_wt_list):
    geom_overlay = fsa_surf.sample_parameterization(
        mean_geoms[ono][pad:-pad, :]
    )
    func_overlay = fsa_surf.sample_parameterization(
        mean_funcs[ono][pad:-pad, :]
    )
    func_tag = fv.OverlayTag(
        func_overlay, name="func_%2.2f" % wt, threshold="2,3"
    )
    geom_tag = fv.OverlayTag(
        geom_overlay, name="geom_%2.2f" % wt, threshold="2,3"
    )
...
fv.surf(
    fsa_inf,
    overlay=geom_tag_list,
    curvature=geom_overlay,
    opts=":overlay_threshold=2,3",
)
fv.show()
