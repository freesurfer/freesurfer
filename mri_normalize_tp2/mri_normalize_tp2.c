...
int main(int argc, char *argv[])
{
  ...
  if (!mri_ctrl)
    ErrorExit(ERROR_BADPARM, "%s: could not read control points volume for tp1 %s",
  ...
  if(mask1_fname){
    ...
    if(!mri_mask1)
      ErrorExit(ERROR_BADPARM, "%s, could not read mask volume for tp1 %s", Progname, mask1_fname);
    mask1_set = 1;
    if((mri_mask1->width != mri_in->width) ||
       (mri_mask1->height != mri_in->height) ||
       (mri_mask1->depth != mri_in->depth)       
       )
      ErrorExit(ERROR_BADPARM, "%s:  mask volumes have different sizes than other volumes \n", Progname);
    
  }
  if(mask2_fname){
    mri_mask2 = MRIread(mask2_fname) ;
    if(!mri_mask2)
      ErrorExit(ERROR_BADPARM, "%s, could not read mask volume for tp2 %s", Progname, mask2_fname);
    mask2_set = 1;
  ...
  if(xform_fname != NULL){
    ...
    mri_tmp = MRIalloc(mri_ctrl->width, mri_ctrl->height, mri_ctrl->depth, mri_ctrl->type) ;
    ...
    if(mask1_fname != NULL && mask2_fname == NULL){
      ...
      mri_mask2 = MRIalloc(mri_in->width, mri_in->height, mri_in->depth, mri_mask1->type) ;
	...
	MRIwrite(mri_mask2, "mri_mask2.mgz");
      ...
      mri_mask1 = MRIalloc(mri_T1->width, mri_T1->height, mri_T1->depth, mri_mask2->type) ;
  ...
  if(mask1_set == 0){
    //create mask1 
    mri_mask1 = MRIalloc(mri_T1->width, mri_T1->height, mri_T1->depth, MRI_UCHAR) ;
    for(z=0; z < depth; z++)
      for(y=0; y< height; y++)
	for(x=0; x < width; x++)
	  {
  ...
  if(mask2_set == 0){
    //create mask2 
    mri_mask2 = MRIalloc(mri_in->width, mri_in->height, mri_in->depth, MRI_UCHAR) ;
    for(z=0; z < depth; z++)
      for(y=0; y< height; y++)
	for(x=0; x < width; x++)
	  {
  ...
  for(z=0; z < depth; z++)
      ...
      for(x=0; x < width; x++)
	{
	  ...
	  for(i=-1; i<=1; i++)
	    for(j=-1; j<=1; j++)
	      for(k=-1;k<=1;k++){
		cx = x+i; cy = y+j, cz = z+k;
		
		if(cx < 0 || cx >= width || cy < 0 || cy >= height || cz < 0 || cz >= depth) continue;
		count++;
  ...
  for(num = 0.0f, b = h->nbins - 1; b >= 1; b --){
    num += h->counts[b];
    if(num > 20000) // this may make me only use WM points, is it good to use only WM to compute scale of intensity??
      break;
  }
  
  printf("using SNR threshold %2.3f at bin %d\n", h->bins[b], b);

  mean1 = 0; std1 = 0; count1 = 0;
  for(z=0; z < depth; z++)
    for(y=0; y< height; y++)
      for(x=0; x < width; x++)
	{
  ...
  //somehow mri_watershed seems to leave some unzero voxels around image border, so I will skip image boundaries
  //no, that's not a problem of most recent mri_watershed; something wrong previously
  mean2 = 0; std2 = 0; count2 = 0;
  for(z=0; z < depth; z++)
    for(y=0; y< height; y++)
      for(x=0; x < width; x++)
  ...
  //  mri_snr = MRIalloc(mri_ctrl->width, mri_ctrl->height, mri_ctrl->depth, MRI_FLOAT) ;
...
void usage(int exit_val)
{
  ...
  fprintf(fout, "this program uses the controls points of tp1 to help normalize tp2 \n") ;
  fprintf(fout, "Options are: \n") ;
  fprintf(fout, "\t -T1 %%s: T1 volume for tp1 (actually the volume to perform mri_normalize on for tp1) \n");
  fprintf(fout, "\t -mask1 %%s: brain mask for tp1 (will be mapped to tp2 by applying the xform if -mask2 not specified) \n");
  fprintf(fout, "\t -mask2 %%s: brain mask for tp2 (will be mapped to tp1 by applying inverse-xform if -mask1 not specified) \n");
  fprintf(fout, "\t -threshold %%d: threshold for background (default = 1.0) \n");
...
get_option(int argc, char *argv[])
{
  ...
  if (!stricmp(option, "debug_voxel"))
    ...
    {
      mask1_fname  = (argv[2]);
      printf("Use volume %s as brain mask for tp1, and will be mapped to tp2 to get rough mask for tp2 in computing intensity normalization \n", mask1_fname) ;
      nargs = 1;
    }
  else if (!stricmp(option, "mask2"))
    {
      mask2_fname  = (argv[2]);
      printf("Use volume %s as brain mask for tp2, and will be mapped to tp1 to get rough mask for tp1 in computing intensity normalization \n", mask2_fname) ;
      nargs = 1;
    }
