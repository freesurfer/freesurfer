...
::Initialize()
{
  ...
  std::vector<CroppedImageReader::Pointer> readers;
  ...
  {  
    ...
    {
      reader->Read( m_ImageFileNames[c].c_str() );
    }
    else
    {
      reader->Read( m_ImageFileNames[c].c_str(), m_BoundingFileName.c_str() );
    }
  ...
  m_OriginalImageCropRegion = readers[0]->GetOriginalImageRegion();
  m_OriginalImageOriginalRegion = readers[0]->GetOriginalImageOriginalRegion();

  // Remember the world-to-image transform of the cropped image, where the first voxel
  // is considered having index (0,0,0)^T
  m_WorldToImageTransform->Compose( readers[0]->GetWorldToImageTransform() );
  ...
  m_Segmenter->SetMeshToImageTranform( readers[0]->GetTransform() );

  const float  determinant = vnl_det( readers[0]->GetTransform()->GetMatrix().GetVnlMatrix() );
  if ( determinant < 0 )
  {
    std::cout << "Careful here: the applied transformation will turn positive tetrahedra into negative ones." << std::endl;
    std::cout << readers[0]->GetTransform()->GetMatrix().GetVnlMatrix() << std::endl;
  ...
  for(int c=0; c<m_n_channels; c++)
  {
    ...
    char aux[5000];
    sprintf(aux,"%s/originalImageBeingSegmented_%d.nii.gz",m_LogDirectory.c_str(),c);
    imageWriter->SetFileName(aux);
    imageWriter->SetInput( readers[c]->GetImage() );
    imageWriter->Update();
...
//   m_NonDownsampledImages.clear();
  m_NonDownsampledImages.resize(m_n_channels);
  for(int c=0; c<m_n_channels; c++)
  {
    typedef CroppedImageReader::ImageType  ImageType; 

    ImageType::Pointer  image = const_cast< ImageType* >( readers[c]->GetImage() );
    ...
    m_NonDownsampledImages[c] = image;
  ...
  m_Segmenter->SetImage( c, image ); 

  } // end of image channel loop 


  // Apply downsampling also to the mesh collection if necessary
  if ( m_DownSamplingFactor > 1 )
  {
  ...
  for(int c=0; c<m_n_channels; c++)
  {
    ...
    ImageWriterType::Pointer  imageWriter = ImageWriterType::New();
    char aux[5000];
    sprintf(aux,"%s/imageBeingSegmented_%d.nii.gz",m_LogDirectory.c_str(),c);
    imageWriter->SetFileName(aux);
    imageWriter->SetInput( m_Segmenter->GetImage(c) );
    imageWriter->Update();
  }
...
::Segment( bool useAffine )
{
  ...
  std::vector<AtlasMeshSegmenter::ImageType::Pointer> unsmoothedImages;
  for(int c=0; c<m_n_channels; c++){
    unsmoothedImages.push_back(m_Segmenter->GetImage(c));
  }
        ...
        m_MultiResolutionLevel++ )
  {
    ...
    for(int c=0; c<m_n_channels; c++)
    {
      AtlasMeshSegmenter::ImageType::Pointer  smoothedImage = unsmoothedImages[c];
      ...
      if ( imageSigma != 0 ) 
      {
        ...
        caster->SetInput( unsmoothedImages[c] );
        SmootherType::Pointer smoother = SmootherType::New();
      ...
      m_Segmenter->SetImage( c, smoothedImage );
    }
...

  {
    ...
    writer->SetFileName( ( m_LogDirectory + "mappedSummary.nii.gz" ).c_str() );
    writer->Write();
    writer->SetInput( this->GetSummaryImage( false, true ) );
    writer->SetFileName( ( m_LogDirectory + "mappedSummaryCrisp.nii.gz" ).c_str() );
    writer->Write();
  }
  ...
  WriterType::Pointer  writer = WriterType::New();
  writer->SetInput( this->GetSummaryImage() ); 
  writer->SetFileName( ( m_LogDirectory + "summary.nii.gz" ).c_str() );
  writer->Write();
  writer->SetInput( this->GetSummaryImage( false, true ) );
  writer->SetFileName( ( m_LogDirectory + "summaryCrisp.nii.gz" ).c_str() );
  writer->Write();
  ...
  for(int c=0; c<m_n_channels; c++)
  {
    ...
    biasCorrectedWriter->SetInput( m_Segmenter->GetEMSegmenter()->GetBiasCorrectedImage(c) );
    char aux[5000];
    sprintf(aux,"%s/biasCorrectedImage_%d.nii.gz",m_LogDirectory.c_str(),c);
    biasCorrectedWriter->SetFileName( aux );
  ...
  for(int c=0; c<m_n_channels; c++)
  {
    ...
    char aux[5000];
    sprintf(aux,"%s/exponentiatedBiasCorrectedImage_%d.nii.gz",m_LogDirectory.c_str(),c);
    exponentiatedWriter->SetFileName( aux );
    exponentiatedWriter->SetInput( AtlasMeshSegmenterConsole::GetExponentiatedImage( m_Segmenter->GetEMSegmenter()->GetBiasCorrectedImage(c) ) );
    exponentiatedWriter->Write();
  }
#endif 

  std::vector<ImageType::Pointer> nativeImages;
  for(int c=0; c<m_n_channels; c++){
    ImageType::Pointer  nativeImage = 0;
    if ( m_Segmenter->GetEMSegmenter()->GetBiasField(c) )
    {
      ...
      biasWriter->SetInput( m_Segmenter->GetEMSegmenter()->GetBiasField(c) );
      char aux[5000];
      sprintf(aux,"%s/bias_%d.nii.gz",m_LogDirectory.c_str(),c);
      biasWriter->SetFileName( aux );
      biasWriter->Write();
      ...
      = m_Segmenter->GetEMSegmenter()->BiasCorrect( m_NonDownsampledImages[c], m_DownSamplingFactor, c ).GetPointer();
      typedef itk::ImageFileWriter< EMSegmenter::BiasCorrectedImageType >  BiasCorrectedWriterType;
std::cout<<"A"<<std::endl;
      BiasCorrectedWriterType::Pointer  biasCorrectedWriter = BiasCorrectedWriterType::New();
std::cout<<"B"<<std::endl;
...
std::cout<<"A"<<std::endl;
std::cout<<nonDownsampledBiasCorrectedImage<<std::endl;
std::cout<<"D"<<std::endl;
      sprintf(aux,"%s/nonDownsampledBiasCorrectedImage_%d.nii.gz",m_LogDirectory.c_str(),c);
std::cout<<"E"<<std::endl;
      biasCorrectedWriter->SetFileName( aux );
std::cout<<"F"<<std::endl;
      biasCorrectedWriter->Write();
std::cout<<"G"<<std::endl;
      ...
      sprintf(aux,"%s/nonDownsampledBiasCorrectedImageNonFloat_%d.nii.gz",m_LogDirectory.c_str(),c);
      nativeWriter->SetFileName( aux );
      nativeWriter->Write();
    }
    else
    {
      nativeImage = m_NonDownsampledImages[c];
    }
  ...
  m_Segmenter->SetMesh( mesh );  // careful, this erases means and precisions from the EM segmenter!
  ...
  std::vector< vnl_vector<float> >  internalMeans( numberOfInternalLabels );
  std::vector< vnl_matrix<float> >  internalPrecisions( numberOfInternalLabels );

  for ( MoreToLessLabelsLookupTableType::const_iterator  it = m_InternalToReducedLookupTable.begin();
        it != m_InternalToReducedLookupTable.end(); ++it )
  {

    internalMeans[ it->first ] = m_Segmenter->GetMeans()[ it->second ]; 
    internalPrecisions[ it->first ] = m_Segmenter->GetPrecisions()[ it->second ];

/*  setmesh a couple of lines above erases means and precision matrices
    internalMeans[ it->first ] = myAbusedEMSegmenter->GetMeans()[ it->second ]; 
    internalPrecisions[ it->first ] = myAbusedEMSegmenter->GetPrecisions()[ it->second ];
*/
  }
  std::vector< float >  internalPriorWeights( numberOfInternalLabels, 1.0f );

  //
  const_cast< std::vector< vnl_vector<float> >& >( myAbusedEMSegmenter->GetMeans() ) = internalMeans;
  const_cast< std::vector< vnl_matrix<float> >& >( myAbusedEMSegmenter->GetPrecisions() ) = internalPrecisions;
  ...
  std::cout << "myAbusedEMSegmenter->GetPrecisions(): [";
  for ( unsigned int i = 0; i < myAbusedEMSegmenter->GetPrecisions().size(); i++ )
  {
    std::cout << " " << myAbusedEMSegmenter->GetPrecisions()[ i ];
  }
  std::cout << "]" << std::endl;
  ...
  myAbusedEMSegmenter->SetImagesAndSegmentItWithCurrentParametersAndWithoutBiasFieldCorrection( nativeImages ); 
  ...
  // Eugenio: write discrete labels  (I don't like the original code  below, it resamples to 512x512...)
  ...
  // Allocate empty image
  typedef  itk::Image< int, 3 >  DiscreteLabelImageType;
  DiscreteLabelImageType::Pointer  discreteLabels = DiscreteLabelImageType::New();
  discreteLabels->SetRegions( myAbusedEMSegmenter->GetPosterior( 0 )->GetLargestPossibleRegion().GetSize() );
  discreteLabels->Allocate();

  // Set spacing, origin, and direction
  discreteLabels->SetSpacing( posteriorSpacing );
  discreteLabels->SetOrigin( posteriorOrigin );
  discreteLabels->SetDirection( posteriorDirection );

  // Iterators
  itk::ImageRegionIterator< DiscreteLabelImageType >  targetIt( discreteLabels, discreteLabels->GetBufferedRegion() );
  std::vector<itk::ImageRegionConstIterator< EMSegmenter::ClassificationImageType > > sourceIts(myAbusedEMSegmenter->GetNumberOfClasses());
  for(unsigned int i = 0; i < myAbusedEMSegmenter->GetNumberOfClasses(); i++)
  {
    itk::ImageRegionConstIterator< EMSegmenter::ClassificationImageType >
    sit( myAbusedEMSegmenter->GetPosterior( i ),
              myAbusedEMSegmenter->GetPosterior( i )->GetBufferedRegion() );
    sourceIts[i]=sit;
  }
  ...
  float max=0;
  float val=0;
  DiscreteLabelImageType::PixelType label = 0;
  unsigned char  externalLabel=0;
  bool found;
  for ( ; !targetIt.IsAtEnd(); ++targetIt )
  {
    ind_max = -1;
    max = 0; 
    for(unsigned int i = 0; i < myAbusedEMSegmenter->GetNumberOfClasses(); i++)
    {
      val = sourceIts[i].Value();
      if(val>max)
      {
        ind_max=i;
        max=val;
    ...
    if(ind_max!=-1)
    {
      label = (DiscreteLabelImageType::PixelType) (inverseLookupTable[ static_cast< CompressionLookupTable::CompressedImageType::PixelType >( m_InternalToExternalLookupTable.find( ind_max )->second ) ]);
    }
    else
      label=0;

    if(label<100 && label!=17 && label!=53) label=0;  // we don't write GM/WM/CSF

    targetIt.Value() = label;
  }
  typedef itk::ImageFileWriter< DiscreteLabelImageType >  DiscreteLabelWriterType;
  DiscreteLabelWriterType::Pointer  writer2 = DiscreteLabelWriterType::New();
  std::ostringstream  fileNameStream2;
  fileNameStream2 << m_LogDirectory << "discreteLabels.mgz";
  std::cout << "Writing discrete labels to " << fileNameStream2.str() <<std::endl;
  writer2->SetFileName( fileNameStream2.str() );
  writer2->SetInput( discreteLabels );
  writer2->Write();
  // End of writing discrete labels
  ...
  if ( myAbusedEMSegmenter->GetSuperResolutionPosteriors() )
  {
    ...
    for(int c=0; c<m_n_channels; c++)
    {
      ...
      WriterType::Pointer  writer = WriterType::New();
      writer->SetInput( myAbusedEMSegmenter->GetSuperResolutionImage(c) ); 
      char aux[5000];
      sprintf(aux,"%s/superResolutionImage_%d.nii.gz",m_LogDirectory.c_str(),c);
  ...
  writer->SetInput( this->GetSummaryImage() );
  writer->SetFileName( ( m_LogDirectory + "nonDownsampledSummary.nii.gz" ).c_str() );
  writer->Write();
  writer->SetInput( this->GetSummaryImage( false, true ) );
  writer->SetFileName( ( m_LogDirectory + "nonDownsampledSummaryCrisp.nii.gz" ).c_str() );
  writer->Write();
  writer->SetInput( this->GetSummaryImage( false, true, false ) );
  writer->SetFileName( ( m_LogDirectory + "nonDownsampledSummaryCrispNotDemangled.nii.gz" ).c_str() );
  writer->Write();
  ...
  freeSurferWriter->SetFileName( ( m_LogDirectory + "discreteLabels.mgz" ).c_str() );
...
AtlasMeshSegmentationDriver
::GetSummaryImage( bool  usePrior, bool  useCrisp, bool joinSplitClasses, const std::vector< vnl_vector<float> >&  means ) const
{
...
::ParseSetUpFile( const std::string& setUpFileName )
{
  ...
  while ( in.getline( buffer, size ) )
  {
    ...
    else if ( !key.compare( "imageFileName" ) )
    {
      ...
      std::string tmp;
      valueStream >> tmp;
      m_ImageFileNames.push_back(tmp);
      std::cout << "m_ImageFileNames[" << m_ImageFileNames.size()-1 << "]: " << m_ImageFileNames[m_ImageFileNames.size()-1] << std::endl;
    }
  ...
  m_n_channels = m_ImageFileNames.size();
