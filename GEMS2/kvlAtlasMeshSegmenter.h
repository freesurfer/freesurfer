...
public :
  ...
  // Set image
  void SetImage( int ind,  ImageType::Pointer  image );
  ...
  SummaryImageType::Pointer  GetSummaryImage( bool usePrior = false,
      bool useCrisp = false,
      const std::vector< vnl_vector<float> >&  means = std::vector< vnl_vector<float> >() ) const;
...
private:
  ...
  std::vector< InternalImageType::ConstPointer >  m_InternalImages; 
