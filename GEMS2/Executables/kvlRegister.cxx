...
int main( int argc, char* argv[] )
{
  ...
  if ( argc < 3 )
    {
    std::cerr << argv[0] << " inputImage referenceImage [ mode=MI(SSD/MI) degreesOfFreedom=6 initialTranslationMode=0 "
                         << "numberOfBins=20(doesn't matter for SSD) useDefaultSchedule=1 extraInputImage1 ... ]" << std::endl;
    return -1;
    }
  ...
  bool modeIsSSD = false;
  if ( argc > 3 )
    {
    const std::string  modeString = argv[ 3 ];
    if ( modeString.compare("SSD") == 0 )
      {
      modeIsSSD=true;
      }
    else if ( modeString.compare("MI") != 0 )
      { 
      std::cerr << "Mode must be SSD or MI" << std::endl; 
      return -1;
      }
    }
  ...
  if ( argc > 4 )
    {
    std::istringstream  degreesOfFreedomStream( argv[ 4 ] );
    degreesOfFreedomStream >> degreesOfFreedom;
    }
  int  initialTranslationMode = 0;
  if ( argc > 5 )
    {
    std::istringstream  initialTranslationModeStream( argv[ 5 ] );
    initialTranslationModeStream >> initialTranslationMode;
    }
  int  numberOfBins = 20;
  if ( argc > 6 )
    {
    std::istringstream  numberOfBinsStream( argv[ 6 ] );
    numberOfBinsStream >> numberOfBins;
    }
  bool  useDefaultSchedule = true;
  if ( argc > 7 )
    {
    std::istringstream  useDefaultScheduleStream( argv[ 7 ] );
  ...
  if ( argc > 8 )
    ...
    for ( int i = 8; i < argc; i++ )
      {
      extraInputImageFileNames.push_back( std::string( argv[ 8 ] ) );
      }
    }
  ...
  std::cout << "modeIsSSD: " << modeIsSSD << std::endl;
  ...
  try
  {
    ...
    registerer->SetModeIsSSD( modeIsSSD );
