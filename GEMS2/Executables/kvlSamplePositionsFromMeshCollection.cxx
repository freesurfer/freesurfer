...
int main( int argc, char** argv )
{
  ...
  if ( argc < 9 )
  {
    std::cerr << "Usage: " << argv[ 0 ]
              << " #nChannels meshFileName imageFileName numberOfSamples numberOfWarmupSweeps numberOfBetweenSamplesSweeps mass"
  ...
  // TODO Eugenio  this is a big hack so it still works with monomodal data...
  std::vector< vnl_vector<float> >  aux = parameterEstimator->GetMeans();
  std::vector<float> aux2(aux.size());
  for(int i=0; i<aux.size(); i++) 
    aux2[i]=aux[i][0];
  const std::vector< float >  means(aux2);
