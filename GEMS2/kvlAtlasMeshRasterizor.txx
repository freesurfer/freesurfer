...
struct sortableItem
  {
  sortableItem( const itk::Vector< double, 3 >& point, const itk::Vector< float, 4 >& pis )
    {
    m_Point = point;
    m_Pis = pis;
    } 

  itk::Vector< double, 3 >  m_Point;
...
::Rasterize( const AtlasMesh* mesh, bool useMultiThreading )
{
  ...
  if ( !useMultiThreading || ( this->GetNumberOfThreads() == 1 ) )
    {
    //
...
#else
                    float fr = FixedToChan( r );
                    float fg = FixedToChan( g );
                    float fb = FixedToChan( b );
                    float fa = FixedToChan( a );
                    if ( fr < 0 )
                      fr = 0;
                    if ( fg < 0 )
                      fg = 0;
