...
void
AtlasMeshSegmenter
::SetImage( int ind,  ImageType::Pointer  image )
{

  if(ind>=m_Images.size()){
    m_Images.resize(ind+1,0);
    m_InternalImages.resize(ind+1,0);
  }
  ...
  windower->Update();
  m_InternalImages[ind] = windower->GetOutput();
  ...
  backCaster->Update();
  m_Images[ind] = backCaster->GetOutput();
#else
  m_Images[ind] = image;
#endif

  m_EMSegmenter->SetImages( m_Images ); 
 
  m_n_channels=m_Images.size();
}
...
::UpdatePosition( bool useAffine )
{
  ...
  InternalImageType::ConstPointer  dummyTemplateImage = m_InternalImages[0];
  if ( m_UsePartialVoluming )
    {
    typedef itk::CastImageFilter< EMSegmenter::ImageType, InternalImageType >  CasterType;
    CasterType::Pointer  caster = CasterType::New();
    caster->SetInput( m_EMSegmenter->GetSuperResolutionImage(0) ); 
    caster->Update();
  ...
  if ( m_CoregisterToPosteriorProbabilities )
    ...
    if ( !m_UsePartialVoluming )
      {
      std::vector<EMSegmenter::BiasCorrectedImageType::Pointer> bcims; 
      for(int c=0; c<m_n_channels; c++)
        bcims.push_back(m_EMSegmenter->GetBiasCorrectedImage(c));
      levenbergMarquardt->SetImages( bcims );
      levenbergMarquardt->SetMeans( m_Means );
      levenbergMarquardt->SetPrecisions( m_Precisions );
      }
    else
      {
  ...
  // Similar set up for trialLevenbergMarquardt 
  ...
  if ( m_CoregisterToPosteriorProbabilities )
    ...
    if ( !m_UsePartialVoluming )
      ...
      for(int c=0; c<m_n_channels; c++)
        bcims.push_back(m_EMSegmenter->GetBiasCorrectedImage(c));
      trialLevenbergMarquardt->SetImages( bcims );
      trialLevenbergMarquardt->SetMeans( m_Means );
      trialLevenbergMarquardt->SetPrecisions( m_Precisions );
      }
    else
      {
      //levenbergMarquardt->SetLikelihoods( m_EMSegmenter->GetSuperResolutionLikelihoods() );
  ...
  for ( m_PositionUpdatingIterationNumber = 0; 
    ...
    while ( true )
      ...
      if ( trialCost < currentCost )
        ...
        AtlasMeshDeformationLevenbergMarquardt::Pointer  tmp = levenbergMarquardt;
        levenbergMarquardt = trialLevenbergMarquardt;
        trialLevenbergMarquardt = levenbergMarquardt;
...
::UpdateModelParameters()
{
  ...
  m_Precisions = m_EMSegmenter->GetPrecisions();
...
AtlasMeshSegmenter
::GetSummaryImage( bool  usePrior, bool  useCrisp, const std::vector< vnl_vector<float> >&  means ) const
{
  ...
  for ( unsigned int  classNumber = 0; classNumber < this->GetNumberOfClasses(); classNumber++ )
    ...
    if ( means.size() != 0 )
      {
      mean = means[ classNumber ][0]; // eugenio: use first channel (Freesurfer's nu.mgz) for now...
      }
    else
      {
      //mean = classNumber;
...
{ 
  ...
  zeroImage->SetRegions( m_InternalImages[0]->GetLargestPossibleRegion() );
  zeroImage->Allocate();
  ...
  */
  ...
  gradientCalculator->SetMeans( m_Means );
  gradientCalculator->SetPrecisions( m_Precisions );
