...
//Eugenio: these 2 functions are for compatibility with 1-d code...
const std::vector<float> 
EMSegmenter
::GetVariances()
  {
    float vars[100];
    for(int i=0; i<m_Precisions.size(); i++)
      vars[i]=1.0f/m_Precisions[i][0][0];

    std::vector<float> result (vars, vars + m_Precisions.size());
    return result; 
  }
...
void
EMSegmenter
::SetImage(ImageType::Pointer image)
{
  std::vector<ImageType::Pointer>  images(1);
  images[0]=image;
...
void
EMSegmenter  
::SetImages( std::vector<ImageType::Pointer>  images )
{

  m_Images = images;
  m_n_channels=images.size();

  m_BiasCorrectedImages.resize(m_n_channels);
  m_BiasFields.resize(m_n_channels);
  m_BiasFieldResidueSmoothers.resize(m_n_channels,0);
  ...
  for(int c=0; c<m_n_channels; c++)
  {
    ...
    caster->SetInput( m_Images[c] );
    caster->Update();
    m_BiasCorrectedImages[c] = caster->GetOutput();
  }
}
...
::Segment()
{
  ...
  if ( m_Images.size()==0 || !m_AtlasMesh )
  {
...
::SplitSharedClasses()
{
        ...
        originalClassNumber++ )
  {
    ...
    vnl_vector<float>  sharedMean = m_Means[ inverseLookupTable[ originalClassNumber ][ 0 ] ];
    vnl_matrix<float>  sharedVariance = vnl_inverse(m_Precisions[ inverseLookupTable[ originalClassNumber ][ 0 ] ]);
    vnl_vector<float>  sharedStdDevVec(sharedMean.size());
    for(int t=0; t<sharedMean.size(); t++)  sharedStdDevVec[t]=sqrt(sharedVariance[t][t]);
    vnl_vector<float>  minimumOfRange = sharedMean - 2.0f * sharedStdDevVec;
    vnl_vector<float>  maximumOfRange = sharedMean + 2.0f *  sharedStdDevVec;
    vnl_vector<float>  slope = ( maximumOfRange - minimumOfRange ) /
                         static_cast< float >( numberOfSharedClasses - 1 );
    vnl_vector<float>  offset = minimumOfRange;
...
::Initialize() const
{
  ...
  if ( m_NumberOfSubvoxels == 1 )
  {
    ...
    templateImage->SetRegions( m_Images[0]->GetLargestPossibleRegion().GetSize() );
    ...
    for ( unsigned int classNumber = 0; classNumber < m_NumberOfClasses; classNumber++ )
    {
      ...
      std::vector< itk::ImageRegionConstIterator< ImageType > > imageIts;
      for(int c=0; c<m_n_channels; c++)
      {
        itk::ImageRegionConstIterator< ImageType >  imageIt( m_Images[c], m_Images[c]->GetLargestPossibleRegion() ); 
        imageIts.push_back(imageIt);
      }
      itk::ImageRegionIterator< AtlasMeshAlphaDrawer::AlphaImageType >  priorIt( prior, prior->GetLargestPossibleRegion() );
      for ( ; !imageIts[0].IsAtEnd();  ++priorIt )
      {
        // if ( classNumber == 0 )
        //   {
        ...
        bool isInBuffer = true;
        for(int c=0; c<m_n_channels; c++)
        {
          if ( imageIts[c].Value() == 0 )
          {
            isInBuffer = false;
          }
          ++(imageIts[c]);
  ...
  else
  {
    ...
    for ( int i = 0; i < 3; i++ )
    {
      highResolutionSize[ i ] = m_Images[0]->GetBufferedRegion().GetSize()[ i ] * m_PartialVolumeUpsamplingFactors[ i ];
    }
    ...
    for(int c=0; c<m_n_channels; c++)
    {
      ImageType::ConstPointer  superResolutionImage = this->GetSuperResolutionImage(c).GetPointer();  
    ...
    m_SubvoxelPriorsImage->SetRegions( m_Images[0]->GetBufferedRegion().GetSize() );
    ...
    for ( unsigned int classNumber = 0; classNumber < m_NumberOfClasses; classNumber++ )
    {
      ...
      prior->SetRegions( m_Images[0]->GetLargestPossibleRegion().GetSize() );
      prior->Allocate();
      prior->FillBuffer( 0 );
...
::UpdatePosteriors()
{
  ...
  likelihoodImage->SetRegions( m_Images[0]->GetLargestPossibleRegion().GetSize() );
  likelihoodImage->Allocate();
  likelihoodImage->FillBuffer( 0 );

  // Eugenio: for the sake of efficiency, we consider 2 cases here: monomodal and multimodal
  // (we don't wanna be wrapping scalars in matrices if monomodal...)
  if(m_n_channels==1)
  {

    for ( unsigned int classNumber = 0; classNumber < m_NumberOfClasses; classNumber++ )
    {
      const float  mean = m_Means[ classNumber ][0];
      const float  variance = 1.0/m_Precisions[classNumber][0][0];
      const float  weight = m_PriorWeights[ classNumber ];
  
      BiasCorrectedImageType::ConstPointer  imageToUse = m_BiasCorrectedImages[0].GetPointer();
  #if 0
  ...
  else  // multimodal case
  {
    ...
    for ( unsigned int classNumber = 0; classNumber < m_NumberOfClasses; classNumber++ )
    {
      const vnl_vector<float>  mean = m_Means[ classNumber ];
      const vnl_matrix<float>  precision = m_Precisions[classNumber];
      const float  weight = m_PriorWeights[ classNumber ];
      const float OneOverSqrtDetCov = sqrt(vnl_determinant(precision));

      std::vector<itk::ImageRegionConstIterator< BiasCorrectedImageType > > imageIts;
      for(int c=0; c<m_n_channels; c++)
      {
        itk::ImageRegionConstIterator< BiasCorrectedImageType >  imageIt( m_BiasCorrectedImages[c].GetPointer(),
          m_BiasCorrectedImages[c]->GetLargestPossibleRegion() );
        imageIts.push_back(imageIt);
      }
      itk::ImageRegionConstIterator< ClassificationImageType >
      priorIt( m_Priors[ classNumber ], m_Priors[ classNumber ]->GetLargestPossibleRegion() );
      ...
      itk::ImageRegionIterator< ClassificationImageType >
      likelihoodIt( likelihoodImage, likelihoodImage->GetLargestPossibleRegion() );
      vnl_vector<float> dataV(m_n_channels);
      for ( ; !imageIts[0].IsAtEnd(); ++priorIt, ++posteriorIt, ++likelihoodIt )
      {
        for(int c=0; c<m_n_channels; c++)
          dataV[c]=imageIts[c].Value()-mean[c];
        
        const float  unnormalizedPosterior = exp( -0.5 * dot_product(dataV,precision*dataV)) * OneOverSqrtDetCov * weight * priorIt.Value();
        posteriorIt.Value() = unnormalizedPosterior;
        likelihoodIt.Value() += unnormalizedPosterior;
...
::UpdateBiasFieldModelParameters( int biasFieldOrderUsed ) 
{
  ...
  // bias field correct every channel independently (maybe a bit ghetto, but whatever...)
  for(int c=0; c<m_n_channels; c++)
  {

  // Set up residue smoother if necessary
  if ( !m_BiasFieldResidueSmoothers[c] )
  {  
      ...
      maskImage->SetRegions( m_Images[c]->GetBufferedRegion() );
      maskImage->Allocate();
      ...
      m_BiasFieldResidueSmoothers[c] = ImageSmoother::New();
      m_BiasFieldResidueSmoothers[c]->SetMaskImage( maskImage );
      m_BiasFieldResidueSmoothers[c]->SetPolynomialOrder( m_BiasFieldOrder );
    } 
    ...
    residueImage->SetRegions( m_Images[c]->GetBufferedRegion() );
    ...
    weightImage->SetRegions( m_Images[c]->GetBufferedRegion() );
  ...
  #endif
    {
      const float  mean = m_Means[ classNumber ][c];
      const float  variance = 1.0/m_Precisions[ classNumber ][c][c];
    ...
    itk::ImageRegionConstIterator< WeightImageType >  weightIt( weightImage,
        weightImage->GetLargestPossibleRegion() );
    itk::ImageRegionConstIterator< ImageType >  imageIt( m_Images[c],
        m_Images[c]->GetLargestPossibleRegion() );
    itk::ImageRegionIterator< ResidueImageType >  residueIt( residueImage,
        residueImage->GetLargestPossibleRegion() );
    ...
    m_BiasFieldResidueSmoothers[c]->SetImage( residueImage );
    m_BiasFieldResidueSmoothers[c]->SetWeightImage( weightImage );
    m_BiasFieldResidueSmoothers[c]->SetPolynomialOrderUsed( biasFieldOrderUsed );
    if(c>=m_BiasFields.size()) m_BiasFields.resize(m_n_channels,0);
    m_BiasFields[c] = m_BiasFieldResidueSmoothers[c]->GetSmoothedImage().GetPointer();
 

    // Calculate corrected image
    imageIt.GoToBegin();
    itk::ImageRegionConstIterator< BiasFieldImageType >  biasIt( m_BiasFields[c],
        m_BiasFields[c]->GetLargestPossibleRegion() );
    itk::ImageRegionIterator< BiasCorrectedImageType >  correctedIt( m_BiasCorrectedImages[c],
        m_BiasCorrectedImages[c]->GetLargestPossibleRegion() );
...
::UpdateMixtureModelParameters()
{
  ...
  // Eugenio: for the sake of efficiency, we have 2 cases: #channels==1 and >1
  if(m_n_channels==1)
  {
    ...
    for ( unsigned int classNumber = 0; classNumber < m_NumberOfClasses; classNumber++ )
    {
      ...
      if ( classNumber == 0 )
      {
         typedef itk::CastImageFilter< ImageType, BiasCorrectedImageType >  CasterType;  CasterType::Pointer  caster = CasterType::New();   caster->SetInput( m_Image );  caster->Update();
        imageToUse = caster->GetOutput();
      }
  #endif
      ...
      numberOfPixels+=1e-6;  // Eugenio added this 
    ...
    {
      const unsigned int  independentClassNumber = m_IndependentParametersLookupTable[ classNumber ];
      vnl_vector<float> aux(1);
      aux[0]=independentMeans[ independentClassNumber ];
      m_Means.push_back(aux);
  #if 1
      vnl_matrix<float> aux2(1,1);
      aux2[0][0]=1/independentVariances[ independentClassNumber ];
      m_Precisions.push_back( aux2 );
  #else
      vnl_matrix<float> aux2(1,1);
      aux2[0][0]=1/globalVariance;
      m_Precisions.push_back( aux2 );
  #endif
  ...
  else  // multiple channels!
  {
    ...
    // Loop over all classes, adding the contributions for the parameter calculations
    std::vector< vnl_vector<float> >  independentLinearSum;
    std::vector< vnl_matrix<float> >  independentQuadraticSum;
    std::vector< float >  independentNumberOfPixels;
    ...
    { 
      ...
      itk::ImageRegionConstIterator< ClassificationImageType >
      posteriorIt( m_Posteriors[ classNumber ], m_Posteriors[ classNumber ]->GetLargestPossibleRegion() );
      vnl_vector<float>  linearSum(m_n_channels,0);
      vnl_matrix<float>  quadraticSum(m_n_channels,m_n_channels,0);
      float  numberOfPixels = 0.0f;
      vnl_vector<float> aux(m_n_channels);
     vnl_matrix<float> auxMat(m_n_channels,m_n_channels,0.0f);
      for ( ;  !imageIts[0].IsAtEnd();  ++posteriorIt ) 
      {
        for(int c=0; c<m_n_channels; c++){
          aux[c]=imageIts[c].Value();
        }

        linearSum += (posteriorIt.Value() * aux);

        vnl_matrix<float> auxMat(m_n_channels,m_n_channels,0.0f);
        for(int c1=0; c1<m_n_channels; c1++){
          for(int c2=c1; c2<m_n_channels; c2++){
	    auxMat[c1][c2]=aux[c1]*aux[c2];
            auxMat[c2][c1]=auxMat[c1][c2];
          }
        }
        quadraticSum += (posteriorIt.Value() * auxMat);

        numberOfPixels += posteriorIt.Value();
        for(int c=0; c<m_n_channels; c++)  ++(imageIts[c]);
      }
      numberOfPixels+=1e-6;
      unnormalizedWeights[ classNumber ] = numberOfPixels;
      ...
      if ( independentClassNumber >= independentLinearSum.size() )
      {
        vnl_vector<float>  aux1(m_n_channels,0.0f); 
        independentLinearSum.resize( independentClassNumber + 1, aux1 );
        vnl_matrix<float>  aux2(m_n_channels,m_n_channels,0.0f);
        independentQuadraticSum.resize( independentClassNumber + 1, aux2 );
        independentNumberOfPixels.resize( independentClassNumber + 1, 0.0f );
      }
      independentLinearSum[ independentClassNumber ] += linearSum;
      independentQuadraticSum[ independentClassNumber ] += quadraticSum;
    ...
    // Calculate independent parameters 
    std::vector< vnl_vector<float> >  independentMeans;
    std::vector< vnl_matrix<float> >  independentVariances;
    vnl_matrix<float>  globalVariance(m_n_channels,m_n_channels,0.0f);
    float  globalNumberOfPixels = 0.0f;
    for ( unsigned int independentClassNumber = 0;
          independentClassNumber < independentLinearSum.size();
          independentClassNumber++ )
    {
      const vnl_vector<float>  linearSum = independentLinearSum[ independentClassNumber ];
      const vnl_matrix<float>  quadraticSum = independentQuadraticSum[ independentClassNumber ];
      const float  numberOfPixels = independentNumberOfPixels[ independentClassNumber ];
  
      const vnl_vector<float>  mean = linearSum / numberOfPixels;
      vnl_matrix<float> variance = quadraticSum / numberOfPixels;
      vnl_matrix<float> auxMat2(m_n_channels,m_n_channels,0.0f);
        for(int c1=0; c1<m_n_channels; c1++){
          for(int c2=c1; c2<m_n_channels; c2++){
	    auxMat2[c1][c2]=mean[c1]*mean[c2];
            auxMat2[c2][c1]=auxMat2[c1][c2];
          }
        }
      variance=variance-auxMat2;
  ...
  #if 1
      m_Precisions.push_back(vnl_inverse(independentVariances[ independentClassNumber ]) );
  #else
      m_Precisions.push_back( vnl_inverse(globalVariance) );
  #endif
...
::DoOneEMIterationWithPartialVoluming()  
{
  ...
  //Eugenio: this method is very messy so I'm going to split the cases with one and multiple channels right away...
...
if(m_n_channels==1)
{
  ...
  itk::ImageRegionConstIterator< ImageType >  intensityIt( m_Images[0], m_Images[0]->GetBufferedRegion() );
  itk::ImageRegionConstIterator< SubvoxelPriorsImageType >  subvoxelPriorIt( m_SubvoxelPriorsImage,
      m_SubvoxelPriorsImage->GetBufferedRegion() );
  ...
  for ( ; !intensityIt.IsAtEnd(); ++intensityIt, ++subvoxelPriorIt )
  {
    ...
    for ( unsigned int classNumber = 0; classNumber < m_NumberOfClasses; classNumber++ )
    {
      ...
      const float  mean = m_Means[ classNumber ][0];
      const float  variance = 1.0f / m_Precisions[ classNumber ][0][0];
    ...
    if ( !dontBotherLookingFurther )
    {
      ...
      for ( unsigned int classNumber1 = 0; classNumber1 < m_NumberOfClasses; classNumber1++ )
      {
        for ( unsigned int classNumber2 = classNumber1+1; classNumber2 < m_NumberOfClasses; classNumber2++ )
        {
          ...
          const float  mean1 = m_Means[ classNumber1 ][0];
          const float  variance1 = 1.0f/m_Precisions[ classNumber1 ][0][0];
          const float  mean2 = m_Means[ classNumber2 ][0];
          const float  variance2 = 1.0f/m_Precisions[ classNumber2 ][0][0];
  ...
  for ( unsigned int classNumber = 0; classNumber < m_NumberOfClasses; classNumber++ )
  {
    ...
    vnl_vector<float> aux(1); aux[0]=independentMeans[ independentClassNumber ];
    m_Means.push_back( aux );
    vnl_matrix<float> aux2(1,1); aux2[0][0]=1.0f/independentVariances[ independentClassNumber ];
    m_Precisions.push_back( aux2 );
  }
...
////////////////////////////////////
else   // multiple image channels //
////////////////////////////////////
{

// Eugenio TODO: adapt this code to multiple channels
itkExceptionMacro( << "PV segmentation not available for multispectral data yet!" );
/*
itk::Array< float >  totalWeights( m_NumberOfClasses );
  itk::Array< float >  means( m_NumberOfClasses );
  itk::Array< float >  variancesTerm1( m_NumberOfClasses );
...
}  // end of multiple image channels
...
EMSegmenter
::BiasCorrect( ImageType::Pointer image, int upsamplingFactor, int channelIndex ) const
{
  ...
  BiasFieldImageType::ConstPointer  biasField = m_BiasFieldResidueSmoothers[channelIndex]->GetSmoothedImage( maskImage, upsamplingFactor ).GetPointer(); 
  ...
  biasCorrectedImage->SetRegions( m_Images[channelIndex]->GetBufferedRegion() );
  ...
  //itk::ImageRegionConstIterator< ImageType >  imageIt( image, image->GetBufferedRegion() );
  itk::ImageRegionConstIterator< BiasFieldImageType >  biasIt( m_BiasFields[channelIndex], m_BiasFields[channelIndex]->GetBufferedRegion() );
...
EMSegmenter
::ConvertToNativeImageType( BiasCorrectedImageType::Pointer image )
{
...
void
EMSegmenter
::SetImageAndSegmentItWithCurrentParametersAndWithoutBiasFieldCorrection( ImageType::Pointer image )
{
  std::vector<ImageType::Pointer> ims(1);
  ims[0]=image;
  SetImagesAndSegmentItWithCurrentParametersAndWithoutBiasFieldCorrection(ims);
}
...
void
EMSegmenter
::SetImagesAndSegmentItWithCurrentParametersAndWithoutBiasFieldCorrection( std::vector<ImageType::Pointer> images )
{
...
::GetSuperResolutionLikelihoods() const
{
  ...
  std::vector<itk::ImageRegionConstIteratorWithIndex< ImageType > > intensityIts;
  for(int c=0; c<m_n_channels; c++)
  {
    itk::ImageRegionConstIteratorWithIndex< ImageType >  intensityIt( m_Images[c], m_Images[c]->GetBufferedRegion() );
    intensityIts.push_back(intensityIt);
  }
  itk::ImageRegionConstIterator< SubvoxelPriorsImageType >  subvoxelPriorIt( m_SubvoxelPriorsImage,
      m_SubvoxelPriorsImage->GetBufferedRegion() );
  ...
  vnl_vector<float> OneOverSqrtDetCov(m_NumberOfClasses);
  std::vector<vnl_matrix<float> > Covs(m_NumberOfClasses);
  for(int classNumber=0; classNumber < m_NumberOfClasses; classNumber++)  // so we don't have to do it at each voxel...
  {  
    OneOverSqrtDetCov[classNumber] = sqrt(vnl_determinant(m_Precisions[classNumber]));
    Covs[classNumber]=vnl_inverse(m_Precisions[classNumber]);
  }

  float piTermMultiv = pow(2*PI,-0.5*m_n_channels);

  for ( ; !intensityIts[0].IsAtEnd(); ++subvoxelPriorIt )
  {
    ...
    for ( unsigned int classNumber = 0; classNumber < m_NumberOfClasses; classNumber++ )
    {
      ...
      // Eugenio: for the sake of efficiency...
      float  gauss=0;
      if(m_n_channels==1)
      {
        const float  mean = m_Means[ classNumber ][0];
        const float  variance = 1.0/m_Precisions[ classNumber ][0][0];
        gauss = exp( -pow( intensityIts[0].Value() - mean, 2 ) / 2.0f / variance ) /
                             sqrt( 2 * PI * variance );
      }
      else
      {
        for(int c=0; c<m_n_channels; c++) 
          dataV[c]=intensityIts[c].Value()-m_Means[ classNumber ][c];

        gauss= exp( -0.5 * dot_product(dataV,m_Precisions[classNumber]*dataV)) * OneOverSqrtDetCov[classNumber] * piTermMultiv;
      ...
      for ( int xStep = 0; xStep < m_PartialVolumeUpsamplingFactors[ 0 ]; xStep++ )
      {
        index[ 0 ] = intensityIts[0].GetIndex()[ 0 ] * m_PartialVolumeUpsamplingFactors[ 0 ] + xStep;
        for ( int yStep = 0; yStep < m_PartialVolumeUpsamplingFactors[ 1 ]; yStep++ )
        {
          index[ 1 ] = intensityIts[0].GetIndex()[ 1 ] * m_PartialVolumeUpsamplingFactors[ 1 ] + yStep;
          for ( int zStep = 0; zStep < m_PartialVolumeUpsamplingFactors[ 2 ]; zStep++ )
          {
            index[ 2 ] = intensityIts[0].GetIndex()[ 2 ] * m_PartialVolumeUpsamplingFactors[ 2 ] + zStep;
            counter++;
    ...
    for ( unsigned int classNumber1 = 0; classNumber1 < m_NumberOfClasses; classNumber1++ )
    {
      for ( unsigned int classNumber2 = classNumber1+1; classNumber2 < m_NumberOfClasses; classNumber2++ )
      {
        ...
        //Eugenio: again, we differentiate cases with single and multiple channels
        ...
        else
        {
          ...
          // Retrieve parameters of the pure tissue classes this PV class is mixing for
          vnl_vector<float> mean1 = m_Means[ classNumber1 ];
          vnl_matrix<float>  variance1 = Covs[ classNumber1 ];
          vnl_vector<float>  mean2 = m_Means[ classNumber2 ];
          vnl_matrix<float>  variance2 = Covs[ classNumber2 ];
          ...
          for ( int gaussianNumber = 0; gaussianNumber < numberOfGaussiansPerPV; gaussianNumber++ )
          {
            // Retrieve mean and variance for this sub-Gaussian
            const float  alpha = static_cast< float >( gaussianNumber + 1 ) / static_cast< float >( m_NumberOfSubvoxels );
            vnl_vector<float>  mean = alpha * mean1 + ( 1 - alpha ) * mean2;
            vnl_matrix<float>  variance = alpha * variance1 + ( 1 - alpha ) * variance2;
  
            // Calculate gauss
            for(int c=0; c<m_n_channels; c++) 
              dataV[c]=intensityIts[c].Value()-mean[c];

            gauss= exp( -0.5 * dot_product(dataV,vnl_inverse(variance)*dataV)) / sqrt(vnl_determinant(variance)) * piTermMultiv;   
...
EMSegmenter::ImageType::Pointer 
EMSegmenter
::GetSuperResolutionImage(int channelIndex) const
{
  ...
  for ( int i = 0; i < 3; i++ )
  {
    superResolutionSize[ i ] = m_Images[channelIndex]->GetBufferedRegion().GetSize()[ i ] * m_PartialVolumeUpsamplingFactors[ i ];
  }
  ...
  for ( itk::ImageRegionConstIteratorWithIndex< ImageType >  it( m_Images[channelIndex],
        m_Images[channelIndex]->GetBufferedRegion() );
        !it.IsAtEnd(); ++it )
  {
