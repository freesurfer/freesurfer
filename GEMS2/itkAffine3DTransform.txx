...
Affine3DTransform():Superclass(ParametersDimension)
{
  m_Offset.Fill( 0 );
  m_Matrix.SetIdentity();

#if 0  
  m_SharedLocalJacobian = JacobianType( SpaceDimension, ParametersDimension );
#endif  
  this->m_SharedLocalJacobian.Fill( 0.0 );
...
SetIdentity( void )
{
  ...
  parameters.Fill( NumericTraits< typename NumericTraits<ParametersType>::ValueType >::Zero );
  for (unsigned int i=6; i<9; i++)
    {
      parameters[i] = 1.0;
    }
...
ComputeJacobianWithRespectToParameters(const InputPointType & p, JacobianType & jacobian ) const
{
  ...
  //
  jacobian.SetSize( SpaceDimension, ParametersDimension );
  
  // First row
  jacobian[0][0] = 0;
  jacobian[0][1] = -scaleX * sinY * ( cosZ + gantryY * sinZ ) * p[0]
                             -scaleY * ( sinY * ( sinZ + gantryX * gantryZ * cosZ ) + gantryZ * cosY ) * p[1]
                             -scaleZ * ( gantryX * sinY * cosZ + cosY ) * p[2];
  jacobian[0][2] = scaleX * cosY * ( -sinZ + gantryY * cosZ ) * p[0] +
                             scaleY * cosY * ( cosZ - gantryX * gantryZ * sinZ ) * p[1] -
                             scaleZ * gantryX * cosY * sinZ * p[2];
  jacobian[0][3] = 1;
  jacobian[0][4] = 0;
  jacobian[0][5] = 0;
  jacobian[0][6] = cosY * ( cosZ + gantryY * sinZ ) * p[0];
  jacobian[0][7] = ( cosY * ( sinZ + gantryX * gantryZ * cosZ ) - gantryZ * sinY ) * p[1];
  jacobian[0][8] = ( gantryX * cosY * cosZ - sinY ) * p[2];
  jacobian[0][9] = scaleY * cosY * gantryZ * cosZ * p[1] +
                             scaleZ * cosY * cosZ * p[2];
  jacobian[0][10] = scaleX * cosY * sinZ * p[0];
  jacobian[0][11] = scaleY * ( cosY * gantryX * cosZ - sinY ) * p[1];

  // Second row
  jacobian[1][0] = scaleX * ( cosX * sinY * ( cosZ + gantryY * sinZ ) + sinX * ( sinZ - gantryY * cosZ ) ) * p[0] +
  ...
  jacobian[1][1] = scaleX * ( sinX * cosY * ( cosZ + gantryY * sinZ ) ) * p[0] +
                             scaleY * ( sinX * cosY * ( sinZ + gantryX * gantryZ * cosZ ) - gantryZ * sinX * sinY ) * p[1] +
                             scaleZ * ( -sinX * sinY + gantryX *sinX * cosY * cosZ ) * p[2];
  jacobian[1][2] = scaleX * ( sinX * sinY * ( -sinZ + gantryY * cosZ ) - cosX * ( cosZ + gantryY * sinZ ) ) * p[0] +
  ...
  jacobian[1][4] = 1;
  jacobian[1][5] = 0;
  jacobian[1][6] = ( sinX * sinY * ( cosZ + gantryY * sinZ ) - cosX * ( sinZ - gantryY * cosZ ) ) * p[0];
  jacobian[1][7] = ( sinX * sinY * ( sinZ + gantryX * gantryZ * cosZ ) + cosX * ( cosZ - gantryX * gantryZ * sinZ )
                               + gantryZ * sinX * cosY ) * p[1];
  jacobian[1][8] = ( sinX * cosY + gantryX * ( sinX * sinY * cosZ - cosX * sinZ ) ) * p[2];
  jacobian[1][9] = scaleY * ( sinX * sinY * gantryZ * cosZ - cosX * gantryZ * sinZ ) * p[1] +
                             scaleZ * ( sinX * sinY * cosZ - cosX * sinZ ) * p[2];
  jacobian[1][10] = scaleX * ( sinX * sinY * sinZ + cosX * cosZ ) * p[0];
  jacobian[1][11] = scaleY * ( sinX * sinY * gantryX * cosZ - cosX * gantryX * sinZ + sinX * cosY ) * p[1];

  // Third row
  jacobian[2][0] = scaleX * ( -sinX * sinY * ( cosZ + gantryY * sinZ ) + cosX * ( sinZ - gantryY * cosZ ) ) * p[0] +
  ...
  jacobian[2][1] = scaleX * cosX * cosY * ( cosZ + gantryY * sinZ ) * p[0] +
                              scaleY * ( cosX * cosY * ( sinZ + gantryX * gantryZ * cosZ ) - gantryZ * cosX * sinY ) * p[1] +
                              scaleZ * ( -cosX * sinY + gantryX * cosX * cosY * cosZ ) * p[2];
  jacobian[2][2] = scaleX * ( cosX * sinY * ( -sinZ + gantryY * cosZ ) + sinX * ( cosZ + gantryY * sinZ ) ) * p[0] +
  ...
  jacobian[2][4] = 0;
  jacobian[2][5] = 1;
  jacobian[2][6] = ( cosX * sinY * ( cosZ + gantryY * sinZ ) + sinX * ( sinZ - gantryY * cosZ ) ) * p[0];
  jacobian[2][7] = ( cosX * sinY * ( sinZ + gantryX * gantryZ * cosZ ) - sinX * ( cosZ - gantryX * gantryZ * sinZ )
                               + gantryZ * cosX * cosY ) * p[1];
  jacobian[2][8] = ( cosX * cosY + gantryX * ( cosX * sinY * cosZ + sinX * sinZ ) ) * p[2];
  jacobian[2][9] = scaleY * ( cosX * sinY * gantryZ * cosZ + sinX * gantryZ * sinZ ) * p[1] +
                             scaleZ * ( cosX * sinY * cosZ + sinX * sinZ ) * p[2];
  jacobian[2][10] = scaleX * ( cosX * sinY * sinZ - sinX * cosZ ) * p[0];
  jacobian[2][11] = scaleY * ( cosX * sinY * gantryX * cosZ + sinX * gantryX * sinZ + cosX * cosY ) * p[1];
