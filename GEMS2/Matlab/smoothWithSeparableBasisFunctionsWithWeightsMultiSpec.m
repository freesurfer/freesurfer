...
for channel = 1:numberOfImages
    tmp = cache.resolution{numberOfBasisFunctions}.Ax' * reshape( sum(weights(:,:,:,numberOfImages*(channel-1)+1:numberOfImages*channel) .* data(:,:,:,numberOfImages*(channel-1)+1:numberOfImages*channel),4), [ Nx Ny*Nz ] );
