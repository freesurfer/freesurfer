function [ smoothedData, C ] = smoothWithSeparableBasisFunctionsWithWeights( data, weights, numberOfBasisFunctions, maximumNumberOfBasisFunctions )
%
%
...
%tic
tmp = cache.resolution{numberOfBasisFunctions}.Ax' * reshape( weights .* data, [ Nx Ny*Nz ] );
...
%
  ...
  smoothedData = smoothWithSeparableBasisFunctionsWithWeights( data, weights, ...
                                                               numberOfBasisFunctions, ...
                                                               maximumNumberOfBasisFunctions );
  for numberOfBasisFunctions = 1 : maximumNumberOfBasisFunctions
    ...
    for i = 1 : 4
      ...
      disp( [ '  run ' num2str( i ) ] )
      disp( '  +++++' )
      smoothedData = smoothWithSeparableBasisFunctionsWithWeights( data, weights, numberOfBasisFunctions );
    end

    % Show result
    figure
    showThreeDImage( smoothedData, false, [ min( data(: ) ) max( data(:) ) ] );
