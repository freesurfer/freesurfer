/**
 * @file  kvlAtlasMeshCollection.cxx
...
void
AtlasMeshCollection
::GenerateFromSingleMesh( AtlasMesh* mesh, unsigned int numberOfMeshes, float K )
{
  ...
  if ( !m_ReferenceTetrahedronInfos )
  {
          ...
          ++cellIt )
    {
      ...
      if( cell->GetType() == AtlasMesh::CellType::TETRAHEDRON_CELL )
      {
        ...
        const float x0 = m_ReferencePosition->ElementAt( *pit )[ 0 ];
        const float y0 = m_ReferencePosition->ElementAt( *pit )[ 1 ];
        const float z0 = m_ReferencePosition->ElementAt( *pit )[ 2 ];
        ++pit;
        const float x1 = m_ReferencePosition->ElementAt( *pit )[ 0 ];
        const float y1 = m_ReferencePosition->ElementAt( *pit )[ 1 ];
        const float z1 = m_ReferencePosition->ElementAt( *pit )[ 2 ];
        ++pit;
        const float x2 = m_ReferencePosition->ElementAt( *pit )[ 0 ];
        const float y2 = m_ReferencePosition->ElementAt( *pit )[ 1 ];
        const float z2 = m_ReferencePosition->ElementAt( *pit )[ 2 ];
        ++pit;
        const float x3 = m_ReferencePosition->ElementAt( *pit )[ 0 ];
        const float y3 = m_ReferencePosition->ElementAt( *pit )[ 1 ];
        const float z3 = m_ReferencePosition->ElementAt( *pit )[ 2 ];
        ...
        const float referenceVolume = ( lambda11 * ( lambda22*lambda33 - lambda32*lambda23 )
        ...
        vnl_matrix_fixed< float, 4, 4 >  referenceMatrix;
        ...
        vnl_matrix_fixed< float, 4, 4 >  inverseReferenceMatrix = vnl_inverse( referenceMatrix );
