...
template <class T>
vnl_matrix<T> const& vnl_qr<T>::Q( bool useEconomySize ) const
{
  int m = qrdc_out_.columns(); // column-major storage
  int n = qrdc_out_.rows();

  int clippedM = m;
  if ( useEconomySize && ( m > n ) )
    {
    clippedM = n;
    }
  ...
  if (!Q_) {
    ((vnl_matrix<T>*&)Q_) = new vnl_matrix<T>(m,clippedM);
    // extract Q.
    if (verbose) {
...
//: Unpack and return R.
template <class T>
vnl_matrix<T> const& vnl_qr<T>::R( bool useEconomySize ) const
{
  if (!R_) {
    int m = qrdc_out_.columns(); // column-major storage
    int n = qrdc_out_.rows();
    ...
    ((vnl_matrix<T>*&)R_) = new vnl_matrix<T>(clippedM,n);
    vnl_matrix<T> & R = *R_;

    for (int i = 0; i < clippedM; ++i)
      for (int j = 0; j < n; ++j)
        if (i > j)
          R(i, j) = T(0);
        else
