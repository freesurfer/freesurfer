...
static MATRIX *R0 ;
static char vcid[] = "$Id: mris_register_to_label.c,v 1.1 2013/03/18 12:53:24 fischl Exp $";
int
main(int argc, char **argv) 
{
  ...
  make_cmd_version_string(argc, argv,
                          "$Id: mris_register_to_label.c,v 1.1 2013/03/18 12:53:24 fischl Exp $",
                          "$Name:  $", cmdline);
  ...
  if (area == NULL)
    ErrorExit(Gerror, "") ;
  ltmp = LabelRemoveAlmostDuplicates(area, 1, NULL) ;
			  ...
			  double ax0, double ay0, double az0, MRI *mri_targ_vol)
{
  ...
  *MATRIX_RELT(m_origin_inv, 3, 4) *= -1 ;
  ...
  if (write_diags)
  {
    ...
    if (debug)
      sprintf(fname, "%s.opt.%3.3d.label", "debug",nfound) ;
    else
      sprintf(fname, "%s.opt.%3.3d.label", base_name,nfound) ;
    ...
    LabelWrite(l, fname) ;
    if (debug)
      sprintf(fname, "%s.opt.%3.3d.mgz", "debug",nfound) ;
    else
      sprintf(fname, "%s.opt.%3.3d.mgz", base_name,nfound) ;
    printf("saving %s\n", fname) ;
		  ...
		  if (debug)
			...
			if (write_diags)
			{
			  ...
			  if (mri_targ_vol)
			  {
			    ...
			    sprintf(fname, "%s.targ.%3.3d.mgz", base_name,nfound) ;
			    ...
			    sprintf(fname, "%s.%3.3d.%s", fname_only, nfound, ext) ;
