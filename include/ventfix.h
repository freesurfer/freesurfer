...
public:
  ...
  static MRI *relabelSegANeighboringSegB(MRI *asegps, int segA, int topo, int *segB, int *newsegid, int numSegs, fsPointSet *centroid);
...
private:
  ...
  static int hasneighbor(MRI* asegps, VOLCLUSTER *vc, int *newsegid, int numSegs, int topo);
