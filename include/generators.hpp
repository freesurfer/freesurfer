...
public:
  ...
  __device__ __host__ float operator() ( const unsigned int i ) const {
    float res = errVal;
  ...
  LinearGenerator( const float myMin,
		   const float myMax,
...
public:
  ...
  __host__ __device__
  float3 operator() ( unsigned int targetIndex ) const {
    float3 res;
  ...
  TranslationGenerator( const float myMin,
			const float myMax,
...
public:
  ...
  __host__ __device__
  void GetTransform( unsigned int targetIndex,
  ...
  TransformGenerator( const LinearGenerator genTrans,
