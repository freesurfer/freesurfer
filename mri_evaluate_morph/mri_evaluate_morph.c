...
static char vcid[] = "$Id: mri_evaluate_morph.c,v 1.3 2003/09/05 04:45:33 kteich Exp $";
...
main(int argc, char *argv[])
{
  ...
  nargs = handle_version_option (argc, argv, "$Id: mri_evaluate_morph.c,v 1.3 2003/09/05 04:45:33 kteich Exp $", "$Name:  $");
  ...
  for (i = FIRST_SUBJECT ; i < argc-1 ; i++)
  {
	...
	for (total_overlap = 0.0f, i = 0 ; i < nsubjects ; i++)
	{
		for (j = i+1 ; j < nsubjects ; j++)
		{
			...
			fflush(fp) ;
			TransformFree(&transform1) ;  TransformFree(&transform2) ; 
		}
	}
								...
								TRANSFORM *transform2)
{
	...
	width = mri_seg1->width ; height = mri_seg1->height ; depth = mri_seg1->depth ; 
